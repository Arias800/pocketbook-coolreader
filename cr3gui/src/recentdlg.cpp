...
CRRecentBookMenuItem::CRRecentBookMenuItem(CRMenu *menu, int index,
                                           CRFileHistRecord *book)
    : CRMenuItem(menu, index, lString16::empty_str, LVImageSourceRef(),
                 LVFontRef()),
      _book(book) {}

void CRRecentBookMenuItem::Draw(LVDrawBuf &buf, lvRect &rc, CRRectSkinRef skin,
                                CRRectSkinRef valueSkin, bool selected) {
  if (!_book) {
...
void CRRecentBooksMenu::showContextMenu() {
  ...
  OpenMenu(_contextMenu, MCMD_OPEN_RECENT_BOOK, ScreenWidth() / 4, y,
           handle_contextMenu);
}
...
CRRecentBooksMenu::CRRecentBooksMenu(CRGUIWindowManager *wm, LVDocView *docview,
                                     int numItems, lvRect &rc)
    : CRFullScreenMenu(wm, MCMD_BOOKMARK_LIST, lString16(_("Open recent book")),
                       numItems, rc) {
