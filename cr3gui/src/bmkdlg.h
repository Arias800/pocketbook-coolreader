...
public:
  ...
  void setBookmark(CRBookmark *bookmark) {
    _bookmark = bookmark;
    _itemDirty = true;
  }
...
public:
  ...
  CRBookmarkMenu(CRGUIWindowManager *wm, LVDocView *docview, int numItems,
                 lvRect &rc, bool goToMode = false);
#ifdef CR_POCKETBOOK
...
public:
  /// returns index of selected item, -1 if no item selected
  virtual int getSelectedItemIndex();
  CRCitesMenu(CRGUIWindowManager *wm, LVDocView *docview, int numItems,
              lvRect &rc);
#ifdef CR_POCKETBOOK
  void showContextMenu();
  void handleContextMenu(int index);
