...
CRTocMenu::CRTocMenu(CRGUIWindowManager *wm, CRPropRef newProps, int id,
  ...
  if (length) {
    ...
    for (int i = 0; i < length; i++) {
      ...
      mainMenu->addItem(
          new CRMenuItem(mainMenu, PB_TOC_SAFE_CMD_RANGE + tocItems[i].page,
                         UnicodeToUtf8(text).c_str(), LVImageSourceRef(),
                         LVFontRef() /*,
  ...
  // Empty contents - shouldn't reach this
    ...
    mainMenu->addItem(new CRMenuItem(mainMenu, PB_TOC_SAFE_CMD_RANGE + 1,
                                     "Empty TOC", LVImageSourceRef(),
                                     LVFontRef()));
  }
