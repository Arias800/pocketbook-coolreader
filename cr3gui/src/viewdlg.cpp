...
static const char *def_view_css =
    "body { text-align: left; text-indent: 0px; font-family: \"DejaVu Sans\", "
    "\"FreeSans\", \"Arial\", sans-serif; }\n"
    "p { text-align: justify; text-indent: 2em; margin-top:0em; margin-bottom: "
    "0em }\n"
    "empty-line { height: 1em }\n"
    "hr { height: 1px; background-color: #808080; margin-top: 0.5em; "
    "margin-bottom: 0.5em }\n"
    ...
    "title p, subtitle p, h1 p, h2 p, h3 p, h4 p, h5 p, h6 p { text-align: "
    ...
    "epigraph { margin-left: 25%; margin-right: 1em; text-align: left; "
    "text-indent: 1px; font-style: italic; margin-top: 15px; margin-bottom: "
    "25px }\n"
    "cite { font-style: italic; margin-left: 5%; margin-right: 5%; text-align: "
    "justyfy; margin-top: 20px; margin-bottom: 20px }\n"
    "title, h1, h2 { text-align: center; text-indent: 0px; font-weight: bold; "
    "hyphenate: none; page-break-before: always; page-break-inside: avoid; "
    "page-break-after: avoid; }\n"
    "subtitle, h3, h4, h5, h6 { text-align: center; text-indent: 0px; "
    "font-weight: bold; hyphenate: none; page-break-inside: avoid; "
    "page-break-after: avoid; }\n"
...
CRViewDialog::CRViewDialog(CRGUIWindowManager *wm, lString16 title,
                           lString8 text, lvRect rect, bool showScroll,
                           bool showFrame)
    : CRDocViewWindow(wm), _text(text), _showScroll(showScroll),
      _showFrame(showFrame), _lastNavigationDirection(0) {
  ...
  getDocView()->setDrawBufferBits(
      _wm->getScreen()->getCanvas()->GetBitsPerPixel());
...
bool CRViewDialog::hasDictionaries() {
  ...
  if (_dict.isNull())
    _dict =
        LVRef<CRDictionary>(new CRTinyDict(Utf8ToUnicode(lString8(dict_path))));
  if (!_dict->empty()) {
    return true;
  }
  // show warning
  ...
  body << "<p>" << _("Place dictionaries to directory 'dict' of SD card.")
       << "</p>";
  body << "<p>"
       << _("Dictionaries in standard unix .dict format are supported.")
       << "</p>";
  body << "<p>"
       << _("For each dictionary, pair of files should be provided: data file "
            "(with .dict or .dict.dz extension, and index file with .index "
            "extension")
       << "</p>";
  lString8 xml = CRViewDialog::makeFb2Xml(body);
  CRViewDialog *dlg = new CRViewDialog(_wm, lString16(_("Dictionary")), xml,
                                       lvRect(), true, true);
...
#endif
    dlg =
        new CRTOCDialog(_wm, lString16(_("Table of contents")),
                        MCMD_GO_PAGE_APPLY, _docview->getPageCount(), _docview);
#if USE_SEPARATE_GO_TO_PAGE_DIALOG == 1
  } else {
    dlg = new CRNumberEditDialog(_wm, lString16(_("Enter page number")),
                                 lString16::empty_str, MCMD_GO_PAGE_APPLY, 1,
...
void CRViewDialog::showGoToPercentDialog() {
  CRNumberEditDialog *dlg;
  dlg = new CRNumberEditDialog(_wm, lString16(_("Enter position percent")),
                               lString16::empty_str, MCMD_GO_PERCENT_APPLY, 0,
                               100);
  dlg->setAccelerators(getDialogAccelerators());
  _wm->activateWindow(dlg);
}
...
void CRViewDialog::showSearchDialog() {
  ...
  CRScreenKeyboard *dlg = new CRScreenKeyboard(
      _wm, MCMD_SEARCH_FINDFIRST, lString16(_("Search")), _searchPattern, rc);
  _wm->activateWindow(dlg);
}
...
bool CRViewDialog::findInDictionary(lString16 pattern) {
  ...
  if (_dict.isNull()) {
    showWaitIcon();
    _dict = LVRef<CRDictionary>(
        new CRTinyDict(Utf8ToUnicode(lString8(DICTD_CONF))));
  }
...
bool CRViewDialog::findText(lString16 pattern, int origin, int direction) {
  ...
  CRLog::debug("CRViewDialog::findText: Current page: %d .. %d", rc.top,
               rc.bottom);
  CRLog::debug(
      "CRViewDialog::findText: searching for text '%s' from %d to %d origin %d",
      LCSTR(pattern), start, end, origin);
  if (_docview->getDocument()->findText(pattern, true, reverse, start, end,
                                        words, 200, pageHeight)) {
...
int CRViewDialog::findPagesText(lString16 pattern, int origin, int direction) {
  if (pattern.empty())
    return -1;
  if (!(origin == 0 || origin == -1 || origin == 1 || direction == 1 ||
        direction == -1))
    return -1;
  showWaitIcon();
  int start, end;
  ...
  for (int i = start; i != end; i += direction) {
    ...
    if (!range.isNull()) {
      if (range->findText(pattern, true, false, words, 200,
                          _docview->getPageHeight(i), true)) {
...
void CRViewDialog::showDictWithVKeyboard() {
  ...
  _searchPattern.clear();
  CRScreenKeyboard *dlg = new CRScreenKeyboard(
      _wm, MCMD_DICT_FIND, lString16(_("Find in dictionary")), _searchPattern,
      rc);
  _wm->activateWindow(dlg);
}
...
static const char *getCommandName(int command) {
  switch (command) {
  case DCMD_BEGIN:
    return _("To first page");
  case DCMD_LINEUP:
  case DCMD_PAGEUP:
    return _("Back by page");
  case DCMD_PAGEDOWN:
  case DCMD_LINEDOWN:
    return _("Forward by page");
  case DCMD_LINK_FORWARD:
    return _("Forward in move history");
  case DCMD_LINK_BACK:
    return _("Back in move history");
  case DCMD_LINK_NEXT:
    return _("Next link");
  case DCMD_LINK_PREV:
    return _("Previous link");
  case DCMD_LINK_GO:
    return _("Go to link");
  case DCMD_END:
    return _("To last page");
  case DCMD_GO_POS:
    return _("Go to position");
  case DCMD_GO_PAGE:
    return _("Go to page");
  case DCMD_ZOOM_IN:
    return _("Zoom in");
  ...
  case DCMD_TOGGLE_BOLD:
    return _("Toggle bold text");
  case DCMD_TOGGLE_TEXT_FORMAT:
    return _("Toggle text format");
  case DCMD_BOOKMARK_SAVE_N:
    return _("Save bookmark by number");
  case DCMD_BOOKMARK_GO_N:
    return _("Go to bookmark by number");
  case DCMD_MOVE_BY_CHAPTER:
    return _("Move by chapter");
  case DCMD_LINK_FIRST:
    return _("First link");
  case DCMD_ROTATE_BY:
    return _("Rotate by");
  case DCMD_ROTATE_SET:
    return _("Set rotate angle");
  case DCMD_SAVE_HISTORY:
    return _("Save history and bookmarks");
  case DCMD_SAVE_TO_CACHE:
    return _("Save document to cache");
  case MCMD_CANCEL:
    return _("Close dialog");
  case MCMD_OK:
    return ("Ok");
  case MCMD_SELECT:
    return ("Ok");
  case MCMD_SCROLL_FORWARD:
    return _("Scroll forward");
  case MCMD_SCROLL_BACK:
    return _("Scroll back");
  case MCMD_QUIT:
    return _("Close book");
  case MCMD_MAIN_MENU:
    return _("Main menu");
  case MCMD_GO_PAGE:
    return _("Go to page dialog");
  case MCMD_SETTINGS:
    return _("Settings menu");
  case MCMD_SETTINGS_FONTSIZE:
    return _("Font size settings");
  case MCMD_SETTINGS_ORIENTATION:
    return _("Page orientation settings");
  case MCMD_GO_LINK:
    return _("Go to link");
#ifdef CR_POCKETBOOK
  case MCMD_DICT:
...
#else
  case MCMD_DICT:
    return _("Find in Dictionary (T5)");
#endif
  case MCMD_BOOKMARK_LIST:
    return _("Bookmark list");
  case MCMD_CITES_LIST:
    return _("Citations list");
  case MCMD_BOOKMARK_LIST_GO_MODE:
    return _("Go to bookmark...");
  case MCMD_RECENT_BOOK_LIST:
    return _("Recent books list");
  case MCMD_OPEN_RECENT_BOOK:
    return _("Open recent book by number");
  case MCMD_SWITCH_TO_RECENT_BOOK:
    return _("Switch to recent book");
  case MCMD_ABOUT:
    return _("About");
  case MCMD_CITE:
    return _("Cite selection dialog");
  case MCMD_SEARCH:
    return _("Search dialog");
  case MCMD_DICT_VKEYBOARD:
    return _("Find in Dictionary (virtual keyboard)");
  case MCMD_KBD_NEXTLAYOUT:
    return _("Next keyboard layout");
  case MCMD_KBD_PREVLAYOUT:
    return _("Previous keyboard layout");
  case MCMD_HELP:
    return _("Show manual");
  case MCMD_HELP_KEYS:
    return _("Show key mapping");
  case MCMD_GO_PERCENT:
    return _("Go to percent...");
#ifdef CR_POCKETBOOK
  case PB_QUICK_MENU:
    return TR("@KA_menu");
  case PB_CMD_ROTATE:
...
void CRViewDialog::showKeymapDialog() {
  ...
  lString8 txt;
  txt << "<table><tr><th>";
  txt << "<b>" << _("Key") << "</b></th><th><b>" << _("Assigned function")
      << "</b></th></tr>";
  ...
  CRViewDialog *dlg = new CRViewDialog(_wm, lString16(_("Keyboard layout")),
...
void CRViewDialog::setRect(const lvRect &rc) {
  // if ( rc == _rect )
  //     return;
  CRLog::info("CRViewDialog::setRect(%d,%d,%d,%d)", rc.left, rc.top, rc.right,
              rc.bottom);
  _rect = rc;
  lvRect clientRect = _rect;
  CRWindowSkinRef skin = getSkin();
...
void CRViewDialog::prepareNextPageImage(int offset) {
  if (_wm->getScreen()->getTurboUpdateEnabled() &&
      _wm->getScreen()->getTurboUpdateSupported()) {
