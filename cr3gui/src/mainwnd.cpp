...
V3DocViewWin::V3DocViewWin(CRGUIWindowManager *wm, lString16 dataDir)
    : CRViewDialog(wm, lString16::empty_str, lString8::empty_str, lvRect(),
                   false, false),
      _dataDir(dataDir), _loadFileStart(0) {
...
#define BATTERY_HEADER                                                         \
  "36 19 5 1", "0 c #000000", "X c #000000", "o c #AAAAAA", ". c #FFFFFF",     \
      "  c None",
...
void V3DocViewWin::OnFormatEnd() {
  ...
  if (t - _loadFileStart >= SECONDS_BEFORE_PROGRESS_BAR)
    ...
    // _docview->updateCache(infinite) is called here. To implement background
    // cache file saving, schedule on Idle state following task: in each idle
    // cycle call _docview->updateCache(timeOut) while it returns CR_TIMEOUT
...
bool V3DocViewWin::loadCSS(lString16 filename) {
  ...
  if (LVLoadStylesheetFile(filename, css)) {
    ...
    // CRLog::debug("Stylesheet file not found %s",
...
bool V3DocViewWin::loadHistory(lString16 filename) {
  CRLog::trace("V3DocViewWin::loadHistory( %s )",
               UnicodeToUtf8(filename).c_str());
...
bool V3DocViewWin::loadDocument(lString16 filename) {
  ...
  if (!_docview->LoadDocument(filename.c_str())) {
    CRLog::error("V3DocViewWin::loadDocument( %s ) - failed!",
                 UnicodeToUtf8(filename).c_str());
    return false;
  }
...
bool V3DocViewWin::saveHistory(lString16 filename, bool exportBookmarks) {
  ...
  if (exportBookmarks) {
    CRLog::debug("Exporting bookmarks to %s",
                 UnicodeToUtf8(_bookmarkDir).c_str());
  ...
  if (!stream) {
    ...
    CRLog::warn("Cannot create settings file, trying to create directory %s",
                path.c_str());
  ...
  if (stream.isNull()) {
    CRLog::error("Error while creating history file %s - position will be lost",
                 UnicodeToUtf8(filename).c_str());
    return false;
  }
...
bool V3DocViewWin::loadSettings(lString16 filename) {
  ...
  if (stream.isNull()) {
    ...
    _wm->getScreen()->setFullUpdateInterval(
        _props->getIntDef(PROP_DISPLAY_FULL_UPDATE_INTERVAL, 1));
    _wm->getScreen()->setTurboUpdateEnabled(
        _props->getIntDef(PROP_DISPLAY_TURBO_UPDATE_MODE, 0));
...
void V3DocViewWin::showSettingsMenu() {
  LVFontRef menuFont(fontMan->GetFont(MENU_FONT_SIZE, 600, true,
  ...
  CRMenu *mainMenu = new CRSettingsMenu(_wm, _newProps, MCMD_SETTINGS_APPLY,
                                        menuFont, getMenuAccelerators(), rc);
...
void V3DocViewWin::showTocTouchMenu(tocentry *tocItems, int length,
  ...
  lvRect rc = _wm->getScreen()->getRect();
  CRMenu *mainMenu =
      new CRTocMenu(_wm, _newProps, MCMD_TOC_TOUCH, getMenuAccelerators(), rc,
                    tocItems, length, currentPage);
...
void V3DocViewWin::showFontSizeMenu() {
  ...
  CRMenu *menu = CRSettingsMenu::createFontSizeMenu(
      _wm, NULL, cr_font_sizes, sizeof(cr_font_sizes) / sizeof(int),
...
lString16 V3DocViewWin::getHelpFile() { return _helpFile; }
...
void V3DocViewWin::showHelpDialog() {
  ...
  if (stream.isNull()) {
    ...
    lString8 body;
    body << "<title><p>"
         << _("No manual currently available for this language, sorry!")
         << "</p></title>";
  ...
  if (!help.empty()) {
    CRViewDialog *dlg =
        new CRViewDialog(_wm, lString16(_("Help")), help, lvRect(), true, true);
...
void V3DocViewWin::showMainMenu() {
  ...
  CRFullScreenMenu *menu_win =
      new CRFullScreenMenu(_wm, 0, lString16(_("Main Menu")), 8, fullRc);
  /*
  ...
  */
  ...
  CRGUIAcceleratorTableRef menuItems =
      _wm->getAccTables().get(lString16("mainMenuItems"));
  if (!menuItems.isNull() && menuItems->length() > 1) {
    ...
    for (int i = 0; i < menuItems->length(); i++) {
      ...
      menu_win->addItem(new CRMenuItem(menu_win, cmd, label.c_str(),
                                       LVImageSourceRef(), LVFontRef()));
    }
  } else {
    // default menu
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_ABOUT, _("About"),
                                     LVImageSourceRef(), LVFontRef()));
#if 0
		menu_win->addItem( new CRMenuItem( menu_win, DCMD_BEGIN,
					...
					LVFontRef() ) );
#endif
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_GO_PAGE, _("Go to page"),
                                     LVImageSourceRef(), LVFontRef()));
#if 0
    ...
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_RECENT_BOOK_LIST,
                                     _("Open recent book"), LVImageSourceRef(),
                                     LVFontRef()));

#ifdef WITH_DICT
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_DICT, _("Dictionary"),
                                     LVImageSourceRef(), LVFontRef()));
#endif
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_CITE, _("Cite"),
                                     LVImageSourceRef(), LVFontRef()));
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_BOOKMARK_LIST,
                                     _("Bookmarks"), LVImageSourceRef(),
                                     LVFontRef()));
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_SEARCH, _("Search"),
                                     LVImageSourceRef(), LVFontRef()));
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_SETTINGS, _("Settings"),
                                     LVImageSourceRef(), LVFontRef()));
    if (!_helpFile.empty())
      menu_win->addItem(new CRMenuItem(menu_win, MCMD_HELP, _("Help"),
                                       LVImageSourceRef(), LVFontRef()));
    menu_win->addItem(new CRMenuItem(menu_win, MCMD_HELP_KEYS,
                                     _("Keyboard layout"), LVImageSourceRef(),
                                     LVFontRef()));
  }
...
void addPropLine(lString8 &buf, const char *description,
                 const lString16 &value) {
  if (!value.empty()) {
    buf << "<tr><td>" << description << "</td><td>"
        << UnicodeToUtf8(value).c_str() << "</td></tr>\n";
  }
}
...
lString16 getDocAuthors(ldomDocument *doc, const char *path,
                        const char *delim) {
  lString16 res;
  for (int i = 0; i < 100; i++) {
    lString8 p = lString8(path) + "[" + lString8::itoa(i + 1) + "]";
...
static void addInfoSection(lString8 &buf, lString8 data, const char *caption) {
  if (!data.empty())
    buf << "<tr><td colspan=\"2\" style=\"font-weight: bold; text-align: "
           "center\">"
        << caption << "</td></tr>" << data;
}
...
#define PACKAGE_VERSION CR_ENGINE_VERSION
  ...
  lString8 statusInfo;
  addPropLine(statusInfo, _("Current page"),
              lString16::itoa(_docview->getCurPage() + 1));
  addPropLine(statusInfo, _("Total pages"),
              lString16::itoa(_docview->getPageCount()));
  addPropLine(statusInfo, _("Battery state"),
              _docview->getBatteryState() == -1
                  ? lString16(_("charging..."))
                  : lString16::itoa(_docview->getBatteryState()) + "%");
  ...
  lString8 fileInfo;
  addPropLine(fileInfo, _("Archive name"),
              props->getStringDef(DOC_PROP_ARC_NAME));
  addPropLine(fileInfo, _("Archive path"),
              props->getStringDef(DOC_PROP_ARC_PATH));
  addPropLine(fileInfo, _("Archive size"),
              props->getStringDef(DOC_PROP_ARC_SIZE));
  addPropLine(fileInfo, _("File name"),
              props->getStringDef(DOC_PROP_FILE_NAME));
  addPropLine(fileInfo, _("File path"),
              props->getStringDef(DOC_PROP_FILE_PATH));
  addPropLine(fileInfo, _("File size"),
              props->getStringDef(DOC_PROP_FILE_SIZE));
  addPropLine(fileInfo, _("File format"),
              props->getStringDef(DOC_PROP_FILE_FORMAT));
  addInfoSection(txt, fileInfo, _("File info"));

  lString8 bookInfo;
  addPropLine(bookInfo, C_("Book", "Title"),
              props->getStringDef(DOC_PROP_TITLE));
  addPropLine(bookInfo, _("Author(s)"), props->getStringDef(DOC_PROP_AUTHORS));
  addPropLine(bookInfo, _("Series name"),
              props->getStringDef(DOC_PROP_SERIES_NAME));
  addPropLine(bookInfo, _("Series number"),
              props->getStringDef(DOC_PROP_SERIES_NUMBER));
  addPropLine(bookInfo, _("Date"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/title-info/date", ", "));
  addPropLine(bookInfo, _("Genres"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/title-info/genre", ", "));
  addPropLine(bookInfo, _("Language"), props->getStringDef(DOC_PROP_LANGUAGE));
  addPropLine(bookInfo, _("Translator"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/title-info/translator",
                         ", "));
  addPropLine(bookInfo, _("Source language"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/title-info/src-lang", ", "));
  addInfoSection(txt, bookInfo, _("Book info"));

  lString8 docInfo;
  addPropLine(docInfo, _("Document author"),
              getDocAuthors(getDocView()->getDocument(),
                            "/FictionBook/description/document-info/author",
                            " "));
  addPropLine(docInfo, _("Document date"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/date", " "));
  addPropLine(docInfo, _("Document source URL"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/src-url",
                         " "));
  addPropLine(docInfo, _("OCR by"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/src-ocr",
                         " "));
  addPropLine(docInfo, _("Document version"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/version",
                         " "));
  addPropLine(docInfo, _("Change history"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/history",
                         " "));
  addPropLine(docInfo, _("ID"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/id", " "));
  addPropLine(docInfo, _("Program used"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/document-info/program-used",
                         " "));
  addInfoSection(txt, docInfo, _("Document info"));

  lString8 pubInfo;
  addPropLine(pubInfo, _("Publication name"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/publish-info/book-name",
                         " "));
  addPropLine(pubInfo, _("Publisher"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/publish-info/publisher",
                         " "));
  addPropLine(pubInfo, _("Publisher city"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/publish-info/city", " "));
  addPropLine(pubInfo, _("Publication year"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/publish-info/year", " "));
  addPropLine(pubInfo, _("ISBN"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/publish-info/isbn", " "));
  addInfoSection(txt, pubInfo, _("Publication info"));

  addPropLine(txt, _("Custom info"),
              getDocText(getDocView()->getDocument(),
                         "/FictionBook/description/custom-info", " "));

#if defined(CR_PB_VERSION) && defined(CR_PB_BUILD_DATE)
  lString8 progInfo;
  addPropLine(progInfo, _("CoolReader for PocketBook"),
              Utf8ToUnicode(lString8(CR_PB_VERSION)));
  addPropLine(progInfo, _("Build date"),
              Utf8ToUnicode(lString8(CR_PB_BUILD_DATE)));
  addPropLine(progInfo, _("Build type"), Utf8ToUnicode(lString8(CR_PB_SDK)));
  addInfoSection(txt, progInfo, _("About program"));

  lString8 debugInfo;
  addPropLine(debugInfo, _("Hardware type"),
              Utf8ToUnicode(lString8(GetHardwareType())));
  addPropLine(debugInfo, _("Firmware version"),
              Utf8ToUnicode(lString8(GetSoftwareVersion())));
  addPropLine(debugInfo, _("Keyboard type"),
              lString16::itoa(getPB_keyboardType()));
  addPropLine(debugInfo, _("Screen type"),
              lString16::itoa((getPB_screenType())));
...
bool V3DocViewWin::onCommand(int command, int params) {
  CRLog::info("V3DocViewWin::onCommand(%d [%s], %d)", command,
              getCommandName(command, params), params);
  switch (command) {
  case MCMD_QUIT:
  ...
  case mm_Skin:
    if (_wm->setSkin(_newProps->getStringDef(PROP_SKIN_FILE))) {
      ...
      _newProps->setString(PROP_SKIN_FILE,
                           _props->getStringDef(PROP_SKIN_FILE));
    }
    return true;
  ...
  default:
    ...
    if (command > PB_TOC_SAFE_CMD_RANGE) {
      CRLog::trace("V3DocViewWin::onCommand(): %d > %d", command,
                   PB_TOC_SAFE_CMD_RANGE);
...
void V3DocViewWin::OnLoadFileFirstPagesReady() {
  if (DISABLE_FIRST_PAGE_RENDER ||
      !_props->getBoolDef(PROP_PROGRESS_SHOW_FIRST_PAGE, 1)) {
    CRLog::info("OnLoadFileFirstPagesReady() - don't paint first page "
                "because " PROP_PROGRESS_SHOW_FIRST_PAGE " setting is 0");
    return;
  }
