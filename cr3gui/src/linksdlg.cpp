    ...
    for (int i = 0; i < list.length(); i++) {
      ...
      if (link && link->getRectangles(rects)) {
        for (int j = 0; j < rects.length(); j++) {
          ...
          if (_docview->docToWindowPoint(topLeft) &&
              _docview->docToWindowPoint(bottomRight)) {
            rc.left = topLeft.x;
            rc.top = topLeft.y;
            rc.right = bottomRight.x;
            rc.bottom = bottomRight.y;
            CRLog::trace("Link control at[%d,%d,%d,%d]", rc.left, rc.top,
                         rc.right, rc.bottom);
...
CRLinksDialog *CRLinksDialog::create(CRGUIWindowManager *wm,
                                     CRViewDialog *docwin, bool backPreffered) {
  ldomXRangeList list;
...
public:
  LinksToolbar(CRGUIWindowBase *window, lString16 id, CRToolBarSkinRef tbskin,
               bool active = true)
  ...
  void initDefault() {
    ...
    addButton(
        new CRToolButton(this, 1, cs16("->"), DCMD_LINK_FORWARD, 0)); // forward
...
CRLinksDialog::CRLinksDialog(CRGUIWindowManager *wm, CRViewDialog *docwin,
  ...
  if (!tb1Skin.isNull()) {
    _toolBar =
        new LinksToolbar(this, cs16("links-toolbar"), tb1Skin, backPreffered);
...
void CRLinksDialog::invalidateCurrentSelection() {
  ...
  if (!link) {
    CRLog::debug(
        "invalidateCurrentSelection() : no current page selection found!");
    return;
  }
  LVArray<lvRect> rects;
  if (link->getRectangles(rects)) {
    ...
    if (_docview->docToWindowPoint(topLeft) &&
      ...
      rc.right = bottomRight.x;
      rc.bottom = bottomRight.y;
      CRLog::debug("invalidating link screenRect { %d, %d, %d, %d }", rc.left,
