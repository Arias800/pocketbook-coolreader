...
CRBookmarkMenuItem::CRBookmarkMenuItem(CRMenu *menu, int shortcut,
                                       CRBookmark *bookmark, int page)
    : CRMenuItem(menu, shortcut, lString16(_("Empty slot")), LVImageSourceRef(),
                 LVFontRef()),
      _bookmark(bookmark), _page(page) {}

void CRBookmarkMenuItem::Draw(LVDrawBuf &buf, lvRect &rc, CRRectSkinRef skin,
  ...
  if (!text.empty()) {
    posRect.bottom = posRect.top + skin->getFont()->getHeight() +
                     itemBorders.top + itemBorders.bottom;
    textRect.top = posRect.bottom - itemBorders.bottom;
  }
  lString16 postext(_("Page $1 ($2%)"));
  postext.replaceIntParam(1, _page + 1);
  postext.replaceParam(2, lString16::itoa(_bookmark->getPercent() / 100)
                              << "."
                              << fmt::decimal(_bookmark->getPercent() % 100));
...
int CRBookmarkMenu::getSelectedItemIndex() {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    if (bm) {
      ldomXPointer p =
          _docview->getDocument()->createXPointer(bm->getStartPos());
      if (!p.isNull()) {
...
void CRBookmarkMenu::showContextMenu() {
  CRBookmarkMenuItem *item =
      static_cast<CRBookmarkMenuItem *>(_items[_selectedItem]);
  ...
  int y = clientRect.top +
          (itemSize.y + separatorHeight) * (_selectedItem - _topItem) +
  ...
  OpenMenu(_contextMenu, _goToMode ? DCMD_BOOKMARK_GO_N : DCMD_BOOKMARK_SAVE_N,
...
CRBookmarkMenu::CRBookmarkMenu(CRGUIWindowManager *wm, LVDocView *docview,
                               int numItems, lvRect &rc, bool goToMode)
    : CRFullScreenMenu(wm, MCMD_BOOKMARK_LIST, lString16(_("Bookmarks")),
                       numItems, rc),
      _docview(docview) {
  ...
  int minitems =
      (MIN_BOOKMARK_ITEMS + _pageItems - 1) / _pageItems * _pageItems;
  if (n < minitems)
    n = minitems;
  for (int i = 1; i <= n; i++) {
    CRBookmark *bm = bookmarks->getShortcutBookmark(i);
    ...
    if (bm) {
      ldomXPointer p =
          docview->getDocument()->createXPointer(bm->getStartPos());
      if (!p.isNull()) {
        /// get page number by bookmark
...
bool CRBookmarkMenu::onCommand(int command, int params) {
  if (command >= MCMD_SELECT_1 && command <= MCMD_SELECT_9) {
    ...
    if (_selectedItem >= 0)
      closeMenu(_goToMode ? DCMD_BOOKMARK_GO_N : DCMD_BOOKMARK_SAVE_N,
                _selectedItem + 1);
    return true;
...
#else
    if (_selectedItem >= 0)
      closeMenu(_goToMode ? DCMD_BOOKMARK_SAVE_N : DCMD_BOOKMARK_GO_N,
                _selectedItem + 1);
#endif
    return true;
...
#endif
  else if ((DCMD_BUTTON_PRESSED == command ||
            DCMD_BUTTON_PRESSED_LONG == command) &&
           BTN_CLOSE == params) {
    closeMenu(0);
    return true;
  }
...
CRCitesMenu::CRCitesMenu(CRGUIWindowManager *wm, LVDocView *docview,
                         int numItems, lvRect &rc)
    : CRFullScreenMenu(wm, MCMD_CITES_LIST, lString16(_("Citations")), numItems,
                       rc),
      _docview(docview) {
  CRGUIAcceleratorTableRef acc = _wm->getAccTables().get("bookmarks");
  if (acc.isNull())
  ...
  for (int i = 0; i < bookmarks.length(); i++) {
    ...
    if (!bmk ||
        ((bmk->getType() != bmkt_comment && bmk->getType() != bmkt_correction)))
      continue;
