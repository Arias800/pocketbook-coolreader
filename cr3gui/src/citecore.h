...
enum granularity { grn_char, grn_word, grn_block };

// const lString16 phrase_bounds(L".?!");

enum last_move_type { not_moved, moved_up, moved_down };

void point_to_end(ldomXPointerEx &xp) {
...
void point_to_begin(ldomXPointerEx &xp) {
  ldomNode *p = xp.getNode();
  if (p->isText())
    xp.setOffset(0);
}
...
public:
  CiteSelection(LVDocView &view) : last_move_(not_moved), view_(view) {
  ...
  ldomXRange select() const { return ldomXRange(start_, end_); }
