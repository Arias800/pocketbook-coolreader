...
time_t ReadingStats::getTime() { return (time_t)time(0); }
...
lString8 ReadingStats::getDateTimeString(time_t t) {
  ...
  tm *bt = localtime(&t);
  char str[12];
  sprintf(str, "%04d-%02d-%02d %02d:%02d:%02d", bt->tm_year + 1900,
...
void ReadingStats::init() {
  ...
  // _lang = ReadString(cfg, const_cast<char *>("language"), const_cast<char
  // *>("en")); currentLang = lString16(_lang.c_str()); _keepOrientation =
  // ReadInt(cfg, const_cast<char *>("keeporient"), 0); _pbDictionary =
  // ReadString(cfg, const_cast<char *>("dictionary"), const_cast<char *>(""));
  CloseConfig(cfg);
}
