    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!item->IsContainer()) {
        ...
        if (name.endsWith(indexExt)) {
          lString16 nameBase =
              name.substr(0, name.length() - indexExt.length());
          ...
          if (index >= 0) {
            // found pair
            dicts.add(UnicodeToUtf8(path + name).c_str(),
                      UnicodeToUtf8(path + dataName).c_str());
          }
        }
      }
    }
...
lString8 CRTinyDict::translate(const lString8 &w) {
  ...
  if (dicts.length() == 0) {
    ...
    for (int d = 0; d < results.length(); d++) {
      ...
      if (words->length() > 0)
        body << "<title><p>" << _("From dictionary ")
             << words->getDictionaryName() << ":</p></title>";
      // for each found word
      for (int i = 0; i < words->length(); i++) {
        // TinyDictWord * word = words->get(i);
        const char *article = words->getArticle(i);
        body << "<code style=\"text-align: left; text-indent: 0; font-size: "
                "22\">";
        if (article) {
          body << article;
        } else {
...
public:
  ...
  void highlight(WordWithRanges *p) {
    crtrace trace;
    trace << "Select word " << p->getWord() << " (" << p->getRanges().length()
          << " occurences) : ";
    for (int i = 0; i < p->getRanges().length(); i++) {
      trace << "[" << p->getRanges()[i].getStart() << ","
            << p->getRanges()[i].getEnd() << "] ";
    }
...
public:
  ...
  selector(LVDocView &docview, const TEncoding &encoding)
      : words_(docview, encoding), current_(0), level_(0), candidates_(),
        prefix_(), repeat_(0) {
...
public:
  ...
  CRT9Keyboard(CRGUIWindowManager *wm, CRDocViewWindow *mainwin, int id,
               lString16 &buffer);

  virtual bool onCommand(int command, int params);
};

const lChar16 *defT5encoding[] = {L"",       // 0 STUB
                                  L"abcde",  // 1
                                  L"fghij",  // 2
                                  L"klmno",  // 3
                                  L"pqrst",  // 4
...
void CRT9Keyboard::setDefaultLayout() { encoding_.init(defT5encoding); }
...
CRT9Keyboard::CRT9Keyboard(CRGUIWindowManager *wm, CRDocViewWindow *mainwin,
                           int id, lString16 &buffer)
    : BackgroundFitWindow(wm, mainwin), _command(id), _buf(buffer),
      encoding_(defT5encoding), selector_(*mainwin->getDocView(), encoding_) {
...
void showT9Keyboard(CRGUIWindowManager *wm, CRDocViewWindow *mainwin, int id,
                    lString16 &buffer) {
