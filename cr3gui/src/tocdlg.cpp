...
void CRTOCDialog::draw() {
  ...
  for (int i = 0; i < _pageItems && i + _topItem < _items.length(); i++) {
    ...
    LVTocItem *nextitem =
        (i + _topItem + 1) < _items.length() ? _items[i + _topItem + 1] : NULL;
    ...
    int pageNumWidth = valueSkin->getFont()->getTextWidth(pageString.c_str(),
                                                          pageString.length());
    int titleWidth = itemSkin->getFont()->getTextWidth(titleString.c_str(),
                                                       titleString.length());
    ...
    pageNumRect.left = pageNumRect.right - pageNumWidth -
                       valueSkin->getBorderWidths().left -
                       valueSkin->getBorderWidths().right;
    itemRect.right = pageNumRect.left;
    if (!itemRect.isEmpty()) {
      lvRect rc = itemRect;
      // rc.extendBy( borders );
      lString16 s =
          limitTextWidth(titleString, rc.width() - borders.left - borders.right,
                         itemSkin->getFont());
    ...
    if (itemRect.left + titleWidth < itemRect.right + 5) {
      itemRect.left =
          itemRect.left + titleWidth + itemSkin->getBorderWidths().left;
...
CRTOCDialog::CRTOCDialog(CRGUIWindowManager *wm, lString16 title, int resultCmd,
                         int pageCount, LVDocView *docview)
    : CRNumberEditDialog(wm, title, lString16::empty_str, resultCmd, 1,
                         pageCount),
      _docview(docview) {
...
int CRTOCDialog::getCurItemIndex() {
  ...
  for (int i = 0; i < _items.length(); i++) {
    ...
    LVTocItem *nextitem = (i + 1) < _items.length() ? _items[i + 1] : NULL;
    bool isSelected = true;
    if (curPage < item->getPage())
      isSelected = false;
    else if (nextitem != NULL && curPage >= nextitem->getPage() &&
