 ...
 *
 * If return value is 1, this means that key has been processed in plugin and
 * viewer should flush the screen. If return value is 2, this means that key has
 * been processed in plugin and no more processing is required. If return value
 * is 0, or no such function defined in plugin, default processing should be
 * done by Viewer.
 */
 ...
 *
 * actionId - id of menu action. Set of standard actions should be defined in
 * SDK header file. Some range should be reserved for plugin items. E.g. 1..999
 * for standard, Viewer-defined actions 1000-1999 reserved for plugins
 *
 * If return value is 1, this means that action has been processed in plugin and
 * viewer should flush the screen. If return value is 2, this means that action
 * has been processed in plugin and no more processing is required. If return
 * value is 0, or no such function defined in plugin, default processing should
 * be done by Viewer.
 */
 ...
 * Viewer menu item.
 *
 * viewer_menu_item_t * is usually a pointer to item array, actionId=0 for
 * end-of-array marker.
 *
 */
struct viewer_menu_item_t {
  int actionId; // item action code. 0 to mark end of array. For submenu, it can
                // be used to allow removing standard submenu item from root
                // level.
  const char
      *captionLabelId; // key string to find menu item label in language file,
                       // NULL to remove standard item with this actionId
  viewer_menu_item_t
      *submenu; // pointer to submenu items array, if any. Null for final item.
                // If submenu is specified, submenu should be shown on item
                // press, instead of calling OnMenuAction( int actionId )
...
{ MY_MENU_ACTION_CHANGE_FONT, "CHANGE_FONT_SIZE", NULL }, // new plugin defined
menu item. CHANGE_FONT_SIZE should be added to language files. {
MY_SUBMENU_INTERLINE_SPACING, "INTERLINE_SPACING", my_submenu_interline }, //
plugin's own custom submenu { 0, NULL, NULL } // end of array marker
};
 ...
 * This can be just a page number, file position or (for libfb2) XML pointer
 * (Xpointer). It's necessary to be able restoring the same position even if
 * number of pages has changed after zoom.
 */
...
/*
On plugin start, IsStandardStatusBarVisible() is being called to determine
whether standard or plugin's own status bar should be used.

If 0 is returned, OnStatusInfoChange() should be called on status change.
If plugin redrawed status, it can inform Viewer that some are should be redrawn
by returning 1, and setting updateRect to coordinates of are it wants to
repaint. In this case viewer should get page image pointer from plugin and then
draw specified rectangle on screen.
*/

/**
 * Return 0 to hide standard statusbar, 1 to show it. If no such function
 * defined in plugin, assume as 1.
 */
int IsStandardStatusBarVisible();

/******************************************MarkSearch.cpp,MarkSearch.h*********************************/
struct status_info_t {
  int bookmarkLabelFlags; // bit set, (1, 2, 4, 8, 16) for bookmark icons 1, 2,
                          // 3, 4, 5 correspondingly
                          /*
                           */
  int musicState;         // 0 no music, 1 music
  int batteryState; // e.g. 0..16 for current energy level, -1 for charging
                    // mode.
  int currentBookmarkFlags; // bit set, (1,2,4,8,16) for bookmarks on current
                            // page (1,2,3,4,5)
};
 ...
 * Plugin should return 1 and write rectangle coordinates to rectToUpdate if it
 * wants to update part of screen to show new status.
 */
...
struct CallbackFunction {
  ...
  void (*EndDialog)(); // exit the customize mode, viewer will flush the screen
                       // in it.
  void (*SetFontSize)(int fontSize);
  void (*SetFontAttr)(int fontAttr); // 0, black font, 1, white font
  void (*TextOut)(int x, int y, char *text, int length,
                  int flags); // flag can be TF_ASCII , TF_UTF8 or TF_UC16
                              // to ascii and utf8, length is the length of
                              // string. to UC16, length is the length of
  void (*BlitBitmap)(int x, int y, int w, int h, int src_x, int src_y,
                     int src_width, int src_height, unsigned char *buf);
...
unsigned short *szGetVoiceDataBlock(int iPage, int *numChars,
                                    int *encodingType);
