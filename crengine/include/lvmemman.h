...
/// set file to remove of fatal error - for removing of book cache file which
/// caused crash
void crSetFileToRemoveOnFatalError(const char *filename);

/// set signal handler to do some cleanup actions (e.g. delete current cache
/// file since it might be corrupted)
...
/// typed realloc with result check (size is counted in T), fatal error if
/// failed
...
#define MAX_LOCAL_BLOCK_SIZE                                                   \
  ((1 << BLOCK_SIZE_GRANULARITY) * LOCAL_STORAGE_COUNT)
...
struct ldomMemSlice {
  ...
  ldomMemSlice(size_t blockSize, size_t blockCount)
      : block_size(blockSize), block_count(blockCount), blocks_used(0) {
...
struct ldomMemManStorage {
  ...
  //======================================
  ldomMemManStorage(size_t blockSize) : block_size(blockSize) {
    slice_count = 1;
