...
*/
template <class T, bool ownItems = true> class LVPtrVector {
  T **_list;
  int _size;
  int _count;
...
public:
  ...
  /// copy constructor
  LVPtrVector(const LVPtrVector &v) : _list(NULL), _size(0), _count(0) {
    if (v._count > 0) {
      reserve(v._count);
  ...
  /// stack-like interface: push item to stack
  void pushHead(T *item) { insert(0, item); }
...
public:
  LVPair(const T1 &first, const T2 &second) : _first(first), _second(second) {}
  LVPair(const LVPair &v) : _first(v._first), _second(v._second) {}
