...
public:
  ...
  void operator delete(void *p) { pmsREF->free((ldomMemBlock *)p); }
#endif
};
...
public:
  ...
  /// Copy constructor.
  /** Creates copy of object pointer. Increments reference counter instead of
  real copy. \param ref is reference to copy
   */
...
public:
  ...
  /// Default constructor.
  /** Initializes pointer to NULL */
  LVRef() : _ptr(&ref_count_rec_t::null_ref) {
...
template <typename T> class LVRefVec {
  LVRef<T> *_array;
  int _size;
  int _count;
...
public:
  ...
  void add(LVRef<T> item) { insert(-1, item); }
