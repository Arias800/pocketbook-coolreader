...
#define CH_PROP_ALPHA                                                          \
  0x0003 ///< alpha flag is combination of uppercase and lowercase flags
...
public:
  lstring16_chunk_t(lChar16 *_buf16)
      : buf16(_buf16), size(1), len(0), nref(1) {}
...
public:
  ...
  lString8 &insert(size_type p0, const lString8 &str, size_type offset,
                   size_type count);
  ...
  int compare(const lString8 &str) const {
    return lStr_cmp(pchunk->buf8, str.pchunk->buf8);
  }
  ...
  int compare(size_type p0, size_type n0, const lString8 &str, size_type pos,
              size_type n) const;
  ...
  int compare(size_type p0, size_type n0, const value_type *s,
              size_type pos) const;
  ...
  /// find position of substring inside string starting from specified position,
  /// -1 if not found
...
public:
  ...
  lString16 &insert(size_type p0, const lString16 &str, size_type offset,
                    size_type count);
  ...
  lString16 &replace(size_type p0, size_type n0, size_type count,
                     value_type ch);
  ...
  lString16 &lowercase();
  /// compare with another string
  int compare(const lString16 &str) const {
    return lStr_cmp(pchunk->buf16, str.pchunk->buf16);
  }
  ...
  int compare(size_type p0, size_type n0, const lString16 &str, size_type pos,
              size_type n) const;
  ...
  /// find position of substring inside string (8bit ASCII only), -1 if not
  /// found
  int pos(const lChar8 *subStr) const;
  /// find position of substring inside string starting from specified position,
  /// -1 if not found
  int pos(const lChar16 *subStr, int start) const;
  /// find position of substring inside string (8bit ASCII only) starting from
  /// specified position, -1 if not found
  ...
  lUInt32 getHash() const;
  /// returns character at specified position, with index bounds checking, fatal
  /// error if fails
  value_type &at(size_type pos) {
  ...
  /// returns string length, in characters
  size_type size() const { return pchunk->len; }
  /// resizes string buffer, appends with specified character if buffer is being
  /// extended
  void resize(size_type count = 0, value_type e = 0);
  ...
  lString16 &trim();
  /// trims duplicate space characters inside string and (optionally) at end and
  /// beginning of string
  lString16 &trimDoubleSpaces(bool allowStartSpace, bool allowEndSpace,
                              bool removeEolHyphens = false);
  /// converts to integer
  int atoi() const;
...
/// -- fast and memory effective replacement of lString8("abc")
const lString8 &cs8(const char *str);
/// get reference to atomic constant wide string for string literal e.g.
/// cs16("abc") -- fast and memory effective replacement of lString16("abc")
const lString16 &cs16(const char *str);
/// get reference to atomic constant wide string for string literal e.g.
/// cs16(L"abc") -- fast and memory effective replacement of lString16(L"abc")
...
public:
  ...
  const lString16 &at(int index) { return ((lString16 *)chunks)[index]; }
  const lString16 &operator[](int index) const {
    return ((lString16 *)chunks)[index];
  }
  lString16 &operator[](int index) { return ((lString16 *)chunks)[index]; }
...
public:
  ...
  void erase(int offset, int count);
  const lString8 &at(int index) { return ((lString8 *)chunks)[index]; }
  const lString8 &operator[](int index) const {
    return ((lString8 *)chunks)[index];
  }
  lString8 &operator[](int index) { return ((lString8 *)chunks)[index]; }
  int length() const { return count; }
  void clear();
  ~lString8Collection() { clear(); }
};
...
inline lString16 operator+(const lString16 &s1, const lString16 &s2) {
  lString16 s(s1);
  s.append(s2);
  return s;
}
inline lString16 operator+(const lString16 &s1, const lChar16 *s2) {
  lString16 s(s1);
  s.append(s2);
  return s;
}
inline lString16 operator+(const lString16 &s1, const lChar8 *s2) {
  lString16 s(s1);
  s.append(s2);
  return s;
}
inline lString16 operator+(const lChar16 *s1, const lString16 &s2) {
  lString16 s(s1);
  s.append(s2);
  return s;
}
inline lString16 operator+(const lChar8 *s1, const lString16 &s2) {
  lString16 s(s1);
  s.append(s2);
  return s;
}
inline lString16 operator+(const lString16 &s1, fmt::decimal v) {
  lString16 s(s1);
  s.appendDecimal(v.get());
  return s;
}
inline lString16 operator+(const lString16 &s1, fmt::hex v) {
  lString16 s(s1);
  s.appendHex(v.get());
  return s;
}
...
public:
  lStringBuf16(lString16 &s) : str(s), pos(0) {}
...
public:
  lStringBuf8(lString8 &s) : str(s), pos(0) {}
...
/// converts 8-bit string to unicode string using specified conversion table for
...
/// truncates string by specified size, appends ... if truncated, prefers to
/// wrap whole words
...
public:
  ...
  /// checks whether specified number of bytes is available, returns true in
  /// case of error
...
public:
  /// log levels
  enum log_level { LL_FATAL, LL_ERROR, LL_WARN, LL_INFO, LL_DEBUG, LL_TRACE };
  ...
  /// write log to specified file, flush after every message if autoFlush
  /// parameter is true
...
public:
  FILE *f;
  DumpFile(const char *fname) : f(NULL) {
    if (fname)
      f = fopen(fname, "at");
    if (!f)
      f = stdout;
