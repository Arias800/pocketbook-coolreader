...
public:
  ...
  LVThread() : _valid(false), _stopped(false) {}
  virtual ~LVThread() {
    if (_valid) {
...
public:
  LVMutex() { _valid = (pthread_mutex_init(&_mutex, NULL) != 0); }
  ~LVMutex() {
    if (_valid)
...
private:
  ...
  static DWORD WINAPI start_routine(LPVOID param) {
    LVThread *thread = (LVThread *)param;
    thread->run();
    thread->_stopped = true;
    return 0;
...
public:
  virtual void start() { ResumeThread(_thread); }
  bool stopped() { return _stopped; }
  void join() {
    if (_valid) {
  ...
  LVThread() : _stopped(false) {
    _thread =
        CreateThread(NULL, 0, start_routine, this, CREATE_SUSPENDED, &_id);
    _valid = _thread != NULL;
  }
  virtual ~LVThread() {
    if (_valid) {
...
public:
  LVLock(LVMutex &mutex) : _mutex(mutex) { _locked = _mutex.lock(); }
  ~LVLock() {
    if (_locked)
      _mutex.unlock();
  }
