...
protected:
  ...
  int indexOf(int keyCode, int keyFlags) {
    ...
    for (int i = 0; i < _items.length(); i++) {
      // CRLog::trace( " compare( %d, %d )", (int)_items[i]->keyCode,
      // (int)_items[i]->keyFlags );
...
public:
  ...
  /// get item by index
  const CRGUIAccelerator *get(int index) const { return _items[index]; }
  ...
  /// constructor from int array: 4 ints per entry (keyCode, keyFlags,
  /// commandId, commandParam), keyCode==0 indicates end of list
 ...
 *   identifier defined in definition file, decimal number, hex number with 0x
 * prefix, or character in '' it's better to avoid constants in this file,
 * instead just place them to definition file and use by identifiers
 *
 * example:
 *
...
public:
  ...
  void add(const char *name, const int *defs) { add(lString16(name), defs); }
  /// add accelerator table definition from array
  ...
  CRGUIAcceleratorTableRef get(const lString16 &name) {
  ...
  CRGUIAcceleratorTableRef get(const char *name) {
    return _table.get(lString16(name));
  }
  /// returns true if there are no no tables in list
  bool empty() { return _table.length() == 0; }
  /// constructs empty list, then it should be filled from file using
  /// openFromFile()
...
public:
  // for turbo updates
  enum UpdateMode { NormalMode, PrepareMode };
  ...
  /// transfers contents of buffer to device, if full==true, redraws whole
  /// screen, otherwise only changed area
...
public:
  ...
  /// peeks head of application message queue, w/o removing from queue (returns
  /// NULL if no events in queue)
  virtual CRGUIEvent *peekEvent() {
    forwardSystemEvents(false);
    return _events.peekHead();
  }
  /// returns head of application message queue, removing from queue (returns
  /// NULL if no events in queue)
  ...
  /// override to handle
  virtual bool handleEvent(CRGUIEvent *event);
  /// called when message queue is empty and application is going to wait for
  /// event
  virtual void idle() {}
  ...
  /// sets screen orientation value, to be red by corresponding getter. Doesn't
  /// rotate screen actually.
  ...
  virtual bool processPostedEvents() { return handleAllEvents(false); }
  /// returns true if command is processed
  ...
  virtual CRGUIScreen *getScreen() { return _screen; }
  /// runs event loop
  virtual int runEventLoop();
  /// constructor
  CRGUIWindowManager(CRGUIScreen *screen)
      : _screen(screen), _ownScreen(false), _postedCommand(0),
        _postedCommandParam(0), _lastProgressUpdate(0),
        _lastProgressPercent(-1), _orientation(CR_ROTATE_ANGLE_0),
        _stopFlag(false), _ignoreTillUp(false) {}
  ...
  /// call when long tap processed and it is known touch up event will not
  /// arrive
...
protected:
  ...
  /// draw status text
  virtual void drawStatusText(LVDrawBuf &buf, const lvRect &rc,
                              CRRectSkinRef skin);
  ...
  virtual void drawScroll(CRScrollSkinRef sskin, lvRect scrollRc, bool vertical,
                          int pos, int maxpos, int pagesize);
...
public:
  ...
  /// sets skin name for window, if a skin doesn't exist set name to the
  /// baseSkin
  ...
  /// returns true if key is processed (by default, let's translate key to
  /// command using accelerator table)
  virtual bool onKeyPressed(int key, int flags = 0);
  virtual bool onTouch(int x, int y, CRGUITouchEventType evType);
  ...
  CRGUIWindowBase(CRGUIWindowManager *wm)
      : _wm(wm), _visible(true), _fullscreen(true), _dirty(true),
        _passKeysToParent(false), _passCommandsToParent(false), _page(0),
        _pages(0), _controlsCreated(false), _sortZorder(true),
        _selectedControl(0) {
...
public:
  ...
  virtual int getFullUpdateInterval() { return _fullUpdateInterval; }
  ...
  /// invalidates rectangle: add it to bounding box of next partial update
  virtual void invalidateRect(const lvRect &rc) { _updateRect.extend(rc); }
  CRGUIScreenBase(int width, int height, bool doublebuffer)
      : _width(width), _height(height), _canvas(NULL), _front(NULL),
        _fullUpdateInterval(1), _fullUpdateCounter(1) {
    if (width && height) {
...
public:
  ...
  CRDocViewWindow(CRGUIWindowManager *wm) : CRGUIWindowBase(wm) {
  ...
  /// returns true if window is changed but now drawn
  virtual bool isDirty() { return _dirty || !_docview->isPageImageReady(0); }
...
enum CRGUICmd { GCMD_PASS_TO_PARENT = 550 };
...
public:
  ...
  /// constructor
  CRMenuItem(CRMenu *menu, int id, lString16 label, LVImageSourceRef image,
             LVFontRef defFont, const lChar16 *propValue = NULL)
      : _menu(menu), _id(id), _label(label), _image(image), _defFont(defFont),
        _propValue(propValue) {}
  /// constructor
  CRMenuItem(CRMenu *menu, int id, const char *label, LVImageSourceRef image,
             LVFontRef defFont, const lChar16 *propValue = NULL)
      : _menu(menu), _id(id), _label(label), _image(image), _defFont(defFont),
        _propValue(propValue) {}
  CRMenuItem(CRMenu *menu, int id, lString16 label, const char *imageId,
             LVFontRef defFont, const lChar16 *propValue = NULL)
      : _menu(menu), _id(id), _label(label), _image(NULL), _defFont(defFont),
        _propValue(propValue), _imageId(imageId) {}
  CRMenuItem(CRMenu *menu, int id, const char *label, const char *imageId,
             LVFontRef defFont, const lChar16 *propValue = NULL)
      : _menu(menu), _id(id), _label(label), _image(NULL), _defFont(defFont),
        _propValue(propValue), _imageId(imageId) {}
  /// measures item size
...
public:
  ...
  CRMenu(CRGUIWindowManager *wm, CRMenu *parentMenu, int id, lString16 label,
         LVImageSourceRef image, LVFontRef defFont, LVFontRef valueFont,
         CRPropRef props = CRPropRef(), const char *propName = NULL,
         int pageItems = 8)
      : CRGUIWindowBase(wm), CRMenuItem(parentMenu, id, label, image, defFont),
        _props(props), _propName(Utf8ToUnicode(lString8(propName))),
        _valueFont(valueFont), _topItem(0), _pageItems(pageItems),
  ...
  CRMenu(CRGUIWindowManager *wm, CRMenu *parentMenu, int id, const char *label,
         const char *imageId, LVFontRef defFont, LVFontRef valueFont,
         CRPropRef props = CRPropRef(), const char *propName = NULL,
         int pageItems = 8)
      : CRGUIWindowBase(wm),
        CRMenuItem(parentMenu, id, label, imageId, defFont), _props(props),
        _propName(Utf8ToUnicode(lString8(propName))), _valueFont(valueFont),
        _topItem(0), _pageItems(pageItems), _cmdToHighlight(-1) {
    _fullscreen = false;
    _helpHeight = 0;
  }
  CRMenu(CRGUIWindowManager *wm, CRMenu *parentMenu, int id, lString16 label,
  ...
  LVImageSourceRef getImage(lString16 imageId) {
    return getImage(imageId.c_str());
  }
...
class CRToolButton {
public:
  CRToolButton(CRToolBar *toolbar, int id, lString16 label, int command,
               int param = 0, int paramLong = -1,
               int commandLong = DCMD_BUTTON_PRESSED_LONG)
      : m_toolbar(toolbar), m_id(id), m_command(command), m_param(param),
        m_commandLong(commandLong), m_paramLong(paramLong), m_label(label),
        m_enabled(true) {}
...
class CRToolBar {
public:
  CRToolBar(CRGUIWindowBase *window, lString16 id, CRToolBarSkinRef tbskin,
            bool active = true);
  virtual ~CRToolBar() {}
  ...
  void draw(LVDrawBuf &buf, const lvRect &rc) { draw(_tbSkin, buf, rc); }
  void draw() {
    lvRect rc;
  ...
  void controlRemoved() { _controlsCreated = false; }
...
public:
  CRMenuItemControl(CRMenu *parent, lvRect rc, int id)
      : CRGUIControl(parent, rc, 2), _id(id) {}
...
public:
  CRCommandControl(CRGUIWindowBase *parent, lvRect rc, int cmd, int param)
      : CRGUIControl(parent, rc, 1), _command(cmd), _param(param) {}
  virtual bool onTouch(CRGUITouchEventType evType) {
...
enum CRGuiButtons { BTN_CLOSE = -1, BTN_SCROLL_DOWN = -2, BTN_SCROLL_UP = -3 };
...
public:
  CRButtonControl(CRGUIWindowBase *parent, lvRect rc, CRButtonSkinRef skin,
                  int id, int command, int param = -1, int paramLong = -1,
                  int commandLong = DCMD_BUTTON_PRESSED_LONG)
      : CRGUIControl(parent, rc, 2), _btnSkin(skin), _selected(false),
        _command(command), _param(param), _commandLong(commandLong),
        _paramLong(paramLong), _btnId(id) {}
  virtual bool onTouch(CRGUITouchEventType evType) {
    switch (evType) {
    case CRTOUCH_UP:
    ...
    case CRTOUCH_DOWN_LONG:
      if (_commandLong == DCMD_BUTTON_PRESSED_LONG && _paramLong == -1)
        _parent->getWindowManager()->postCommand(DCMD_BUTTON_PRESSED_LONG,
                                                 _btnId);
      else
  ...
  virtual bool setSelected(bool selected) {
    if (selected != _selected) {
      ...
      int flags = selected ? CRButtonSkin::ENABLED | CRButtonSkin::PRESSED
                           : CRButtonSkin::ENABLED;
...
public:
  CRScrollButtonControl(CRGUIWindowBase *parent, lvRect rc,
                        CRButtonSkinRef skin, bool down)
      : CRButtonControl(parent, rc, skin,
                        down ? BTN_SCROLL_DOWN : BTN_SCROLL_UP,
                        DCMD_BUTTON_PRESSED),
        _down(down) {}
...
public:
  CRToolBarControl(CRGUIWindowBase *parent, CRToolBar *toolbar, lvRect rc)
      : CRGUIControl(parent, rc, 3), m_toolbar(toolbar) {}
  void addButton(CRButtonControl *button);
  virtual bool hitTest(lvPoint &pt);
  virtual bool onTouch(CRGUITouchEventType evType);
  virtual bool setSelected(bool selected);
  virtual ~CRToolBarControl() { m_toolbar->controlRemoved(); }
...
public:
  CRClientControl(CRGUIWindowBase *parent, lvRect rc)
      : CRGUIControl(parent, rc) {}
  virtual bool hitTest(lvPoint &pt) {
    _touchPt = pt;
...
public:
  CRGUIUpdateEvent(bool fullScreen = false) : CRGUIEvent(CREV_UPDATE) {
...
public:
  ...
  CRGUIKeyDownEvent(int key, int params) : CRGUIEvent(CREV_KEYDOWN) {
    _param1 = key;
    _param2 = params;
  }
...
public:
  CRGUICommandEvent(int cmd, int params) : CRGUIEvent(CREV_COMMAND) {
    _param1 = cmd;
    _param2 = params;
  }
  virtual bool handle(CRGUIWindow *window);
