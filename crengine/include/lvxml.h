...
public:
  ...
  virtual void OnTagOpenAndClose(const lChar16 *nsname,
...
/// converts XML text in-place: decode character entities, convert space chars,
...
protected:
  ...
  /// reads specified number of bytes, converts to characters and saves to
  /// buffer, returns number of chars read
...
public:
  /// returns true if end of fle is reached, and there is no data left in buffer
  virtual bool Eof() {
    return m_eof; /* m_buf_fpos + m_buf_pos >= m_stream_size;*/
  }
  virtual void Reset();
  ...
  // lString16 ReadLine( int maxLineSize, lvpos_t & fpos, lvsize_t & fsize,
  // lUInt32 & flags );
...
private:
  struct cache_item {
    ...
    lUInt32 flags;
    lString16 text;
    cache_item(lString16 &txt) : next(NULL), text(txt) {}
  };
...
public:
  ...
  virtual bool Parse();
  /// constructor
  LVXMLTextCache(LVStreamRef stream, lUInt32 max_itemcount,
                 lUInt32 max_charcount)
      : LVTextFileBase(stream), m_head(NULL), m_max_itemcount(max_itemcount),
...
public:
  ...
  LVXMLParser(LVStreamRef stream, LVXMLParserCallback *callback,
              bool allowHtml = true, bool fb2Only = false);
  /// changes space mode
