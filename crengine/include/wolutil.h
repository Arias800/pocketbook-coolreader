...
class WOLWriter : public WOLBase {
  ...
  public:
    ...
    bool isPrevSibling(TocItemInfo &v) {
      if (l1index == v.l1index) {
        if (l2index != 0 && l2index == v.l2index) {
          ...
          return (l2index != 0 && l2index == v.l2index + 1 && l3index == 0 &&
                  v.l3index == 0);
        }
      } else {
    ...
    TocItemInfo(int idx, int l1, int l2, int l3, int p, lString8 n)
        : index(idx), l1index(l1), l2index(l2), l3index(l3), page(p), name(n),
          catindex(0), parent(NULL), firstChild(NULL), nextSibling(NULL),
          prevSibling(NULL) {}
  };
...
public:
  ...
  ~WOLWriter();
  void addTocItem(int level1index, int level2index, int level3index,
                  int pageNumber, lString8 title);
  void addTitle(const lString8 &title, const lString8 &subject,
                const lString8 &author, const lString8 &adapter,
                const lString8 &translator, const lString8 &publisher,
                const lString8 &time_publish, const lString8 &introduction,
                const lString8 &isbn);
