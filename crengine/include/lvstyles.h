...
typedef struct css_style_rec_tag {
  ...
  css_style_rec_tag()
      : refCount(0), hash(0), display(css_d_inherit),
        white_space(css_ws_inherit), text_align(css_ta_inherit),
        text_align_last(css_ta_inherit), text_decoration(css_td_inherit),
        vertical_align(css_va_inherit), font_family(css_ff_inherit),
        font_size(css_val_inherited, 0), font_style(css_fs_inherit),
        font_weight(css_fw_inherit), text_indent(css_val_inherited, 0),
        line_height(css_val_inherited, 0), width(css_val_unspecified, 0),
        height(css_val_unspecified, 0), color(css_val_inherited, 0),
        background_color(css_val_unspecified, 0),
        letter_spacing(css_val_unspecified, 0), page_break_before(css_pb_auto),
        page_break_after(css_pb_auto), page_break_inside(css_pb_auto),
        hyphenate(css_hyph_inherit), list_style_type(css_lst_inherit),
...
enum lvdom_element_render_method {
  ...
  erm_final,  ///< final element: render the whole it's content as single render
              ///< block
  erm_inline, ///< inline element
  erm_mixed,  ///< block and inline elements are mixed: autobox inline portions
              ///< of nodes; TODO
...
public:
  ...
  {}
  ~lvdomElementFormatRec() {}
  void clear() { _x = _width = _y = _height = 0; }
  bool operator==(lvdomElementFormatRec &v) {
    return (_height == v._height && _y == v._y && _width == v._width &&
            _x == v._x);
  }
  bool operator!=(lvdomElementFormatRec &v) {
    return (_height != v._height || _y != v._y || _width != v._width ||
