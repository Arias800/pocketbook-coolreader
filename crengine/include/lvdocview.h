...
public:
  ...
  /// set page to cache
  void set(int offset, int page, LVRef<LVDrawBuf> drawbuf,
           LVRef<LVThread> thread) {
    LVLock lock(_mutex);
    _last = (_last + 1) & 1;
  ...
  LVDocImageRef get(int offset, int page) {
    ...
    if (!buf.isNull())
      return LVDocImageRef(
          new LVDocImageHolder(getWithoutLock(offset, page), _mutex));
  ...
  LVDocViewImageCache() : _last(0) {
    for (int i = 0; i < 2; i++)
      _items[i]._valid = false;
  }
  ~LVDocViewImageCache() { clear(); }
};
#endif
...
enum LVDocViewMode { DVM_SCROLL, DVM_PAGES };
...
private:
  ...
  // current position
  int _pos; // >=0 is correct vertical offset inside document, <0 - get based on
            // m_page
...
protected:
  virtual void drawNavigationBar(LVDrawBuf *drawbuf, int pageIndex,
                                 int percent);
...
public:
  ...
  /// draw to specified buffer by either Y pos or page number (unused param
  /// should be -1)
  void Draw(LVDrawBuf &drawbuf, int pageTopPosition, int pageNumber,
            bool rotate, bool autoresize = true);
  ...
  /// get screen rectangle for current cursor position, returns false if not
  /// visible
  ...
  /// try swappping of document to cache, if size is big enough, and no swapping
  /// attempt yet done
  void swapToCache();
  ...
  /// saves new bookmark
  CRBookmark *saveRangeBookmark(ldomXRange &range, bmk_type type,
                                lString16 comment);
  ...
  /// selects first link on page, if any. returns selected link range, null if
  /// no links.
  ...
  /// selects previous link on page, if any. returns selected link range, null
  /// if no links.
  ...
  bool IsDrawed();
  /// cache page image (render in background if necessary) (0=current, -1=prev,
  /// 1=next)
  ...
  /// rotate point by current angle, winToDoc==false for doc->window
  /// translation, true==ccw
  ...
  void toggleViewMode();
  /// returns current pages visible setting value (independent on window and
  /// font size)
  ...
  /// set window visible page count, to use exact value independent of font size
  /// and window sides
  ...
  virtual void drawPageHeader(LVDrawBuf *drawBuf, const lvRect &headerRc,
                              int pageIndex, int headerInfoFlags,
                              int pageCount);
  ...
  lUInt32 getBackgroundColor() { return m_backgroundColor; }
  ...
  /// returns text color
  lUInt32 getTextColor() { return m_textColor; }
  /// sets text color
  ...
  /// returns text color
  lUInt32 getStatusColor() { return m_statusColor; }
  /// sets text color
  ...
  /// draws scaled image into buffer, clear background according to current
  /// settings
  bool drawImage(LVDrawBuf *buf, LVImageSourceRef img, int x, int y, int dx,
                 int dy);
  /// converts point from window to document coordinates, returns true if
  /// success
  bool windowToDocPoint(lvPoint &pt);
  /// converts point from documsnt to window coordinates, returns true if
  /// success
  ...
  void drawPageTo(LVDrawBuf *drawBuf, LVRendPageInfo &page, lvRect *pageRect,
                  int pageCount, int basePage);
  ...
  bool getBookmarkPosText(ldomXPointer bm, lString16 &titleText,
                          lString16 &posText);
  ...
  /// change font size, if rollCyclic is true, largest font is followed by
...
#ifdef ORIGINAL_FILENAME_PATCH
  void setOriginalFilename(const lString16 &fn) { m_originalFilename = fn; }
  const lString16 &getOriginalFilename() { return m_originalFilename; }
...
public:
  ...
  SimpleTitleFormatter(lString16 text, lString8 fontFace, bool bold,
