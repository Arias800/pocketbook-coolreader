...
public:
  ...
  /// execute task delayed; already scheduled but not executed task will be
  /// deleted; pass NULL task to cancel active tasks
  virtual void executeGui(CRRunnable *task, int delayMillis) = 0;
  /// sleep current thread
  virtual void sleepMs(int durationMs) = 0;
  virtual void setThreadPriority(int p) { CR_UNUSED(p); }
};
