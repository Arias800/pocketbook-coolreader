...
*/
#pragma pack(push, 1)
typedef struct tag_lvfont_header {
  ...
  lUInt32 rangesOffset[1024]; /**< \brief byte offset from beginning of file to
                                 64 chars unicode ranges
...
*/
#pragma pack(push, 1)
typedef struct tag_lvfont_range {
  lUInt16
      glyphsOffset[64]; /** \brief offset table for 64 glyphs in range
                             - 0 if no glyph for char
                             - byte offset from beginning of range to glyph if
                           exists
...
*/
#pragma pack(push, 1)
typedef struct tag_lvfont_glyph {
...
/** \brief retrieves pointer to glyph structure for specified char by font
   handle \param hfont is font handle \param code is unicode character \return
   pointer to glyph structure
*/
...
/** \brief measures test line width
    ...
    \param max_width is a width limit -- measuring stops when total width
   exceeds this value \param widths returns running total widths of string
   [0..i] including text[i] \return number of items written to widths[]
*/
lUInt16 lvfontMeasureText(const lvfont_handle pfont, const lChar16 *text,
                          int len, lUInt16 *widths, lUInt8 *flags,
                          int max_width, lChar16 def_char);

#define LCHAR_IS_SPACE 1 ///< flag: this char is one of unicode space chars
#define LCHAR_ALLOW_WRAP_AFTER                                                 \
  2 ///< flag: line break after this char is allowed
#define LCHAR_DEPRECATED_WRAP_AFTER                                            \
  4 ///< flag: line break after this char is possible but deprecated
#define LCHAR_ALLOW_HYPH_WRAP_AFTER                                            \
  8 ///< flag: line break after this char is allowed with addition of hyphen
...
*/
void lvfontUnpackGlyph(const lUInt8 *packed, const hrle_decode_info_t *table,
                       lUInt8 *unpacked, int unp_size);
