...
public:
  ...
  */
  ...
  virtual bool Read(lUInt8 *buf) {
    lvsize_t nBytesRead;
    if (Read(buf, sizeof(lUInt8), &nBytesRead) == LVERR_OK &&
        nBytesRead == sizeof(lUInt8))
      return true;
    return false;
  }
  ...
  virtual bool Read(lUInt16 *buf) {
    lvsize_t nBytesRead;
    if (Read(buf, sizeof(lUInt16), &nBytesRead) == LVERR_OK &&
        nBytesRead == sizeof(lUInt16))
      return true;
    return false;
  }
  ...
  virtual bool Read(lUInt32 *buf) {
    lvsize_t nBytesRead;
    if (Read(buf, sizeof(lUInt32), &nBytesRead) == LVERR_OK &&
        nBytesRead == sizeof(lUInt32))
      return true;
    return false;
  }
...
public:
  ...
  */
  ...
  virtual lvsize_t GetSize() { return _base->GetSize(); }

  virtual lverror_t GetSize(lvsize_t *pSize) { return _base->GetSize(pSize); }
  ...
  */
  ...
  virtual bool Eof() { return _base->Eof(); }
};
...
public:
  LVNamedStream()
      : _crc(0), _crcFailed(false), _autosyncLimit(0), _bytesWritten(0) {}
  /// set write bytes limit to call flush(true) automatically after writing of
  /// each sz bytes
...
public:
  virtual const lChar16 *GetName() { return m_base_stream->GetName(); }
  virtual lvopen_mode_t GetMode() { return m_base_stream->GetMode(); }
  virtual lverror_t Seek(lvoffset_t offset, lvseek_origin_t origin,
                         lvpos_t *pNewPos) {
    return m_base_stream->Seek(offset, origin, pNewPos);
  }
  virtual lverror_t Tell(lvpos_t *pPos) { return m_base_stream->Tell(pPos); }
  // virtual lverror_t   SetPos(lvpos_t p)
  virtual lvpos_t SetPos(lvpos_t p) { return m_base_stream->SetPos(p); }
  virtual lvpos_t GetPos() { return m_base_stream->GetPos(); }
  ...
  virtual bool Eof() { return m_base_stream->Eof(); }
...
public:
  ...
  void SetName(const lChar16 *name) { m_name = name; }
  void SetItemInfo(lString16 fname, lvsize_t size, lUInt32 flags,
  ...
  LVCommonContainerItemInfo()
      : m_size(0), m_flags(0), m_is_container(false), m_srcpos(0), m_srcsize(0),
        m_srcflags(0) {}
...
public:
  ...
  void Add(LVCommonContainerItemInfo *item) { m_list.add(item); }
...
public:
  ...
  virtual lverror_t Seek(lvoffset_t pos, lvseek_origin_t origin,
                         lvpos_t *newPos) {
...
/**
    \param buf is pointer to buffer, if NULL, empty read/write memory stream
   will be created \param bufSize is buffer size, in bytes \param createCopy if
   true, read/write copy of specified data is being created, otherwise
   non-managed readonly buffer is being used as is \param mode is open mode
    \return reference to opened stream if success, NULL reference if error
*/
LVStreamRef LVCreateMemoryStream(void *buf = NULL, int bufSize = 0,
...
/// creates cache buffers for stream, to write data by big blocks to optimize
/// Flash drives writing performance
...
LVContainerRef LVOpenDirectory(const lString16 &path,
                               const wchar_t *mask = L"*.*");
LVContainerRef LVOpenDirectory(const lString8 &path,
                               const wchar_t *mask = L"*.*");
...
/// factory to handle filesystem access for paths started with ASSET_PATH_PREFIX
/// (@ sign)
