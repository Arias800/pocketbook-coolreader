...
template <class ref_t> class LVRefCache {

  class LVRefCacheRec {
    ref_t style;
    lUInt32 hash;
    LVRefCacheRec *next;
    LVRefCacheRec(ref_t &s, lUInt32 h) : style(s), hash(h), next(NULL) {}
...
template <class ref_t> class LVIndexedRefCache {
  ...
  int indexItem(LVRefCacheRec *rec) {
    ...
    if (n >= indexsize) {
      ...
      if (indexsize == 0)
        ...
        indexsize *= 2;
      index = (LVRefCacheIndexRec *)realloc(index, sizeof(LVRefCacheIndexRec) *
                                                       indexsize);
...
public:
  ...
  LVIndexedRefCache(LVArray<ref_t> &list)
      : index(NULL), indexsize(0), nextindex(0), freeindex(0), numitems(0) {
    setIndex(list);
  }
...
public:
  int length() { return numitems; }
  LVCacheMap(int maxSize) : size(maxSize), numitems(0), lastAccess(1) {
    buf = new Pair[size];
    clear();
  }
  void clear() {
