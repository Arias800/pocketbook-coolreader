...
enum page_type_t { PAGE_TYPE_NORMAL = 0, PAGE_TYPE_COVER = 1 };
...
public:
  int start;
  int height;
  LVPageFootNoteInfo() : start(0), height(0) {}
  LVPageFootNoteInfo(int s, int h) : start(s), height(h) {}
};
...
template <typename T, int RESIZE_MULT, int RESIZE_ADD> class CompactArray {
  struct Array {
    ...
    int _size;
    int _length;
    Array() : _list(NULL), _size(0), _length(0) {}
    ~Array() { clear(); }
    void add(T item) {
      if (_size <= _length) {
    ...
    T get(int index) const { return _list[index]; }
...
public:
  ...
  int length() const { return _data ? _data->length() : 0; }
  T get(int index) const { return _data->get(index); }
  const T &operator[](int index) const { return _data->operator[](index); }
  T &operator[](int index) { return _data->operator[](index); }
...
public:
  ...
  CompactArray<LVPageFootNoteInfo, 1, 4>
      footnotes; /// footnote fragment list for page
  ...
  LVRendPageInfo() : start(0), index(0), height(0), type(PAGE_TYPE_NORMAL) {}
...
public:
  ...
  */
  bool empty() const { return start == -1; }
  ...
  LVRendLineInfo(int line_start, int line_end, lUInt16 line_flags)
      : links(NULL), start(line_start),
        height((lUInt16)(line_end - line_start)), flags(line_flags) {}
  LVFootNoteList *getLinks() { return links; }
  ~LVRendLineInfo() { clear(); }
...
public:
  LVFootNote(lString16 noteId) : id(noteId) {}
  void addLine(LVRendLineInfo *line) { lines.add(line); }
