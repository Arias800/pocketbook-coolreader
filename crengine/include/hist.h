...
// These include need to be in this order.
// clang-format off
#include "lvptrvec.h"
#include <time.h>
// clang-format on

enum bmk_type { bmkt_lastpos, bmkt_pos, bmkt_comment, bmkt_correction };
...
public:
  ...
  CRBookmark(lString16 startPos, lString16 endPos)
      : _startpos(startPos), _endpos(endPos), _percent(0), _type(0),
        _shortcut(0), _postext(lString16::empty_str),
        _titletext(lString16::empty_str), _commenttext(lString16::empty_str),
        _timestamp(time_t(0)), _page(0) {}
  CRBookmark(const CRBookmark &v)
      : _startpos(v._startpos), _endpos(v._endpos), _percent(v._percent),
        _type(v._type), _shortcut(v._shortcut), _postext(v._postext),
        _titletext(v._titletext), _commenttext(v._commenttext),
        _timestamp(v._timestamp), _page(v._page) {}
...
public:
  ...
  static bool findNextRecordBounds(lChar8 *buf, int start, int end,
...
public:
  ...
  CRFileHistRecord() : _size(0) {}
  CRFileHistRecord(const CRFileHistRecord &v)
      : _fname(v._fname), _fpath(v._fpath), _title(v._title),
        _author(v._author), _series(v._series), _size(v._size),
        _bookmarks(v._bookmarks), _lastpos(v._lastpos) {}
  ~CRFileHistRecord() {}
};
...
public:
  ...
  CRFileHistRecord *savePosition(lString16 fpathname, size_t sz,
                                 ...
                                 const lString16 &series, ldomXPointer ptr);
  ldomXPointer restorePosition(ldomDocument *doc, lString16 fpathname,
                               size_t sz);
  CRFileHist() {}
  ~CRFileHist() { clear(); }
  void clear();
