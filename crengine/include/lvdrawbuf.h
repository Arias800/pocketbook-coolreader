...
public:
  ...
  /// returns tile drawbuf for tiled image, returns this for non tiled draw
  /// buffer
  ...
  virtual int getXtiles() { return 1; }
  /// returns number of tiles in column
  virtual int getYtiles() { return 1; }
  ...
  /// get average pixel value for area (coordinates are fixed floating points
  /// *16)
  virtual lUInt32 GetAvgColor(lvRect &rc16) = 0;
  /// get linearly interpolated pixel value (coordinates are fixed floating
  /// points *16)
  virtual lUInt32 GetInterpolatedColor(int x16, int y16) = 0;
  /// draw gradient filled rectangle with colors for top-left, top-right,
  /// bottom-right, bottom-left
  virtual void GradientRect(int x0, int y0, int x1, int y1, lUInt32 color1,
                            lUInt32 color2, lUInt32 color3, lUInt32 color4) {
  ...
  inline void DrawFrame(const lvRect &rc, lUInt32 color, int width = 1) {
    ...
    FillRect(rc.left, rc.top + width, rc.left + width, rc.bottom - width,
             color);
    FillRect(rc.right - width, rc.top + width, rc.right, rc.bottom - width,
             color);
  }
  /// fills rectangle with specified color
  ...
  inline void Rect(int x0, int y0, int x1, int y1, int borderWidth,
                   lUInt32 color) {
  ...
  /// draws rounded rectangle with specified line width, rounding radius, and
  /// color
  void RoundRect(int x0, int y0, int x1, int y1, int borderWidth, int radius,
                 lUInt32 color, int cornerFlags = 0x0F);
  /// draws rectangle with specified color
  ...
  virtual void FillRectPattern(int x0, int y0, int x1, int y1, lUInt32 color0,
                               lUInt32 color1, lUInt8 *pattern) = 0;
  ...
  virtual void Draw(int x, int y, const lUInt8 *bitmap, int width, int height,
                    lUInt32 *palette) = 0;
  /// draws image
  ...
  virtual void Draw(LVImageSourceRef img, int x, int y, int width, int height,
                    ...
                    bool dither = true) {
    CR_UNUSED10(img, x, y, width, height, srcx, srcy, srcwidth, srcheight,
                dither);
  }
  /// for GL buf only - rotated drawing
  virtual void DrawRotated(LVImageSourceRef img, int x, int y, int width,
                           int height, int rotationAngle) {
    Draw(img, x, y, width, height);
    CR_UNUSED(rotationAngle);
  }
  /// draws buffer content to another buffer doing color conversion if necessary
  virtual void DrawTo(LVDrawBuf *buf, int x, int y, int options,
                      lUInt32 *palette) = 0;
  /// draws rescaled buffer content to another buffer doing color conversion if
  /// necessary
  virtual void DrawRescaled(LVDrawBuf *src, int x, int y, int dx, int dy,
                            int options) = 0;
  /// draws rescaled buffer content to another buffer doing color conversion if
  /// necessary
  virtual void DrawFragment(LVDrawBuf *src, int srcx, int srcy, int srcdx,
                            int srcdy, int x, int y, int dx, int dy,
                            int options) {
  ...
  */
  ...
  /*
      ...
      virtual void DrawFormattedText( formatted_text_fragment_t * text, int x,
     int y ) = 0;
  */
...
public:
  ...
  */
  ...
  LVBaseDrawBuf()
      : _dx(0), _dy(0), _rowsize(0), _data(NULL), _hidePartialGlyphs(true) {}
...
public:
  ...
  LVDrawStateSaver(LVDrawBuf &buf)
      : _buf(buf), _textColor(buf.GetTextColor()),
        _backgroundColor(buf.GetBackgroundColor()), _alpha(buf.getAlpha()) {
...
inline lUInt16 rgb888to565(lUInt32 cl) {
  return (lUInt16)(((cl >> 8) & 0xF800) | ((cl >> 5) & 0x07E0) |
                   ((cl >> 3) & 0x001F));
}
