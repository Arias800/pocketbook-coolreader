...
/// auto pointer
template <class T> class LVAutoPtr {
  T *p;
  LVAutoPtr(const LVAutoPtr &v) { CR_UNUSED(v); } // no copy allowed
...
public:
  LVAutoPtr() : p(NULL) {}
  explicit LVAutoPtr(T *ptr) : p(ptr) {}
...
/// unique pointer
template <class T> class LVUniquePtr {
  T *p;

public:
  LVUniquePtr() : p(NULL) {}
  explicit LVUniquePtr(T *ptr) : p(ptr) {}
...
/// unique pointer
template <class T> class LVClonePtr {
  T *p;

public:
  LVClonePtr() : p(NULL) {}
  explicit LVClonePtr(T *ptr) : p(ptr ? (T *)ptr->clone() : NULL) {}
