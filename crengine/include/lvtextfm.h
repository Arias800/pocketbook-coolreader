...
#define LTEXT_LAST_LINE_ALIGN_LEFT                                             \
  0x00010000 /**< \brief last line of justified paragraph should be            \
                left-aligned */
#define LTEXT_LAST_LINE_ALIGN_RIGHT                                            \
  0x00020000 /**< \brief last line of justified paragraph should be            \
                right-aligned */
#define LTEXT_LAST_LINE_ALIGN_CENTER                                           \
  0x00030000 /**< \brief last line of justified paragraph should be centered   \
              */
#define LTEXT_LAST_LINE_ALIGN_WIDTH                                            \
  0x00040000 /**< \brief last line of justified paragraph should be justified  \
              */

#define LTEXT_FLAG_NEWLINE 0x0007 /**< \brief new line flags mask */
#define LTEXT_FLAG_OWNTEXT                                                     \
  0x0008 /**< \brief store local copy of text instead of pointer */
...
#define LTEXT_FLAG_PREFORMATTED                                                \
  0x0080 /**< \brief element space mode is preformatted */
...
struct text_highlight_options_t {
  ...
  int bookmarkHighlightMode; /**< bookmark highlight mode: 0=no highlight,
                                1=solid fill, 2=underline */
...
typedef struct {
  ...
  lInt32 img_zoom_in_mode_block;  /**< can zoom in block images: 0=disabled,
                                     1=integer scale, 2=free scale */
  lInt32 img_zoom_in_scale_block; /**< max scale for block images zoom in: 1, 2,
                                     3 */
  lInt32 img_zoom_in_mode_inline; /**< can zoom in inline images: 0=disabled,
                                     1=integer scale, 2=free scale */
  lInt32 img_zoom_in_scale_inline;  /**< max scale for inline images zoom in: 1,
                                       2, 3 */
  lInt32 img_zoom_out_mode_block;   /**< can zoom out block images: 0=disabled,
                                       1=integer scale, 2=free scale */
  lInt32 img_zoom_out_scale_block;  /**< max scale for block images zoom out: 1,
                                       2, 3 */
  lInt32 img_zoom_out_mode_inline;  /**< can zoom out inline images: 0=disabled,
                                       1=integer scale, 2=free scale */
  lInt32 img_zoom_out_scale_inline; /**< max scale for inline images zoom out:
                                       1, 2, 3 */
  lInt32 min_space_condensing_percent; /**< min size of space (relative to
                                          normal size) to allow fitting line by
                                          reducing of spaces */
  lInt32 max_space_expanding_percent; /**< max size of space (relative to normal
                                         size) to allow fitting line by adding
                                         word space */
  text_highlight_options_t
      highlight_options; /**< options for selection/bookmark highlighting */
...
public:
  ...
  void AddSourceLine(
      ...
      void *object = NULL, lUInt32 offset = 0, lInt8 letter_spacing = 0) {
    lvtextAddSourceLine(m_pbuffer,
                        font, // font->GetHandle()
                        text, len, color, bgcolor, flags, interval, margin,
                        object, (lUInt16)offset, letter_spacing);
  }

  lUInt32 Format(lUInt16 width, lUInt16 page_height);

  int GetSrcCount() { return m_pbuffer->srctextlen; }

  int GetWidth() { return m_pbuffer->width; }

  const src_text_fragment_t *GetSrcInfo(int index) {
    return &m_pbuffer->srctext[index];
  }

  int GetLineCount() { return m_pbuffer->frmlinecount; }
