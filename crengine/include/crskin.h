...
inline int toSkinPercent(int x) { return ((x) | SKIN_COORD_PERCENT_FLAG); }
...
public:
  ...
  virtual void setWordWrap(bool v) {
    _textAlign =
        v ? (_textAlign | SKIN_WORD_WRAP) : (_textAlign & ~SKIN_WORD_WRAP);
  }
  ...
  virtual void drawText(LVDrawBuf &buf, const lvRect &rc, lString16 text,
                        LVFontRef font) {
    drawText(buf, rc, text, font, getTextColor(), getBackgroundColor(),
             getTextAlign());
  }
  virtual void drawText(LVDrawBuf &buf, const lvRect &rc, lString16 text) {
    drawText(buf, rc, text, LVFontRef(), getTextColor(), getBackgroundColor(),
             getTextAlign());
  }
  virtual void drawText(LVDrawBuf &buf, const lvRect &rc, lString16 text,
                        lUInt32 color) {
    drawText(buf, rc, text, LVFontRef(), color, getBackgroundColor(),
             getTextAlign());
  }
...
public:
  enum { ENABLED = 1, PRESSED = 2, SELECTED = 4 };
  ...
  virtual void drawButton(LVDrawBuf &buf, const lvRect &rc,
                          int flags = ENABLED);
  LVImageSourceRef getImage(int flags = ENABLED);
  CRButtonSkin();
  virtual ~CRButtonSkin() { CRLog::trace("~CRButtonSkin()"); }
};
typedef LVRef<CRButtonSkin> CRButtonSkinRef;

class CRScrollSkin : public CRRectSkin {
public:
  enum Location { Title, Status };
...
public:
  ...
  int length() { return _list.length(); }
  CRButtonSkinRef get(int index) {
    return (index >= 0 && index < _list.length()) ? _list[index]
                                                  : LVRef<CRButtonSkin>();
  }
  ...
  CRButtonList() {}
  virtual ~CRButtonList() { CRLog::trace("~CRButtonList();"); }
};
...
public:
  CRToolBarSkin() {}
  virtual ~CRToolBarSkin() { CRLog::trace("~CRToolBarSkin();"); }

  CRButtonListRef getButtons() { return _buttons; }
  void setButtons(CRButtonListRef list) { _buttons = list; }
  virtual void drawToolBar(LVDrawBuf &buf, const lvRect &rc, bool enabled,
                           int selectedButton);
};
...
public:
  /// retuns path to base definition, if attribute base="#nodeid" is specified
  /// for element of path
  ...
  /// gets image from container
  virtual LVImageSourceRef getImage(const lString16 &filename) {
    return getImage(filename.c_str());
  }
  /// gets doc pointer by path
  virtual ldomXPointer getXPointer(const lString16 &xPointerStr) = 0;
  /// gets doc pointer by path
  virtual ldomXPointer getXPointer(const lChar16 *xPointerStr) {
    return getXPointer(lString16(xPointerStr));
  }
  /// reads int value from attrname attribute of element specified by path,
  /// returns defValue if not found
  virtual int readInt(const lChar16 *path, const lChar16 *attrname,
                      int defValue, bool *res = NULL);
  /// reads boolean value from attrname attribute of element specified by path,
  /// returns defValue if not found
  virtual bool readBool(const lChar16 *path, const lChar16 *attrname,
                        bool defValue, bool *res = NULL);
  /// reads image transform value from attrname attribute of element specified
  /// by path, returns defValue if not found
  ImageTransform readTransform(const lChar16 *path, const lChar16 *attrname,
                               ImageTransform defValue, bool *res);
  /// reads h align value from attrname attribute of element specified by path,
  /// returns defValue if not found
  virtual int readHAlign(const lChar16 *path, const lChar16 *attrname,
                         int defValue, bool *res = NULL);
  /// reads h align value from attrname attribute of element specified by path,
  /// returns defValue if not found
  virtual int readVAlign(const lChar16 *path, const lChar16 *attrname,
                         int defValue, bool *res = NULL);
  ...
  /// returns defValue if not found
  virtual lString16 readString(const lChar16 *path, const lChar16 *attrname,
                               const lString16 &defValue, bool *res = NULL);
  /// reads color value from attrname attribute of element specified by path,
  /// returns defValue if not found
  virtual lUInt32 readColor(const lChar16 *path, const lChar16 *attrname,
                            lUInt32 defValue, bool *res = NULL);
  /// reads rect value from attrname attribute of element specified by path,
  /// returns defValue if not found
  virtual lvRect readRect(const lChar16 *path, const lChar16 *attrname,
                          lvRect defValue, bool *res = NULL);
  /// reads point(size) value from attrname attribute of element specified by
  /// path, returns defValue if not found
  virtual lvPoint readSize(const lChar16 *path, const lChar16 *attrname,
                           lvPoint defValue, bool *res = NULL);
  /// reads rect value from attrname attribute of element specified by path,
  /// returns null ref if not found
  virtual LVImageSourceRef readImage(const lChar16 *path,
                                     const lChar16 *attrname, bool *res = NULL);
  /// reads list of icons
