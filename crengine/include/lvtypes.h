...
public:
  ...
  /// returns non-NULL pointer to trimming values for 4 sides of rc, if clipping
  /// is necessary
  ...
  /// makes this rect to cover both this and specified rect (bounding box for
  /// two rectangles)
  ...
  bool isRectInside(lvRect rc) const {
    if (rc.isEmpty() || isEmpty())
      return false;
    if (rc.left < left || rc.right > right || rc.top < top ||
        rc.bottom > bottom)
      return false;
    return true;
  }
...
public:
  ...
  lvColor(lUInt32 r, lUInt32 g, lUInt32 b, lUInt32 a)
      : value(((a & 255) << 24) | ((r & 255) << 16) | ((g & 255) << 8) |
              (b & 255)) {}
...
public:
  ...
  inline static lUInt16 rev(lUInt16 w) {
    return (lUInt16)(((w & 0xFF00) >> 8) | ((w & 0x00FF) << 8));
  }
  /// make 32 bit word least-significant-first byte order (Intel)
  lUInt32 lsf(lUInt32 w) { return (_lsf) ? w : rev(w); }
  /// make 32 bit word most-significant-first byte order (PPC)
  lUInt32 msf(lUInt32 w) { return (!_lsf) ? w : rev(w); }
  /// make 16 bit word least-significant-first byte order (Intel)
  lUInt16 lsf(lUInt16 w) { return (_lsf) ? w : rev(w); }
  /// make 16 bit word most-significant-first byte order (PPC)
  lUInt16 msf(lUInt16 w) { return (!_lsf) ? w : rev(w); }
  void rev(lUInt32 *w) { *w = rev(*w); }
  void rev(lUInt16 *w) { *w = rev(*w); }
  void msf(lUInt32 *w) {
    if (_lsf)
      *w = rev(*w);
  }
  ...
  bool lsf() { return (_lsf); }
  bool msf() { return (!_lsf); }
};
...
public:
  static lInt64 getSystemTimeMillis() {
    ...
    return ((lInt64)ts.dwLowDateTime) / 10000 +
           ((lInt64)ts.dwHighDateTime) * 1000;
#else
    timeval ts;
    gettimeofday(&ts, 0);
  ...
  void restart() { _start = getSystemTimeMillis(); }
  ...
  void cancel() { _interval = 0; }

  /// returns true if timeout is infinite
  bool infinite() { return _interval == -1; }
  ...
  lInt64 elapsed() { return getSystemTimeMillis() - _start; }
  int interval() { return (int)_interval; }
};
