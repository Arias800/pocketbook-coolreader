...
/// uncomment following LOG_RTF_PARSING definition to allow debug log of rtf
/// parsing
...
enum rtfImageFormat { rtf_img_unknown, rtf_img_png, rtf_img_jpeg };
...
enum tpropIndex {
  ...
  tpi_irowbandN, // N is the row index of the row, adjusted to account for
  ...
  tpi_clmrg,     // Contents of the table cell are merged with those of the
                 // preceding cell.
  tpi_clvmgf,    // The first cell in a range of table cells to be vertically
                 // merged.
  tpi_clvmrg, // Contents of the table cell are vertically merged with those of
              // the preceding cell.
  tpi_max
};
...
enum rtfTblState { tbls_none = 0, tbls_intable, tbls_inrow, tbls_incell };

enum rtf_cmd_id {
#define RTF_IPR(name, index, defvalue) RTF_##name,
#define RTF_TPR(name, index, defvalue) RTF_##name,
#define RTF_ACT(name, index) RTF_##name,
#define RTF_CMD(name, type, index) RTF_##name,
#define RTF_DST(name, index) RTF_##name,
#define RTF_CHC(name, index) RTF_##name,
#define RTF_CHR(character, name, index) RTF_##name,
...
public:
  enum rtf_actions { RA_PARA, RA_PARD, RA_PAGE, RA_SECTION };
...
public:
  /// constructor
  LVRtfValueStack() : dest(NULL), sp(0), error(false) {
    sp = 0;
  ...
  void setDestination(LVRtfDestination *newDest) { dest = newDest; }
  ...
  inline void set(LVRtfDestination *newdest) {
    if (sp >= MAX_PROP_STACK_SIZE) {
      ...
      CRLog::trace("Changing destination. Level=%d old=%08X new=%08X", sp,
                   (unsigned)dest, (unsigned)newdest);
#endif
  ...
  /// get int property
  inline int getInt(int index) { return props[index].i; }
  /// if int property > 0, decrement its value and return 0, otherwise do
  ...
  void *getPtr(int index) { return props[index].p; }
  ...
  bool restore() {
    for (;;) {
      ...
      if (i == pi_destination) {
...
#ifdef LOG_RTF_PARSING
        CRLog::trace("Restoring destination. Level=%d Value=%08X", sp,
                     (unsigned)dest);
#endif
      } else {
        sp--;
        ...
        //                if (i == pi_ansicpg) {
        //                    const lChar16 * table = (const lChar16
        //                    *)props[i].p;
        //                    this->getDestination()->SetCharsetTable(table);
        //                }
      }
