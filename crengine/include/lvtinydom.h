...
/// docFlag mask, enable internal stylesheet of document and style attribute of
/// elements
...
/// default docFlag set
#define DOC_FLAG_DEFAULTS                                                      \
  (DOC_FLAG_ENABLE_INTERNAL_STYLES | DOC_FLAG_ENABLE_FOOTNOTES |               \
   DOC_FLAG_ENABLE_DOC_FONTS)
...
struct img_scaling_options_t {
  ...
  /// returns hash value
  int getHash() {
    return (((zoom_in_inline.getHash() * 33 + zoom_in_block.getHash()) * 33 +
             zoom_out_inline.getHash()) *
                33 +
            zoom_out_block.getHash());
  }
...
public:
  ...
  /// Called when page images should be invalidated (clearImageCache() called in
  /// LVDocView)
  virtual void OnImageCacheClear() {}
  /// return true if reload will be processed by external code, false to let
  /// internal code process it
...
public:
  ...
  lUInt32 allocElem(lUInt32 dataIndex, lUInt32 parentIndex, int childCount,
                    int attrCount);
  /// get text by address
  ...
  ldomDataStorageManager(tinyNodeCollection *owner, char type,
                         int maxUnpackedSize, int chunkSize);
...
class ldomTextStorageChunk {
  ...
  lUInt32 _bufpos; /// _buf (uncompressed) data write position (for appending of
                   /// new data)
...
public:
  ...
  /// adds new element item to buffer, returns offset inside chunk of stored
  /// data
  int addElem(lUInt32 dataIndex, lUInt32 parentIndex, int childCount,
              int attrCount);
  ...
  ldomTextStorageChunk(int preAllocSize, ldomDataStorageManager *manager,
                       lUInt16 index);
...
protected:
  ...
  ldomDataStorageManager _styleStorage; // element style storage (font & style
                                        // indexes ldomNodeStyleInfo)
  ...
  /// uniquie id of file format parsing option (usually 0, but 1 for
...
public:
...
#if BUILD_LITE != 1
  /// swaps to cache file or saves changes, limited by time interval (can be
  /// called again to continue after TIMEOUT)
  ...
  /// saves recent changes to mapped file, with timeout (can be called again to
  ...
  inline int getFontContextDocIndex() {
    return (_docFlags & DOC_FLAG_ENABLE_DOC_FONTS) &&
                   (_docFlags & DOC_FLAG_ENABLE_INTERNAL_STYLES)
               ? _docIndex
               : -1;
  }
...
private:
  ...
  union {                    // [8] 8 bytes (16 bytes on x64)
    ...
    lUInt32
        _ptext_addr; // NT_PTEXT: persistent text storage address: chunk+offset
#endif
...
public:
  ...
  bool initNodeFont();
  void initNodeStyle();
  /// init render method for this node only (children should already have rend
  /// method set)
  ...
  const lString16 &getAttributeValue(const lChar16 *nsName,
                                     const lChar16 *attrName) const;
  /// returns attribute value by attribute name and namespace
  const lString16 &getAttributeValue(const lChar8 *nsName,
  ...
  /// formats final block
  int renderFinalBlock(LFormattedTextRef &frmtext, RenderRectAccessor *fmt,
                       int width);
  ...
  bool getNodeListMarker(int &counterValue, lString16 &marker,
                         int &markerWidth);
};
...
/**
    Helps to decrease memory usage and increase performance for DOM
   implementations. Maintains Name<->Id maps for element names, namespaces and
   attributes. It allows to use short IDs instead of strings in DOM internals,
...
public:
  ...
  /// deserialize from byte array (pointer will be incremented by number of
  /// bytes read)
...
protected:
  ...
  LVHashTable<lString16, LVImageSourceRef>
      _urlImageMap;    // url to image source map
  ...
  public:
    ...
    XPointerData(ldomNode *node, int offset)
        : _doc(node ? node->getDocument() : NULL),
          _dataIndex(node ? node->getDataIndex() : 0), _offset(offset),
          _refCount(1) {}
    // clone
    ...
    inline bool operator==(const XPointerData &v) const {
      return _doc == v._doc && _dataIndex == v._dataIndex &&
             _offset == v._offset;
    }
    inline bool operator!=(const XPointerData &v) const {
      return _doc != v._doc || _dataIndex != v._dataIndex ||
    ...
    inline ldomNode *getNode() {
      return _dataIndex > 0 ? ((lxmlDocBase *)_doc)->getTinyNode(_dataIndex)
                            : NULL;
    }
...
public:
  ...
  bool isPath() const { return !_data->isNull() && getOffset() == -1; }
  /// returns true if pointer is NULL
  bool operator!() const { return _data->isNull(); }
  /// returns true if pointers are equal
  bool operator==(const ldomXPointer &v) const { return *_data == *v._data; }
  ...
  ldomXPointer *clone() { return new ldomXPointer(_data); }
...
public:
  ...
  /// copy constructor
  ldomXPointerEx(const ldomXPointer &v) : ldomXPointer(v._data) { initIndex(); }
  /// copy constructor
  ...
  bool operator==(const ldomXPointerEx &v) const {
    return _data->getDocument() == v._data->getDocument() &&
           _data->getNode() == v._data->getNode() &&
           _data->getOffset() == v._data->getOffset();
  }
  ...
  /// ensure that current node is element (move to parent, if not - from text
  /// node to element)
  bool ensureElement();
  /// moves pointer to parent element with FINAL render method, returns true if
  /// success
  bool ensureFinal();
...
public:
  ...
  ldomXRange(const ldomWord &word)
      : _start(word.getStartXPointer()), _end(word.getEndXPointer()),
        _flags(1) {}
  ...
  bool findText(lString16 pattern, bool caseInsensitive, bool reverse,
                LVArray<ldomWord> &words, int maxCount, int maxHeight,
                bool checkMaxFromStart = false);
};
...
public:
  ...
  ldomMarkedText(const ldomMarkedText &v) : text(v.text), flags(v.flags) {}
};

typedef LVPtrVector<ldomMarkedText> ldomMarkedTextList;

enum MoveDirection { DIR_ANY, DIR_LEFT, DIR_RIGHT, DIR_UP, DIR_DOWN };
...
public:
  ...
  /// returns mark middle point for single line mark, or start point for
  /// multiline mark
...
private:
  ...
  //====================================================
  // LVTocItem( ldomXPointer pos, const lString16 & name ) : _parent(NULL),
  // _level(0), _index(0), _page(0), _percent(0), _name(name),
  // _path(pos.toString()), _position(pos) { }
...
public:
  ...
  LVTocItem(ldomDocument *doc)
      : _parent(NULL), _doc(doc), _level(0), _index(0) {}
  ~LVTocItem() { clear(); }
};
...
public:
  ...
  void setDocStylesheetFileName(lString16 fileName) {
  ...
  virtual int render(LVRendPageList *pages, LVDocViewCallback *callback,
                     int width, int dy, bool showCover, int y0,
                     font_ref_t def_font, int def_interline_space,
                     CRPropRef props);
  /// renders (formats) document in memory
  virtual bool setRenderProps(int width, int dy, bool showCover, int y0,
                              font_ref_t def_font, int def_interline_space,
                              CRPropRef props);
#endif
  ...
  bool findText(lString16 pattern, bool caseInsensitive, bool reverse, int minY,
                int maxY, LVArray<ldomWord> &words, int maxCount,
                int maxHeight);
#endif
};
...
class ldomElementWriter {
  ...
  ldomElementWriter(ldomDocument *document, lUInt16 nsid, lUInt16 id,
...
protected:
  ...
  virtual void setClass(const lChar16 *className,
...
public:
  ...
  virtual void OnText(const lChar16 *text, int len, lUInt32 flags);
  /// constructor
  ldomDocumentWriterFilter(ldomDocument *document, bool headerOnly,
                           const char ***rules);
  /// destructor
...
public:
  ...
  /// constructor
  ldomDocumentFragmentWriter(LVXMLParserCallback *parentWriter,
                             lString16 baseTagName,
                             lString16 baseTagReplacementName,
                             lString16 fragmentFilePath)
      : parent(parentWriter), baseTag(baseTagName),
        baseTagReplacement(baseTagReplacementName), insideTag(false),
        styleDetectionState(0), pathSubstitutions(100), baseElement(NULL),
        lastBaseElement(NULL), headStyleState(0) {
...
lString16 extractDocAuthors(ldomDocument *doc,
                            lString16 delimiter = lString16::empty_str,
                            bool shortMiddleName = true);
lString16 extractDocTitle(ldomDocument *doc);
lString16 extractDocLanguage(ldomDocument *doc);
/// returns "(Series Name #number)" if pSeriesNumber is NULL, separate name and
/// number otherwise
...
public:
  ...
  static LVStreamRef openExisting(lString16 filename, lUInt32 crc,
                                  lUInt32 docFlags);
  /// create new cache file
  static LVStreamRef createNew(lString16 filename, lUInt32 crc,
