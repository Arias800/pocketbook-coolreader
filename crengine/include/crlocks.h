...
public:
  CRGuard(CRMutexRef &_mutex) : mutex(_mutex.get()) {
    if (mutex)
      mutex->acquire();
  }
  CRGuard(CRMonitorRef &_mutex) : mutex(_mutex.get()) {
    if (mutex)
      mutex->acquire();
  }
  CRGuard(CRMutex *_mutex) : mutex(_mutex) {
    if (mutex)
      mutex->acquire();
  }
  CRGuard(CRMonitor *_mutex) : mutex(_mutex) {
    if (mutex)
      mutex->acquire();
  }
  ~CRGuard() {
...
// use FONT_GLYPH_CACHE_GUARD to acquire font global glyph cache operations
// mutex
#define FONT_GLYPH_CACHE_GUARD                                                 \
  CRGuard _fontGlyphCacheGuard(_fontGlyphCacheMutex);                          \
  CR_UNUSED(_fontGlyphCacheGuard);
// use FONT_LOCAL_GLYPH_CACHE_GUARD to acquire font global glyph cache
// operations mutex
