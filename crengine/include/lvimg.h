...
typedef void (*onObjectDestroyedCallback_t)(CacheObjectListener *pcache,
                                            lUInt32 pobject);
...
struct CR9PatchInfo {
  ...
  void calcRectangles(const lvRect &dst, const lvRect &src, lvRect dstitems[9],
...
enum ImageTransform {
  ...
  IMG_TRANSFORM_SPLIT, // split at specified pixel, fill extra middle space with
                       // value of this pixel
...
/// splitX, splitY
LVImageSourceRef
LVCreateStretchFilledTransform(LVImageSourceRef src, int newWidth,
                               int newHeight,
                               ImageTransform hTransform = IMG_TRANSFORM_SPLIT,
                               ImageTransform vTransform = IMG_TRANSFORM_SPLIT,
...
LVImageSourceRef LVCreateTileTransform(LVImageSourceRef src, int newWidth,
                                       int newHeight, int offsetX, int offsetY);
/// creates XPM image
...
/// maxSize (8 bit gray or 32 bit color)
LVImageSourceRef LVCreateUnpackedImageSource(
    LVImageSourceRef srcImage,
    int maxSize = MAX_SKIN_IMAGE_CACHE_ITEM_UNPACKED_SIZE, bool gray = false);
/// creates decoded memory copy of image, if it's unpacked size is less than
/// maxSize; bpp: 8,16,32 supported
...
/// RGB components added first, then multiplyed by multiplyRGB fixed point
/// components (0x20 is 1.0f)
...
#define IMAGE_SOURCE_FROM_BYTES(imgvar, bufvar)                                \
  ...
  LVImageSourceRef imgvar =                                                    \
      LVCreateStreamImageSource(LVCreateMemoryStream(bufvar, bufvar##_size))
