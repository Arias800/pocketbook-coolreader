...
struct LVFontGlyphCacheItem {
  ...
  static LVFontGlyphCacheItem *newItem(LVFontLocalGlyphCache *local_cache,
                                       lChar16 ch, int w, int h) {
    LVFontGlyphCacheItem *item = (LVFontGlyphCacheItem *)malloc(
        sizeof(LVFontGlyphCacheItem) + (w * h - 1) * sizeof(lUInt8));
    item->ch = ch;
  ...
  static void freeItem(LVFontGlyphCacheItem *item) { free(item); }
};
...
public:
  ...
  */
  ...
  virtual void DrawTextString(LVDrawBuf *buf, int x, int y, const lChar16 *text,
                              int len, lChar16 def_char,
                              lUInt32 *palette = NULL, bool addHyphen = false,
...
enum font_antialiasing_t { font_aa_none, font_aa_big, font_aa_all };
...
public:
  ...
  bool add(lString16 url) {
    return add(url, lString8::empty_str, false, false);
  }
...
public:
  ...
  virtual bool RegisterExternalFont(lString16 /*name*/, lString8 /*face*/,
                                    bool /*bold*/, bool /*italic*/) {
    return false;
  }
  /// registers document font
  virtual bool RegisterDocumentFont(int /*documentId*/,
                                    LVContainerRef /*container*/,
                                    lString16 /*name*/, lString8 /*face*/,
                                    bool /*bold*/, bool /*italic*/) {
    return false;
  }
  ...
  /// constructor
  LVFontManager()
      : _antialiasMode(font_aa_all), _allowKerning(false),
        _hintingMode(HINTING_MODE_AUTOHINT), _embolding(0), _tracking(0) {}
  /// destructor
  virtual ~LVFontManager() {}
  /// returns available typefaces
  virtual void getFaceList(lString16Collection &) {}

  /// returns first found face from passed list, or return face for font found
  /// by family only
  virtual lString8 findFontFace(lString8 commaSeparatedFaceList,
  ...
  virtual bool setalias(lString8 alias, lString8 facename, int id, bool italic,
                        bool bold) {
...
public:
  ...
  /// retrieves font handle
  virtual void *GetHandle() { return (void *)_hfont; }
  ...
  /// returns true if font is empty
  virtual bool IsNull() const { return (_hfont == NULL); }

  virtual bool operator!() const { return (_hfont == NULL); }

  virtual void Clear();

  virtual bool Create(const LOGFONTA &lf);

  virtual bool Create(int size, int weight, bool italic,
                      css_font_family_t family, lString8 typeface);

  virtual int getWeight() const { return _logfont.lfWeight; }

  virtual int getItalic() const { return _logfont.lfItalic; }

  virtual lString8 getTypeFace() const { return lString8::empty_str; }

  virtual css_font_family_t getFontFamily() const { return css_ff_inherit; }
...
public:
  GlyphCache(lUInt32 size) : _size(size) {
