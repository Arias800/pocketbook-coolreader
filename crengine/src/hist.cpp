...
public:
  ...
  virtual ldomNode *OnTagOpen(const lChar16 *nsname, const lChar16 *tagname) {
    ...
    if (lStr_cmp(tagname, "FictionBookMarks") == 0 && state == in_xml) {
      ...
      state = in_series;
    } else if (lStr_cmp(tagname, "doc-filename") == 0 &&
               state == in_file_info) {
      state = in_filename;
    } else if (lStr_cmp(tagname, "doc-filepath") == 0 &&
               state == in_file_info) {
      state = in_filepath;
    } else if (lStr_cmp(tagname, "doc-filesize") == 0 &&
               state == in_file_info) {
      state = in_filesize;
  ...
  virtual void OnTagClose(const lChar16 *nsname, const lChar16 *tagname) {
    ...
    } else if (lStr_cmp(tagname, "start-point") == 0 &&
               state == in_start_point) {
      state = in_bm;
    } else if (lStr_cmp(tagname, "end-point") == 0 && state == in_end_point) {
      state = in_bm;
    } else if (lStr_cmp(tagname, "header-text") == 0 &&
               state == in_header_txt) {
      state = in_bm;
    } else if (lStr_cmp(tagname, "selection-text") == 0 &&
               state == in_selection_txt) {
      state = in_bm;
    } else if (lStr_cmp(tagname, "comment-text") == 0 &&
               state == in_comment_txt) {
      state = in_bm;
    }
  }
  ...
  virtual void OnAttribute(const lChar16 *nsname, const lChar16 *attrname,
    ...
    if (lStr_cmp(attrname, "type") == 0 && state == in_bm) {
      static const char *tnames[] = {"lastpos", "position", "comment",
                                     "correction"};
      for (int i = 0; i < 4; i++) {
...
static void putTagValue(LVStream *stream, int level, const char *tag,
                        lString16 value) {
...
static void putBookmark(LVStream *stream, CRBookmark *bmk) {
  static const char *tnames[] = {"lastpos", "position", "comment",
                                 "correction"};
  const char *tname =
      bmk->getType() >= bmkt_lastpos && bmk->getType() <= bmkt_correction
          ? tnames[bmk->getType()]
          : "unknown";
  char bmktag[256];
  sprintf(bmktag,
          "bookmark type=\"%s\" percent=\"%d.%02d%%\" timestamp=\"%d\" "
          "shortcut=\"%d\" page=\"%d\"",
          tname, bmk->getPercent() / 100, bmk->getPercent() % 100,
          (int)bmk->getTimestamp(), (int)bmk->getShortcut(),
          (int)bmk->getBookmarkPage());
...
bool CRFileHist::saveToStream(LVStream *targetStream) {
  ...
  const char *xml_hdr = "\xef\xbb\xbf<?xml version=\"1.0\" "
                        "encoding=\"utf-8\"?>\r\n<FictionBookMarks>\r\n";
  ...
  *stream << xml_hdr;
  for (int i = 0; i < _records.length(); i++) {
    ...
    putTagValue(stream, 3, "doc-filesize",
                lString16::itoa((unsigned int)rec->getFileSize()));
    putTag(stream, 2, "/file-info");
...
CRBookmark *CRFileHistRecord::setShortcutBookmark(int shortcut,
                                                  ldomXPointer ptr) {
  if (ptr.isNull())
    return NULL;
...
int CRFileHistRecord::getLastShortcutBookmark() {
  int last = -1;
  for (int i = 0; i < _bookmarks.length(); i++) {
    if (_bookmarks[i]->getShortcut() > 0 &&
        _bookmarks[i]->getShortcut() > last &&
        _bookmarks[i]->getShortcut() < MAX_SHORTCUT_BOOKMARKS &&
        _bookmarks[i]->getType() == bmkt_pos)
...
int CRFileHist::findEntry(const lString16 &fname, const lString16 &fpath,
                          lvsize_t sz) {
  CR_UNUSED(fpath);
  for (int i = 0; i < _records.length(); i++) {
    CRFileHistRecord *rec = _records[i];
    ...
    if (rec->getFileSize() != sz) {
      CRLog::warn("CRFileHist::findEntry() Filename matched %s but sizes are "
                  "different %d!=%d",
                  LCSTR(fname), sz, rec->getFileSize());
      continue;
    }
    return i;
  }
...
void CRFileHistRecord::setLastPos(CRBookmark *bmk) { _lastpos = *bmk; }
...
lString16 CRBookmark::getChapterName(ldomXPointer ptr) {
  ...
  lUInt16 section_id =
      ptr.getNode()->getDocument()->getElementNameIndex(L"section");
  if (!ptr.isNull()) {
    ...
    while (!p.isNull()) {
      ...
      if (!nname.compare("title") || !nname.compare("h1") ||
          !nname.compare("h2") || !nname.compare("h3")) {
...
ldomXPointer CRFileHist::restorePosition(ldomDocument *doc, lString16 fpathname,
                                         size_t sz) {
  lString16 name;
  lString16 path;
  splitFName(fpathname, path, name);
...
CRBookmark::CRBookmark(ldomXPointer ptr)
    : _startpos(lString16::empty_str), _endpos(lString16::empty_str),
      _percent(0), _type(0), _shortcut(0), _postext(lString16::empty_str),
      _titletext(lString16::empty_str), _commenttext(lString16::empty_str),
      _timestamp(time_t(0)), _page(0) {
  //
  if (ptr.isNull())
    return;
...
lString16 CRFileHistRecord::getLastTimeString(bool longFormat) {
  ...
  char str[20];
  if (!longFormat)
    sprintf(str, "%02d.%02d.%04d", bt->tm_mday, 1 + bt->tm_mon,
            1900 + bt->tm_year);
  else
    sprintf(str, "%02d.%02d.%04d %02d:%02d", bt->tm_mday, 1 + bt->tm_mon,
            1900 + bt->tm_year, bt->tm_hour, bt->tm_min);
...
ChangeInfo::ChangeInfo(CRBookmark *bookmark, lString16 fileName, bool deleted)
    : _bookmark(bookmark ? new CRBookmark(*bookmark) : NULL),
      _fileName(fileName), _deleted(deleted) {
  _timestamp = bookmark && bookmark->getTimestamp() > 0
...
ChangeInfo *ChangeInfo::fromString(lString8 s) {
  ...
  if (rows.length() < 3 || rows[0] != START_TAG ||
      rows[rows.length() - 1] != END_TAG)
    return NULL;
  ...
  if (ci->_fileName.empty() || ci->_timestamp == 0 ||
      (!ci->_bookmark && !ci->_deleted)) {
    delete ci;
    return NULL;
  }
  return ci;
...
bool ChangeInfo::findNextRecordBounds(lChar8 *buf, int start, int end,
                                      int &recordStart, int &recordEnd) {
