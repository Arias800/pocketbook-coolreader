...
/// returns first found face from passed list, or return face for font found by
/// family only
lString8 LVFontManager::findFontFace(lString8 commaSeparatedFaceList,
                                     css_font_family_t fallbackByFamily) {
  // faces we want
...
int LVFontManager::GetGammaIndex() { return gammaIndex; }
...
void LVFontManager::SetGammaIndex(int index) {
  ...
  if (gammaIndex != index) {
    CRLog::trace("FontManager gamma index changed from %d to %d", gammaIndex,
                 index);
    gammaIndex = index;
...
double LVFontManager::GetGamma() { return gammaLevel; }
...
void LVFontManager::SetGamma(double gamma) {
  ...
  if (gammaIndex != oldGammaIndex) {
    CRLog::trace("FontManager gamma index changed from %d to %d", oldGammaIndex,
                 gammaIndex);
    clearGlyphCache();
  }
}
...
bool LVEmbeddedFontList::addAll(LVEmbeddedFontList &list) {
  ...
  for (int i = 0; i < list.length(); i++) {
    ...
    changed =
        add(def->getUrl(), def->getFace(), def->getBold(), def->getItalic()) ||
        changed;
  }
  return changed;
}

bool LVEmbeddedFontList::add(lString16 url, lString8 face, bool bold,
                             bool italic) {
...
public:
  LVFontDef(const lString8 &name, int size, int weight, int italic,
            css_font_family_t family, const lString8 &typeface, int index = -1,
            int documentId = -1, LVByteArrayRef buf = LVByteArrayRef())
      : _size(size), _weight(weight), _italic(italic), _family(family),
        _typeface(typeface), _name(name), _index(index),
        _documentId(documentId), _buf(buf) {}
  LVFontDef(const LVFontDef &def)
      : _size(def._size), _weight(def._weight), _italic(def._italic),
        _family(def._family), _typeface(def._typeface), _name(def._name),
        _index(def._index), _documentId(def._documentId), _buf(def._buf) {}
  ...
  bool operator==(const LVFontDef &def) const {
    return (_size == def._size || _size == -1 || def._size == -1) &&
           ...
           _family == def._family && _typeface == def._typeface &&
           _name == def._name && (_index == def._index || def._index == -1) &&
           (_documentId == def._documentId || _documentId == -1);
  }

  lUInt32 getHash() {
    return ((((_size * 31) + _weight) * 31 + _italic) * 31 + _family) * 31 +
           _name.getHash();
  }
...
public:
  ...
  LVFontCacheItem(const LVFontDef &def) : _def(def) {}
};
  ...
  virtual void getFaceList(lString16Collection &list) {
    ...
    for (int i = 0; i < _registered_list.length(); i++) {
      if (_registered_list[i]->getDef()->getDocumentId() != -1)
        continue;
      lString16 name =
          Utf8ToUnicode(_registered_list[i]->getDef()->getTypeFace());
...
public:
  ...
  LVFontGlyphWidthCache() { memset(ptrs, 0, 128 * sizeof(lUInt8 *)); }
  ~LVFontGlyphWidthCache() { clear(); }
};

class LVFreeTypeFace;
static LVFontGlyphCacheItem *newItem(LVFontLocalGlyphCache *local_cache,
                                     lChar16 ch,
                                     FT_GlyphSlot slot) // , bool drawMonochrome
{
  ...
  LVFontGlyphCacheItem *item =
      LVFontGlyphCacheItem::newItem(local_cache, ch, w, h);
  ...
  if ((myabs(slot->metrics.horiAdvance) + tempTracking) < 0)
    tempTracking = 0;
  item->advance =
      (lUInt8)((myabs(slot->metrics.horiAdvance) + tempTracking) >> 6);
  return item;
}
...
lString8 familyName(FT_Face face) {
  ...
  if (faceName == "Arial" && face->style_name &&
      !strcmp(face->style_name, "Narrow"))
    faceName << " " << face->style_name;
  else if (/*faceName == "Arial" &&*/ face->style_name &&
           strstr(face->style_name, "Condensed"))
    faceName << " " << "Condensed";
  return faceName;
}
...
static lUInt16 char_flags[] = {
    ...
    0,
    0,
    LCHAR_IS_SPACE | LCHAR_IS_EOL | LCHAR_ALLOW_WRAP_AFTER,
    0,
    0,
    LCHAR_IS_SPACE | LCHAR_IS_EOL | LCHAR_ALLOW_WRAP_AFTER,
    0,
    ...
    0,
    0, // 24   18
    LCHAR_IS_SPACE | LCHAR_ALLOW_WRAP_AFTER,
    0,
    0,
    0,
    0,
  ...
  (ch < 48 ? char_flags[ch]                                                    \
           : (ch == UNICODE_SOFT_HYPHEN_CODE                                   \
                  ...
                  : (ch == UNICODE_NO_BREAK_SPACE                              \
                         ? LCHAR_DEPRECATED_WRAP_AFTER | LCHAR_IS_SPACE        \
...
public:
  ...
  LVFreeTypeFace(LVMutex &mutex, FT_Library library,
                 LVFontGlobalGlyphCache *globalCache)
      : _mutex(mutex), _fontFamily(css_ff_sans_serif), _library(library),
        _face(NULL), _size(0), _hyphen_width(0), _baseline(0), _weight(400),
        _italic(0), _glyph_cache(globalCache), _drawMonochrome(false),
        _allowKerning(false), _hintingMode(HINTING_MODE_AUTOHINT),
        _embolding(0), _tracking(0), _fallbackFontIsSet(false) {
    _matrix.xx = 0x10000;
    _matrix.yy = 0x10000;
    _matrix.xy = 0;
    _matrix.yx = 0;
  ...
  bool loadFromBuffer(LVByteArrayRef buf, int index, int size,
                      css_font_family_t fontFamily, bool monochrome,
                      bool italicize) {
    FONT_GUARD
    _hintingMode = fontMan->GetHintingMode();
    _embolding = fontMan->GetEmbolding();
    ...
    CRLog::debug("Loaded font %s [%d]: faceName=%s, ", _fileName.c_str(), index,
                 _faceName.c_str());
  ...
  bool loadFromFile(const char *fname, int index, int size,
    ...
    if (_fileName.empty())
      return false;
    int error = FT_New_Face(_library, _fileName.c_str(), index,
                            &_face); /* create face object */
    if (error)
      return false;
    if (_fileName.endsWith(".pfb") || _fileName.endsWith(".pfa")) {
  ...
  */
  virtual bool getGlyphInfo(lUInt16 code, glyph_info_t *glyph,
    ...
    int light_mode =
        ((_hintingMode == HINTING_MODE_LIGHT) ? FT_LOAD_TARGET_LIGHT
    ...
    if ((myabs(_slot->metrics.horiAdvance) + tempTracking) < 0)
      tempTracking = 0;
    glyph->width =
        (lUInt8)((myabs(_slot->metrics.horiAdvance) + tempTracking) >> 6);
    return true;
  }
  /*
  ...
  virtual lUInt16 measureText(const lChar16 *text, int len, lUInt16 *widths,
    ...
    for (nchars = 0; nchars < len; nchars++) {
      ...
      if (w == 0xFF) {
        ...
        //                error = FT_Load_Glyph( _face,          /* handle to
        //                face object */
        //                        ch_glyph_index,                /* glyph index
        //                        */ FT_LOAD_DEFAULT );             /* load
        //                        flags, see below */
        //                if ( error ) {
        //                    widths[nchars] = prev_width;
        //                    continue;  /* ignore errors */
  ...
  */
  virtual lUInt32 getTextWidth(const lChar16 *text, int len) {
    ...
    if (len <= 0)
      return 0;
    lUInt16 res = measureText(text, len, widths, flags,
                              2048, // max_width,
                              L' ', // def_char
                              0);
  ...
  */
  virtual LVFontGlyphCacheItem *getGlyph(lUInt16 ch, lChar16 def_char = 0) {
    ...
    if (!item) {
      ...
      FT_Render_Glyph(_face->glyph, (!_drawMonochrome ? FT_RENDER_MODE_NORMAL
                                                      : FT_RENDER_MODE_MONO));
  ...
  virtual int getBaseline() { return _baseline; }
  ...
  virtual lString8 getTypeFace() const { return _faceName; }

  /// returns font family id
  virtual css_font_family_t getFontFamily() const { return _fontFamily; }
  ...
  /// draws text string
  virtual void DrawTextString(LVDrawBuf *buf, int x, int y, const lChar16 *text,
                              int len, lChar16 def_char, lUInt32 *palette,
                              bool addHyphen, lUInt32 flags,
                              int letter_spacing) {
    FONT_GUARD
    if (len <= 0 || _face == NULL)
      return;
    ...
    for (i = 0; i <= len; i++) {
      ...
      if (!item)
        continue;
      if ((item && !isHyphen) ||
          i >= len - 1) { // avoid soft hyphens inside text string
        int w = item->advance + (kerning >> 6);
        buf->Draw(x + (kerning >> 6) + item->origin_x,
                  y + _baseline - item->origin_y, item->bmp, item->bmp_width,
                  item->bmp_height, palette);
  ...
  virtual bool IsNull() const { return _face == NULL; }

  virtual bool operator!() const { return _face == NULL; }
...
public:
  ...
  /// returns italic flag
  virtual int getItalic() const { return _baseFont->getItalic(); }
  ...
  */
  virtual lUInt16 measureText(const lChar16 *text, int len, lUInt16 *widths,
    ...
    lUInt16 res = _baseFont->measureText(text, len, widths, flags, max_width,
                                         def_char, letter_spacing);
    int w = 0;
    for (int i = 0; i < res; i++) {
      w += _hShift;
      widths[i] += w;
  ...
  */
  virtual LVFontGlyphCacheItem *getGlyph(lUInt16 ch, lChar16 def_char = 0) {
    ...
    item = LVFontGlyphCacheItem::newItem(&_glyph_cache, ch, dx,
                                         dy); //, _drawMonochrome
  ...
  /** \brief get glyph image in 1 byte per pixel format
      ...
      \return true if glyph was found
  */
  //    virtual bool getGlyphImage(lUInt16 code, lUInt8 * buf, lChar16
  //    def_char=0 )
  //    {
  ...
  /// returns font typeface name
  virtual lString8 getTypeFace() const { return _baseFont->getTypeFace(); }
  ...
  virtual void DrawTextString(LVDrawBuf *buf, int x, int y, const lChar16 *text,
                              int len, lChar16 def_char, lUInt32 *palette,
                              bool addHyphen, lUInt32 flags = 0,
                              int letter_spacing = 0) {
    if (len <= 0)
      return;
    if (letter_spacing < 0 || letter_spacing > 50)
    ...
    for (i = 0; i <= len; i++) {
      ...
      if (item) {
        ...
        if (item->bmp_width && item->bmp_height &&
            (!isHyphen || i >= len - 1)) {
          buf->Draw(x + item->origin_x, y + _baseline - item->origin_y,
                    item->bmp, item->bmp_width, item->bmp_height, palette);
        }
      }
      x += w + letter_spacing;
    }
  ...
  virtual bool getBitmapMode() { return _baseFont->getBitmapMode(); }

  /// set bitmap mode (true=monochrome bitmap, false=antialiased)
  virtual void setBitmapMode(bool m) { _baseFont->setBitmapMode(m); }

  /// sets current hinting mode
  virtual void setHintingMode(hinting_mode_t mode) {
    _baseFont->setHintingMode(mode);
  }
  /// returns current hinting mode
  virtual hinting_mode_t getHintingMode() const {
    return _baseFont->getHintingMode();
  }
  ...
  /// returns true if font is empty
  virtual bool IsNull() const { return _baseFont->IsNull(); }

  virtual bool operator!() const { return !(*_baseFont); }
  virtual void Clear() { _baseFont->Clear(); }
...
static LVFontRef dumpFontRef(LVFontRef fnt) {
  CRLog::trace("%s %d (%d) w=%d %s", fnt->getTypeFace().c_str(), fnt->getSize(),
               fnt->getHeight(), fnt->getWeight(),
               fnt->getItalic() ? "italic" : "");
  return fnt;
}
#endif
...
public:
  ...
  virtual lUInt32 GetFontListHash(int documentId) {
    FONT_MAN_GUARD
    return _cache.GetFontListHash(documentId) * 75 +
           _fallbackFontFace.getHash();
  }
  ...
  /// sets current gamma level
  virtual hinting_mode_t GetHintingMode() { return _hintingMode; }
  ...
  virtual int GetEmbolding() { return _embolding; }
  ...
  virtual int GetTracking() { return _tracking; }
  ...
  virtual int GetFontCount() { return _cache.length(); }

  bool initSystemFonts() {
#if (DEBUG_FONT_SYNTHESIS == 1)
    fontMan->RegisterFont(
        lString8("/usr/share/fonts/liberation/LiberationSans-Regular.ttf"));
    CRLog::debug("fonts:");
    LVFontRef fnt4 = dumpFontRef(fontMan->GetFont(
        24, 200, true, css_ff_sans_serif, cs8("Arial, Helvetica")));
    LVFontRef fnt1 = dumpFontRef(fontMan->GetFont(
        18, 200, false, css_ff_sans_serif, cs8("Arial, Helvetica")));
    LVFontRef fnt2 = dumpFontRef(fontMan->GetFont(
        20, 400, false, css_ff_sans_serif, cs8("Arial, Helvetica")));
    LVFontRef fnt3 = dumpFontRef(fontMan->GetFont(
        22, 600, false, css_ff_sans_serif, cs8("Arial, Helvetica")));
    LVFontRef fnt5 = dumpFontRef(fontMan->GetFont(
        26, 400, true, css_ff_sans_serif, cs8("Arial, Helvetica")));
    LVFontRef fnt6 = dumpFontRef(fontMan->GetFont(
        28, 600, true, css_ff_sans_serif, cs8("Arial, Helvetica")));
...
#elif (USE_FONTCONFIG == 1)
    {
      ...
      FcObjectSet *os =
          FcObjectSetBuild(FC_FILE, FC_WEIGHT, FC_FAMILY, FC_SLANT, FC_SPACING,
                           FC_INDEX, FC_STYLE, NULL);
      ...
      for (int i = 0; i < fontset->nfont; i++) {
        ...
        FcResult res;
        // FC_SCALABLE
        // res = FcPatternGetBool( fontset->fonts[i], FC_OUTLINE, 0,
        // (FcBool*)&b); if(res != FcResultMatch)
        //     continue;
        // if ( !b )
        ...
        fn16.lowercase();
        if (!fn16.endsWith(".ttf") && !fn16.endsWith(".odf") &&
            !fn16.endsWith(".otf") && !fn16.endsWith(".pfb") &&
            !fn16.endsWith(".pfa")) {
          continue;
        }
        ...
        res = FcPatternGetString(fontset->fonts[i], FC_FAMILY, 0,
                                 (FcChar8 **)&family);
        if (res != FcResultMatch) {
          CRLog::debug("no FC_FAMILY for %s", s);
          continue;
        }
        res = FcPatternGetString(fontset->fonts[i], FC_STYLE, 0,
                                 (FcChar8 **)&style);
        if (res != FcResultMatch) {
        ...
        LVFontDef def(lString8((const char *)s),
                      -1, // height==-1 for scalable fonts
                      weight, italic, fontFamily, face, index);

        CRLog::debug("FONTCONFIG: Font family:%s style:%s weight:%d slant:%d "
                     "spacing:%d file:%s",
      ...
      const char *fallback_faces[] = {
          "Arial Unicode MS", "AR PL ShanHeiSun Uni", "Liberation Sans", NULL};
    ...
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Arial Bold.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Arial Italic.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Arial Bold Italic.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Arial Unicode.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Arial Narrow.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Arial Narrow Bold.ttf"));
    facesFound += fontMan->RegisterFont(
        lString8("/Library/Fonts/Arial Narrow Italic.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Courier New.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Courier New Bold.ttf"));
    facesFound += fontMan->RegisterFont(
        lString8("/Library/Fonts/Courier New Italic.ttf"));
    facesFound += fontMan->RegisterFont(
        lString8("/Library/Fonts/Courier New Bold Italic.ttf"));
    facesFound += fontMan->RegisterFont(lString8("/Library/Fonts/Georgia.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Georgia Bold.ttf"));
    facesFound +=
        fontMan->RegisterFont(lString8("/Library/Fonts/Georgia Italic.ttf"));
    facesFound += fontMan->RegisterFont(
        lString8("/Library/Fonts/Georgia Bold Italic.ttf"));
  ...
  LVFreeTypeFontManager() : _library(NULL), _globalCache(GLYPH_CACHE_SIZE) {
    FONT_MAN_GUARD
  ...
  bool setalias(lString8 alias, lString8 facename, int id, bool italic,
                bool bold) {
    FONT_MAN_GUARD
    lString8 fontname = lString8("\0");
    LVFontDef def(fontname, 10, 400, true, css_ff_inherit, facename, -1, id);
    LVFontCacheItem *item = _cache.find(&def);
    LVFontDef def1(fontname, 10, 400, false, css_ff_inherit, alias, -1, id);
    if (!item->getDef()->getName().empty()) {
      ...
      for (;; index++) {
        int error = FT_New_Face(_library, item->getDef()->getName().c_str(),
                                index, &face); /* create face object */
        if (error) {
          if (index == 0) {
        ...
        bool boldFlag = !facename.empty()
                            ? bold
                            : (face->style_flags & FT_STYLE_FLAG_BOLD) != 0;
        bool italicFlag = !facename.empty()
                              ? italic
                              : (face->style_flags & FT_STYLE_FLAG_ITALIC) != 0;

        LVFontDef def2(item->getDef()->getName(),
                       -1, // height==-1 for scalable fonts
                       boldFlag ? 700 : 400, italicFlag, fontFamily, alias,
                       index, id);
  ...
  virtual LVFontRef GetFont(int size, int weight, bool italic,
    ...
    if (_log) {
      fprintf(
          _log,
          "GetFont(size=%d, weight=%d, italic=%d, family=%d, typeface='%s')\n",
          size, weight, italic ? 1 : 0, (int)family, typeface.c_str());
    }
#endif
    lString8 fontname;
    LVFontDef def(fontname, size, weight, italic, family, typeface, -1,
                  documentId);
#if (DEBUG_FONT_MAN == 1)
    if (_log)
      fprintf(_log, "GetFont: %s %d %s %s\n", typeface.c_str(), size,
              weight > 400 ? "bold" : "", italic ? "italic" : "");
#endif
    LVFontCacheItem *item = _cache.find(&def);
#if (DEBUG_FONT_MAN == 1)
    if (item && _log) { //_log &&
      fprintf(_log,
              "   found item: (file=%s[%d], size=%d, weight=%d, italic=%d, "
              "family=%d, typeface=%s, weightDelta=%d) FontRef=%d\n",
              item->getDef()->getName().c_str(), item->getDef()->getIndex(),
              item->getDef()->getSize(), item->getDef()->getWeight(),
              item->getDef()->getItalic() ? 1 : 0,
              (int)item->getDef()->getFamily(),
              item->getDef()->getTypeFace().c_str(),
              weight - item->getDef()->getWeight(),
              item->getFont().isNull() ? 0 : item->getFont()->getHeight());
    }
#endif
    ...
    if (!item->getFont().isNull()) {
      ...
      if (deltaWeight >= 200) {
        ...
        LVFontRef ref =
            LVFontRef(new LVFontBoldTransform(item->getFont(), &_globalCache));
        _cache.update(&newDef, ref);
        return ref;
      } else {
    ...
    if (_log) {
      ...
      fprintf(_log,
              "   no instance: adding new one for filename=%s, index = %d\n",
              fname.c_str(), index);
    }
#endif
    ...
    if (item->getDef()->getBuf().isNull())
      loaded =
          font->loadFromFile(pathname.c_str(), item->getDef()->getIndex(), size,
                             family, isBitmapModeForSize(size), italicize);
    else
      loaded = font->loadFromBuffer(item->getDef()->getBuf(),
                                    item->getDef()->getIndex(), size, family,
  ...
  */
  ...
  virtual bool RegisterDocumentFont(int documentId, LVContainerRef container,
                                    lString16 name, lString8 faceName,
                                    bool bold, bool italic) {
    FONT_MAN_GUARD
    lString8 name8 = UnicodeToUtf8(name);
    CRLog::debug("RegisterDocumentFont(documentId=%d, path=%s)", documentId,
                 name8.c_str());
    ...
    if (stream->Read(buf->get(), size, &bytesRead) != LVERR_OK ||
        bytesRead != size)
      return false;
    bool res = false;
    ...
    for (;; index++) {
      ...
      //                    name.c_str(), !charset?"no mandatory characters in
      //                    charset" : "font is not scalable"
      //                    );
      //    //            }
      //    //#endif
      //                if ( face ) {
      ...
      LVFontDef def(name8,
                    -1, // height==-1 for scalable fonts
                    boldFlag ? 700 : 400, italicFlag, fontFamily, familyName,
                    index, documentId, buf);
#if (DEBUG_FONT_MAN == 1)
      if (_log) {
        fprintf(_log,
                "registering font: (file=%s[%d], size=%d, weight=%d, "
                "italic=%d, family=%d, typeface=%s)\n",
                def.getName().c_str(), def.getIndex(), def.getSize(),
                def.getWeight(), def.getItalic() ? 1 : 0, (int)def.getFamily(),
  ...
  virtual bool RegisterExternalFont(lString16 name, lString8 family_name,
                                    bool bold, bool italic) {
    ...
    // for all faces in file
    for (;; index++) {
      int error = FT_New_Face(_library, fname.c_str(), index,
                              &face); /* create face object */
      if (error) {
        if (index == 0) {
          CRLog::error("FT_New_Face returned error %d", error);
      ...
      if (!scal || !charset) {
        // #if (DEBUG_FONT_MAN==1)
        //            if ( _log ) {
        CRLog::debug("    won't register font %s: %s", name.c_str(),
                     !charset ? "no mandatory characters in charset"
      ...
      LVFontDef def(fname,
                    -1, // height==-1 for scalable fonts
                    bold ? 700 : 400, italic ? true : false, fontFamily,
                    family_name, index);
#if (DEBUG_FONT_MAN == 1)
      if (_log) {
        fprintf(_log,
...
public:
  ...
  lString8 makeFontFileName(lString8 name) {
    lString8 filename = _path;
    if (!filename.empty() &&
        _path[filename.length() - 1] != PATH_SEPARATOR_CHAR)
      filename << PATH_SEPARATOR_CHAR;
    filename << name;
    return filename;
  }
  virtual LVFontRef GetFont(int size, int weight, bool italic,
                            css_font_family_t family, lString8 typeface,
                            int documentId) {
    LVFontDef *def = new LVFontDef(lString8::empty_str, size, weight, italic,
                                   family, typeface, documentId);
    // fprintf( _log, "GetFont: %s %d %s %s\n",
    //     typeface.c_str(),
    //     size,
    //     weight>400?"bold":"",
  ...
  virtual bool RegisterFont(lString8 name) {
    ...
    if (stream->Read(&hdr, sizeof(hdr), &bytes_read) == LVERR_OK &&
        bytes_read == sizeof(hdr)) {
      LVFontDef def(name, hdr.fontHeight, hdr.flgBold ? 700 : 400,
                    hdr.flgItalic ? true : false,
                    (css_font_family_t)hdr.fontFamily, lString8(hdr.fontName));
...
public:
  ...
  virtual LVFontRef GetFont(int size, int weight, bool bitalic,
                            css_font_family_t family, lString8 typeface) {
    ...
    LVFontDef def(lString8::empty_str, size, weight, italic, family, typeface);
    ...
    LVFontDef def2(fdef->getName(), size, weight, italic, fdef->getFamily(),
                   fdef->getTypeFace());

    if (font->Create(size, weight, italic ? true : false, fdef->getFamily(),
                     fdef->getTypeFace())) {
      // fprintf(_log, "    : loading from file %s : %s %d\n",
      // item->getDef()->getName().c_str(),
      //     item->getDef()->getTypeFace().c_str(), item->getDef()->getSize() );
  ...
  virtual bool RegisterFont(lString8 name) { return false; }
  virtual bool Init(lString8 path) {
...
int LVFontDef::CalcDuplicateMatch(const LVFontDef &def) const {
  if (def._documentId != -1 && _documentId != def._documentId)
    return false;
  bool size_match =
      (_size == -1 || def._size == -1) ? true : (def._size == _size);
  bool weight_match =
      (_weight == -1 || def._weight == -1) ? true : (def._weight == _weight);
  bool italic_match =
      (_italic == def._italic || _italic == -1 || def._italic == -1);
  bool family_match = (_family == css_ff_inherit ||
                       def._family == css_ff_inherit || def._family == _family);
  bool typeface_match = (_typeface == def._typeface);
  return size_match && weight_match && italic_match && family_match &&
         typeface_match;
}
...
int LVFontDef::CalcMatch(const LVFontDef &def) const {
  ...
  int italic_match =
      (_italic == def._italic || _italic == -1 || def._italic == -1) ? 256 : 0;
  if ((_italic == 2 || def._italic == 2) && _italic > 0 && def._italic > 0)
    italic_match = 128;
  int family_match =
      (_family == css_ff_inherit || def._family == css_ff_inherit ||
       def._family == _family)
          ? 256
          : ((_family == css_ff_monospace) == (def._family == css_ff_monospace)
                 ? 64
                 : 0);
  int typeface_match = (_typeface == def._typeface) ? 256 : 0;
  return +(size_match * 100) + (weight_match * 5) + (italic_match * 5) +
         (family_match * 100) + (typeface_match * 1000);
}
...
void LVBaseFont::DrawTextString(LVDrawBuf *buf, int x, int y,
                                ...
                                lUInt32 *palette, bool addHyphen, lUInt32,
                                int) {
  ...
  while (len >= (addHyphen ? 0 : 1)) {
    if (len <= 1 || *text != UNICODE_SOFT_HYPHEN_CODE) {
      ...
      if (item) {
        ...
        w = item->advance;
        if (item->bmp_width && item->bmp_height) {
          buf->Draw(x + item->origin_x, y + baseline - item->origin_y,
                    item->bmp, item->bmp_width, item->bmp_height, palette);
        }
      }
      x += w; // + letter_spacing;
...
lUInt16 LBitmapFont::measureText(const lChar16 *text, int len, lUInt16 *widths,
  ...
  return lvfontMeasureText(m_font, text, len, widths, flags, max_width,
                           def_char);
}
...
LVFontCacheItem *LVFontCache::findDocumentFontDuplicate(int documentId,
                                                        lString8 name) {
  for (int i = 0; i < _registered_list.length(); i++) {
...
void LVFontCache::gc() {
  ...
  if (CRLog::isDebugEnabled())
    CRLog::debug("LVFontCache::gc() : %d fonts still used, %d fonts dropped",
                 usedCount, droppedCount);
}
...
bool LVBaseWin32Font::Create(const LOGFONTA &lf) {
  ...
  GetTextFaceA(_drawbuf.GetDC(), sizeof(_logfont.lfFaceName) - 1,
               _logfont.lfFaceName);
  _height = tm.tmHeight;
...
bool LVBaseWin32Font::Create(int size, int weight, bool italic,
                             css_font_family_t family, lString8 typeface) {
  if (!IsNull())
    Clear();
  //
...
int LVWin32DrawFont::getCharWidth(lChar16 ch, lChar16 def_char) {
  ...
  lUInt32 res = GetCharacterPlacementW(_drawbuf.GetDC(), str, 1, 100, &gcpres,
...
/** \brief measure text
    ...
    \return true if glyph was found
*/
lUInt16 LVWin32DrawFont::measureText(const lChar16 *text, int len,
                                     lUInt16 *widths, lUInt8 *flags,
                                     int max_width, lChar16 def_char,
                                     int letter_spacing,
  ...
  lUInt32 res =
      GetCharacterPlacementW(_drawbuf.GetDC(), pstr, len, max_width, &gcpres,
                             GCP_MAXEXTENT); //|GCP_USEKERNING
  if (!res) {
    // ERROR
    widths[0] = 0;
  ...
  for (; wsum < max_width && nchars < len && nchars < gcpres.nMaxFit;
       nchars++) {
    bflags = 0;
    ch = text[nchars];
...
void LVWin32DrawFont::DrawTextString(LVDrawBuf *buf, int x, int y,
  ...
  if (buf->GetBitsPerPixel() < 16) {
    ...
    SIZE sz;
    if (!GetTextExtentPoint32W(_drawbuf.GetDC(), str.c_str(), str.length(),
                               &sz))
      return;
    ...
    if (ExtTextOutW(dc, 0, 0,
                    0, // ETO_OPAQUE
                    NULL, str.c_str(), str.length(), NULL)) {
...
int LVWin32Font::GetGlyphIndex(HDC hdc, wchar_t code) {
  ...
  DWORD res = GetCharacterPlacementW(hdc, s, 1, 1000, &gcp, 0);
  if (!res)
    return 0;
  return g[0];
}
...
glyph_t *LVWin32Font::GetGlyphRec(lChar16 ch) {
  ...
  lUInt32 res;
  res = GetGlyphOutlineW(_drawbuf.GetDC(), ch, GGO_METRICS, &metrics, 0, NULL,
                         &identity);
  if (res == GDI_ERROR)
    return false;
...
/** \brief measure text
    ...
    \return true if glyph was found
*/
lUInt16 LVWin32Font::measureText(const lChar16 *text, int len, lUInt16 *widths,
                                 lUInt8 *flags, int max_width, lChar16 def_char,
                                 int letter_spacing, bool allow_hyphenation) {
  if (_hfont == NULL)
    return 0;
  ...
  HyphMan::hyphenate(text + hwStart, hwEnd - hwStart, widths + hwStart,
                     flags + hwStart, hyphwidth, max_width);
...
bool LVWin32Font::Create(int size, int weight, bool italic,
                         css_font_family_t family, lString8 typeface) {
...
bool operator==(const LVFont &r1, const LVFont &r2) {
  if (&r1 == &r2)
    return true;
  return r1.getSize() == r2.getSize() && r1.getWeight() == r2.getWeight() &&
