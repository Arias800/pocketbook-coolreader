...
static lString16 picasToPercent(const lChar16 *prop, int p, int minvalue,
                                int maxvalue) {
...
static lString16 picasToPx(const lChar16 *prop, int p, int minvalue,
                           int maxvalue) {
  int v = 600 * p / 5000;
  if (v > maxvalue)
    v = maxvalue;
...
static lString16 fontSizeToPercent(const lChar16 *prop, int p, int minvalue,
                                   int maxvalue) {
  int v = 100 * p / 20;
  if (v > maxvalue)
    v = maxvalue;
...
static void setOptions() {
  options_type tOptions = {
      DEFAULT_SCREEN_WIDTH, conversion_xml, TRUE,    TRUE,          FALSE,
      encoding_utf_8,       INT_MAX,        INT_MAX, level_default,
  };
 ...
 */
static void vPrologue1(diagram_type *pDiag, const char *szTask,
 ...
 */
static void vEpilogue(diagram_type *pDiag) {
 ...
 */
diagram_type *pCreateDiagram(const char *szTask, const char *szFilename) {
 ...
 */
void vSubstring2Diagram(diagram_type *pDiag, char *szString,
                        size_t tStringLength, long lStringWidth,
                        UCHAR ucFontColor, USHORT usFontstyle,
...
void vStoreStyle(diagram_type *pDiag, output_type *pOutput,
  ...
  sLeftIndent1 =
      pStyle->sLeftIndent1; /* First line left indentation in twips */
  sRightIndent = pStyle->sRightIndent; /* Right indentation in twips */
  usBeforeIndent =
      pStyle->usBeforeIndent; /* Vertical indent before paragraph in twips */
  usAfterIndent =
      pStyle->usAfterIndent; /* Vertical indent after paragraph in twips */

  TRACE("antiword::vStoreStyle(al=%d, li1=%d, li=%d, ri=%d)", alignment,
        sLeftIndent1, sLeftIndent, sRightIndent);
 ...
 */
void vEndOfParagraph(diagram_type *pDiag, drawfile_fontref tFontRef,
 ...
 */
BOOL bAddTableRow(diagram_type *pDiag, char **aszColTxt, int iNbrOfColumns,
                  const short *asColumnWidth, UCHAR ucBorderInfo) {
  ...
  if (table_col_count != iNbrOfColumns) {
    ...
    if (totalWidth > 0) {
      for (i = 0; i < iNbrOfColumns; i++) {
        ...
        if (cw >= 0)
          writer->OnAttribute(NULL, L"width",
                              (lString16::itoa(cw) + "%").c_str());
        writer->OnTagBody();
...
public:
  AntiwordStreamGuard(LVStreamRef stream) { antiword_stream = stream.get(); }
  ~AntiwordStreamGuard() { antiword_stream = NULL; }
  operator FILE *() { return (FILE *)antiword_stream; }
};
...
bool ImportWordDocument(LVStreamRef stream, ldomDocument *m_doc,
...
#define SAVE_COPY_OF_LOADED_DOCUMENT 1 // def _DEBUG
  ...
  if (bResult != 0) {
    ...
    LVStreamRef ostream =
        LVOpenFileStream("/tmp/test_save_source.xml", LVOM_WRITE);
    if (!ostream.isNull())
