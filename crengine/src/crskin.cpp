...
/// encodes percent value*100 (0..10000), to store in skin, from string like
/// "75%" or "10"
...
CRPageSkin::CRPageSkin()
    : _scrollSkin(new CRRectSkin()), _leftPageSkin(new CRRectSkin()),
      _rightPageSkin(new CRRectSkin()), _singlePageSkin(new CRRectSkin()),
      _name("Default") {}
...
CRIconSkin::CRIconSkin()
    : _bgcolor(0xFF000000) // transparent
      ,
      _hTransform(IMG_TRANSFORM_SPLIT), _vTransform(IMG_TRANSFORM_SPLIT),
      _splitPoint(-1, -1), _pos(0, 0),
...
void CRIconSkin::draw(LVDrawBuf &buf, const lvRect &rc) {
  ...
  if (_image.isNull()) {
    ...
    LVImageSourceRef img = LVCreateStretchFilledTransform(
        _image, rc2.width(), rc2.height(), _hTransform, _vTransform,
        _splitPoint.x, _splitPoint.y);
...
/// retuns path to base definition, if attribute base="#nodeid" is specified for
/// element of path
...
public:
  ...
  virtual ldomXPointer getXPointer(const lString16 &xPointerStr) {
    return _doc->createXPointer(xPointerStr);
  }
  /// returns read only file stream from skin
  virtual LVStreamRef getStream(const lChar16 *fname) {
    return _container->OpenStream(fname, LVOM_READ);
  }
  /// garbage collection
  virtual void gc() { _imageCache.clear(); }
  /// constructor does nothing
  CRSkinImpl()
      : _imageCache(8), _rectCache(8), _scrollCache(1), _windowCache(8),
        _menuCache(8), _pageCache(8), _toolbarCache(2), _iconCache(8) {}
  virtual ~CRSkinImpl() {}
  // open from container
...
LVImageSourceRef CRSkinImpl::getImage(const lChar16 *filename) {
  ...
  if (!standard && !!_container) {
    ...
    if (!!stream) {
      ...
      res = LVCreateUnpackedImageSource(
          res, MAX_SKIN_IMAGE_CACHE_ITEM_UNPACKED_SIZE, COLOR_BACKBUFFER == 0);
    }
  }
...
/// reads string value from attrname attribute of element specified by path,
/// returns empty string if not found
lString16 CRSkinContainer::readString(const lChar16 *path,
                                      const lChar16 *attrname, bool *res) {
...
/// reads string value from attrname attribute of element specified by path,
/// returns defValue if not found
lString16 CRSkinContainer::readString(const lChar16 *path,
                                      const lChar16 *attrname,
                                      const lString16 &defValue, bool *res) {
...
/// reads color value from attrname attribute of element specified by path,
/// returns defValue if not found
lUInt32 CRSkinContainer::readColor(const lChar16 *path, const lChar16 *attrname,
                                   lUInt32 defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
  css_length_t cv;
...
/// reads rect value from attrname attribute of element specified by path,
/// returns defValue if not found
lvRect CRSkinContainer::readRect(const lChar16 *path, const lChar16 *attrname,
                                 lvRect defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
  lvRect p = defValue;
...
/// reads boolean value from attrname attribute of element specified by path,
/// returns defValue if not found
bool CRSkinContainer::readBool(const lChar16 *path, const lChar16 *attrname,
                               bool defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
...
/// reads image transform value from attrname attribute of element specified by
/// path, returns defValue if not found
ImageTransform CRSkinContainer::readTransform(const lChar16 *path,
                                              const lChar16 *attrname,
                                              ImageTransform defValue,
                                              bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
...
/// reads h align value from attrname attribute of element specified by path,
/// returns defValue if not found
int CRSkinContainer::readHAlign(const lChar16 *path, const lChar16 *attrname,
                                int defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
  if (value == "left") {
...
/// reads h align value from attrname attribute of element specified by path,
/// returns defValue if not found
int CRSkinContainer::readVAlign(const lChar16 *path, const lChar16 *attrname,
                                int defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
...
/// reads int value from attrname attribute of element specified by path,
/// returns defValue if not found
int CRSkinContainer::readInt(const lChar16 *path, const lChar16 *attrname,
                             int defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
...
/// reads point(size) value from attrname attribute of element specified by
/// path, returns defValue if not found
lvPoint CRSkinContainer::readSize(const lChar16 *path, const lChar16 *attrname,
                                  lvPoint defValue, bool *res) {
  lString16 value = readString(path, attrname);
  if (value.empty())
    return defValue;
  lvPoint p = defValue;
...
/// reads rect value from attrname attribute of element specified by path,
/// returns null ref if not found
LVImageSourceRef CRSkinContainer::readImage(const lChar16 *path,
                                            const lChar16 *attrname, bool *r) {
  lString16 value = readString(path, attrname);
  if (value.empty()) {
#ifdef TRACE_SKIN_ERRORS
    crtrace log;
    log << "CRSkinContainer::readImage( " << path << ", " << attrname
...
#ifdef TRACE_SKIN_ERRORS
    crtrace log;
    log << "CRSkinContainer::readIcons( " << path
        << ") - cannot read icon from specified path";
#endif
...
CRSkinRef LVOpenSkin(const lString16 &pathname) {
  ...
  if (!container) {
    ...
    if (stream.isNull()) {
      CRLog::error(
          "cannot open skin: specified archive or directory not found");
      return CRSkinRef();
    }
...
// default parameters
// LVFontRef CRSkinnedItem::getFont() { return fontMan->GetFont( 24, 300, false,
// css_ff_sans_serif, cs8("Arial")) }
...
lvPoint CRWindowSkin::getWindowSize(const lvPoint &clientSize) {
  ...
  lvPoint tsz = getTitleSize();
  return lvPoint(clientSize.x + borders.left + borders.right + tsz.x,
                 clientSize.y + borders.top + borders.bottom + tsz.y);
}

CRSkinnedItem::CRSkinnedItem()
    : _textcolor(0x000000), _fontFace("Arial"), _fontSize(24), _fontBold(false),
...
LVFontRef CRSkinnedItem::getFont() {
  if (_font.isNull()) {
    _font = fontMan->GetFont(_fontSize, _fontBold ? 700 : 400, _fontItalic,
                             css_ff_sans_serif, UnicodeToUtf8(_fontFace));
  }
  return _font;
}
...
static void wrapLine(lString16Collection &dst, lString16 stringToSplit,
                     int maxWidth, LVFontRef font) {
...
void CRSkinnedItem::drawText(LVDrawBuf &buf, const lvRect &rc, lString16 text,
                             LVFontRef font, lUInt32 textColor, lUInt32 bgColor,
                             int flags) {
  SAVE_DRAW_STATE(buf);
  if (font.isNull())
    font = getFont();
  ...
  int ttw = tabText.empty()
                ? 0
                : font->getTextWidth(tabText.c_str(), tabText.length());
  ...
  for (int i = 0; i < lines.length(); i++) {
    ...
    font->DrawTextString(&buf, x, y, s.c_str(), s.length(), L'?', NULL, false,
                         0);
    if (!tabText.empty()) {
      font->DrawTextString(&buf, txtrc.right - ttw, y, tabText.c_str(),
                           tabText.length(), L'?', NULL, false, 0);
      tabText.clear();
    }
    y = y + lh;
  }
...
void CRButtonSkin::drawButton(LVDrawBuf &buf, const lvRect &rect, int flags) {
  ...
  if (!btnImage.isNull()) {
    LVImageSourceRef img =
        LVCreateStretchFilledTransform(btnImage, rc.width(), rc.height());
...
void CRScrollSkin::drawGauge(LVDrawBuf &buf, const lvRect &rect, int percent) {
  ...
  if (!bodyImg.isNull()) {
    LVImageSourceRef img = LVCreateStretchFilledTransform(
        bodyImg, bodyRect.width(), bodyRect.height());
    buf.Draw(img, bodyRect.left, bodyRect.top, bodyRect.width(),
             bodyRect.height(), false);
  }
  if (!sliderImg.isNull()) {
    LVImageSourceRef img = LVCreateStretchFilledTransform(
        sliderImg, sliderRect.width(), sliderRect.height());
    buf.Draw(img, sliderRect.left, sliderRect.top, sliderRect.width(),
             sliderRect.height(), false);
  }
}

void CRToolBarSkin::drawToolBar(LVDrawBuf &buf, const lvRect &rect,
                                bool enabled, int selectedButton) {
  draw(buf, rect);
  lvRect rc = rect;
  rc.shrinkBy(_margins);
  int width = 0;
...
CRWindowSkin::CRWindowSkin() { _fullscreen = false; }

CRMenuSkin::CRMenuSkin()
    : _minItemCount(-1), _maxItemCount(-1), _showShortcuts(true) {}
...
/*
    <item>
        ...
        <title>
            ...
            <text color="" face="" size="" bold="" italic="" valign=""
   halign=""/> <background image="filename" color=""/> <border
   widths="left,top,right,bottom"/> <icon image="filename" valign="" halign="">
        </title>
        <item>
            <size minvalue="x,y" maxvalue=""/>
            <text color="" face="" size="" bold="" italic="" valign=""
...
public:
  CRSimpleMenuSkin(CRSkinImpl *skin) {
    ...
    _itemSkin->setBackgroundImage(
        skin->getImage(L"std_menu_item_background.xpm"));
    _itemSkin->setBorderWidths(lvRect(8, 8, 8, 8));
    _itemShortcutSkin = CRRectSkinRef(new CRRectSkin());
    _itemShortcutSkin->setBackgroundImage(
        skin->getImage(L"std_menu_shortcut_background.xpm"));
...
bool CRSkinContainer::readButtonSkin(const lChar16 *path, CRButtonSkin *res) {
  ...
  if (!img.isNull()) {
    flg = true;
    res->setMinSize(lvPoint(margins.left + margins.right + img->GetWidth(),
                            margins.top + margins.bottom + img->GetHeight()));
  }
...
CRScrollSkin::CRScrollSkin()
    : _autohide(false), _showPageNumbers(true),
...
bool CRSkinContainer::readScrollSkin(const lChar16 *path, CRScrollSkin *res) {
  ...
  res->setShowPageNumbers(
      readBool((p).c_str(), L"show-page-numbers", res->getShowPageNumbers()));
...
bool CRSkinContainer::readIconSkin(const lChar16 *path, CRIconSkin *res) {
  ...
  res->setHTransform(
      readTransform(path, L"htransform", res->getHTransform(), &flg));
  res->setVTransform(
      readTransform(path, L"vtransform", res->getVTransform(), &flg));
...
bool CRSkinContainer::readRectSkin(const lChar16 *path, CRRectSkin *res) {
  ...
  // res->setBackgroundColor( readColor( bgpath.c_str(), L"color",
  // res->getBackgroundColor(), &flg ) );
  res->setBorderWidths(
      readRect(borderpath.c_str(), L"widths", res->getBorderWidths(), &flg));
  res->setMinSize(
      readSize(sizepath.c_str(), L"minvalue", res->getMinSize(), &flg));
  res->setMaxSize(
      readSize(sizepath.c_str(), L"maxvalue", res->getMaxSize(), &flg));
  res->setFontFace(
      readString(textpath.c_str(), L"face", res->getFontFace(), &flg));
  res->setTextColor(
      readColor(textpath.c_str(), L"color", res->getTextColor(), &flg));
  res->setFontBold(
      readBool(textpath.c_str(), L"bold", res->getFontBold(), &flg));
  res->setWordWrap(
      readBool(textpath.c_str(), L"wordwrap", res->getWordWrap(), &flg));
  res->setFontItalic(
      readBool(textpath.c_str(), L"italic", res->getFontItalic(), &flg));
  res->setFontSize(
      readInt(textpath.c_str(), L"size", res->getFontSize(), &flg));
  res->setTextHAlign(
      readHAlign(textpath.c_str(), L"halign", res->getTextHAlign(), &flg));
  res->setTextVAlign(
      readVAlign(textpath.c_str(), L"valign", res->getTextVAlign(), &flg));
...
lvPoint CRWindowSkin::getTitleSize() {
  lvPoint minsize =
      _titleSkin.isNull() ? lvPoint(0, 0) : _titleSkin->getMinSize();
  return minsize;
}
...
bool CRSkinContainer::readPageSkin(const lChar16 *path, CRPageSkin *res) {
  ...
  lString16 name = ptr.getNode()->getAttributeValue(
      ptr.getNode()->getDocument()->getAttrNameIndex("name"));
  if (!name.empty())
    res->setName(name);

  flg = readRectSkin((p + "scroll-skin").c_str(),
                     res->getSkin(PAGE_SKIN_SCROLL).get()) ||
        flg;
  flg = readRectSkin((p + "left-page-skin").c_str(),
                     res->getSkin(PAGE_SKIN_LEFT_PAGE).get()) ||
        flg;
  flg = readRectSkin((p + "right-page-skin").c_str(),
                     res->getSkin(PAGE_SKIN_RIGHT_PAGE).get()) ||
        flg;
  flg = readRectSkin((p + "single-page-skin").c_str(),
                     res->getSkin(PAGE_SKIN_SINGLE_PAGE).get()) ||
        flg;
...
bool CRSkinContainer::readMenuSkin(const lChar16 *path, CRMenuSkin *res) {
  ...
  b = readRectSkin((p + "/selshortcut-even").c_str(),
                   evenshortcutSelSkin.get());
  flg = flg || b;
  if (b)
    res->setEvenSelItemShortcutSkin(evenshortcutSelSkin);

  res->setMinItemCount(
      readInt(path, L"min-item-count", res->getMinItemCount()));
  res->setMaxItemCount(
      readInt(path, L"max-item-count", res->getMaxItemCount()));
  res->setShowShortcuts(
      readBool(path, L"show-shortcuts", res->getShowShortcuts()));
...
#ifdef TRACE_SKIN_ERRORS
    crtrace log;
    log << "CRSkinContainer::readButtons( " << path
        << ") - cannot read button from specified path";
#endif
    if (res)
      *res = false;
...
CRSkinRef CRSkinListItem::getSkin() { return LVOpenSkin(getFileName()); }
