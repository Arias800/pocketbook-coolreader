...
/// set container to handle filesystem access for paths started with
/// ASSET_PATH_PREFIX (@ sign)
...
const lChar16 *LVStorageObject::GetName() { return NULL; }

LVContainer *LVStorageObject::GetParentContainer() { return NULL; }

void LVStorageObject::SetName(const lChar16 *) {}

bool LVStorageObject::IsContainer() { return false; }
...
public:
  static LVStreamBufferRef create(LVStreamRef stream, lvpos_t pos,
                                  lvsize_t size, bool readonly) {
    ...
    if (stream->Read(buf->m_buf, size, &bytesRead) != LVERR_OK ||
        bytesRead != size) {
      delete buf;
      return res;
    }
  ...
  LVDefStreamBuffer(LVStreamRef stream, lvpos_t pos, lvsize_t size,
                    bool readonly)
      : m_stream(stream), m_buf(NULL), m_pos(pos), m_size(size),
        m_readonly(readonly) {
  ...
  virtual const lUInt8 *getReadOnly() { return m_writeonly ? NULL : m_buf; }
  /// get pointer to read-write buffer, returns NULL if unavailable
  virtual lUInt8 *getReadWrite() { return m_readonly ? NULL : m_buf; }
    ...
    if (m_buf) {
      if (!m_readonly) {
        if (m_stream->SetPos(m_pos) != LVERR_OK) {
          ...
          if (m_stream->Write(m_buf, m_size, &bytesWritten) != LVERR_OK ||
              bytesWritten != m_size) {
            res = false;
          }
        }
  ...
  /// flush on destroy
  virtual ~LVDefStreamBuffer() { close(); }
};
...
private:
  ...
  public:
    ...
    /// get pointer to read-only buffer, returns NULL if unavailable
    virtual const lUInt8 *getReadOnly() { return m_buf; }
...
public:
  ...
  virtual LVStreamBufferRef GetReadBuffer(lvpos_t pos, lvpos_t size) {
    ...
    if (!m_map)
      return res;
    if ((m_mode != LVOM_APPEND && m_mode != LVOM_READ) || pos + size > m_size ||
        size == 0)
      return res;
    return LVStreamBufferRef(
        new LVBuffer(LVStreamRef(this), m_map + pos, size, true));
  }
  ...
  virtual LVStreamBufferRef GetWriteBuffer(lvpos_t pos, lvpos_t size) {
    ...
    if (m_mode != LVOM_APPEND || pos + size > m_size || size == 0)
      return res;
    return LVStreamBufferRef(
        new LVBuffer(LVStreamRef(this), m_map + pos, size, false));
  }
  ...
  */
  ...
  static LVFileMappedStream *CreateFileStream(lString16 fname,
                                              lvopen_mode_t mode, int minSize) {
  ...
  lverror_t Map() {
    ...
    m_map = (lUInt8 *)MapViewOfFile(
        m_hMap,
        m_mode == LVOM_READ ? FILE_MAP_READ : FILE_MAP_READ | FILE_MAP_WRITE, 0,
        0, m_size);
    if (m_map == NULL) {
      CRLog::error("LVFileMappedStream::Map() -- Cannot map file to memory");
      return error();
  ...
  lverror_t OpenFile(lString16 fname, lvopen_mode_t mode,
    ...
    m_hFile =
        CreateFileW(fname.c_str(), m, s, NULL, c, FILE_ATTRIBUTE_NORMAL, NULL);
    if (m_hFile == INVALID_HANDLE_VALUE || !m_hFile) {
      // unicode not implemented?
      lUInt32 err = GetLastError();
      if (err == ERROR_CALL_NOT_IMPLEMENTED)
        m_hFile = CreateFileA(fn8.c_str(), m, s, NULL, c, FILE_ATTRIBUTE_NORMAL,
                              NULL);
      if ((m_hFile == INVALID_HANDLE_VALUE) || (!m_hFile)) {
...
*/
LVStreamRef LVMapFileStream(const lChar8 *pathname, lvopen_mode_t mode,
                            lvsize_t minSize) {
...
public:
  virtual lverror_t Seek(lvoffset_t offset, lvseek_origin_t origin,
    ...
    CRLog::error("error setting file position to %d (%d)", (int)offset,
                 (int)origin);
    return LVERR_FAIL;
  }
  ...
  /// flushes unsaved data from buffers to file, with optional flush of OS
  /// buffers
  ...
  virtual bool Eof() { return feof(m_file) != 0; }
  ...
  LVFileStream() : m_file(NULL) { m_mode = LVOM_ERROR; }
...
public:
  ...
  virtual bool Eof() { return m_size <= m_pos; }
  ...
  virtual lverror_t Write(const void *buf, lvsize_t count,
    ...
    if (WriteFile(m_hFile, buf, (lUInt32)count, (LPDWORD)&dwBytesWritten,
                  NULL)) {
      if (nBytesWritten)
  ...
  lverror_t OpenFile(lString16 fname, int mode) {
    ...
    if (m_hFile == INVALID_HANDLE_VALUE || !m_hFile) {
      ...
      lUInt32 err = GetLastError();
      if (err == ERROR_CALL_NOT_IMPLEMENTED)
        m_hFile = CreateFileA(UnicodeToLocal(fname).c_str(), m, s, NULL, c,
    ...
    int flags = (mode == LVOM_READ)
                    ? O_RDONLY
                    : O_RDWR | O_CREAT | (use_sync ? O_SYNC : 0) |
                          (mode == LVOM_WRITE ? O_TRUNC : 0);
    lString8 fn8 = UnicodeToUtf8(fname);
    m_fd = open(fn8.c_str(), flags, (mode_t)0666);
    if (m_fd == -1) {
#ifndef ANDROID
      CRLog::error("Error opening file %s for %s", fn8.c_str(),
                   (mode == LVOM_READ) ? "reading" : "read/write");
      // CRLog::error( "Error opening file %s for %s, errno=%d, msg=%s",
      // fn8.c_str(), (mode==LVOM_READ) ? "reading" : "read/write",  (int)errno,
      // strerror(errno) );
#endif
      return LVERR_FAIL;
    }
...
/// tries to split full path name into archive name and file name inside archive
/// using separator "@/" or "@\"
bool LVSplitArcName(lString16 fullPathName, lString16 &arcPathName,
...
/// tries to split full path name into archive name and file name inside archive
/// using separator "@/" or "@\"
bool LVSplitArcName(lString8 fullPathName, lString8 &arcPathName,
                    lString8 &arcItemPathName) {
  int p = fullPathName.pos("@/");
  if (p < 0)
    p = fullPathName.pos("@\\");
  if (p < 0)
...
lvopen_mode_t LVTextStream::GetMode() { return m_base_stream->GetMode(); }

lverror_t LVTextStream::Seek(lvoffset_t offset, lvseek_origin_t origin,
                             lvpos_t *pNewPos) {
...
lvpos_t LVTextStream::SetPos(lvpos_t p) { return m_base_stream->SetPos(p); }

lvpos_t LVTextStream::GetPos() { return m_base_stream->GetPos(); }
...
lverror_t LVTextStream::Write(const void *buf, lvsize_t count,
                              lvsize_t *nBytesWritten) {
  return m_base_stream->Write(buf, count, nBytesWritten);
}

bool LVTextStream::Eof() { return m_base_stream->Eof(); }
...
public:
  ...
  virtual LVContainer *GetParentContainer() { return (LVContainer *)m_parent; }
  virtual const LVContainerItemInfo *GetObjectInfo(int index) {
    if (index >= 0 && index < m_list.length())
      return m_list[index];
    return NULL;
  }
  virtual int GetObjectCount() const { return m_list.length(); }
  virtual lverror_t GetSize(lvsize_t *pSize) {
    if (m_fname.empty())
      return LVERR_FAIL;
  ...
  static LVDirectoryContainer *OpenDirectory(const wchar_t *path,
      ...
      for (;;) {
        ...
        if ((dwAttrs & FILE_ATTRIBUTE_DIRECTORY)) {
          ...
          if (!lStr_cmp(pfn, L"..") || !lStr_cmp(pfn, L".")) {
            ...
            // normal directory
            LVDirectoryContainerItemInfo *item =
...
#define CACHE_BUF_BLOCK_SIZE (1 << CACHE_BUF_BLOCK_SHIFT)
  ...
  bool fillItem(BufItem *item) {
    ...
    if (m_stream->SetPos(item->start) != (lvpos_t)item->start)
      return false;
    // int streamSize=m_stream->GetSize(); int bytesLeft = m_stream->GetSize() -
    // m_stream->GetPos();
    lvsize_t bytesRead = 0;
    if (m_stream->Read(item->buf, item->size, &bytesRead) != LVERR_OK ||
        bytesRead != item->size)
      return false;
    return true;
  }
...
public:
  LVCachedStream(LVStreamRef stream, int bufSize)
      : m_stream(stream), m_pos(0), m_head(NULL), m_tail(NULL), m_bufLen(0) {
    m_size = m_stream->GetSize();
    m_bufItems =
        (int)((m_size + CACHE_BUF_BLOCK_SIZE - 1) >> CACHE_BUF_BLOCK_SHIFT);
    if (!m_bufItems)
      m_bufItems = 1;
  ...
  virtual lverror_t getcrc32(lUInt32 &dst) { return m_stream->getcrc32(dst); }
...
struct ZipHd2 {
  ...
  lUInt32 getAttr() {
    return _Attr_and_Offset[1] | ((lUInt32)_Attr_and_Offset[2] << 16);
  }
  lUInt32 getOffset() {
    return _Attr_and_Offset[3] | ((lUInt32)_Attr_and_Offset[4] << 16);
  }
  void byteOrderConv() {
    //
    lvByteOrderConv cnv;
...
private:
  ...
  LVZipDecodeStream(LVStreamRef stream, lvsize_t start, lvsize_t packsize,
      ...
      : m_stream(stream), m_start(start), m_packsize(packsize),
        m_unpacksize(unpacksize), m_inbytesleft(0), m_outbytesleft(0),
        m_zInitialized(false), m_decodedpos(0), m_inbuf(NULL), m_outbuf(NULL),
        m_CRC(0), m_originalCRC(crc) {
  ...
  int fillInBuf() {
    if (m_zstream.avail_in < ARC_INBUF_SIZE / 4 && m_inbytesleft > 0) {
      ...
      if (bytes_to_read > 0) {
        lvsize_t bytesRead = 0;
        if (m_stream->Read(m_inbuf + tailpos, bytes_to_read, &bytesRead) !=
            LVERR_OK) {
          // read error
        ...
        if (m_CRC != m_originalCRC) {
          CRLog::error("ZIP stream '%s': CRC doesn't match",
                       LCSTR(lString16(GetName())));
          return -1; // CRC error
        }
      }
    }
  ...
  /// decode next portion of data, returns number of decoded bytes available, -1
  /// if error
  int decodeNext() {
    ...
    if (m_decodedpos > ARC_OUTBUF_SIZE / 2 ||
        (m_zstream.avail_out < ARC_OUTBUF_SIZE / 4 && m_outbytesleft > 0)) {

      int outpos = (int)(m_zstream.next_out - m_outbuf);
      if (m_decodedpos > ARC_OUTBUF_SIZE / 2 ||
          outpos > ARC_OUTBUF_SIZE * 2 / 4 || m_zstream.avail_out == 0 ||
          m_inbytesleft == 0) {
        // move rest of data to beginning of buffer
    ...
    int res = inflate(&m_zstream,
                      m_inbytesleft > 0
                          ? Z_NO_FLUSH
                          : Z_FINISH); // m_inbytesleft | m_zstream.avail_in
...
public:
  ...
  virtual lvsize_t GetSize() { return m_unpacksize; }
  virtual lvpos_t GetPos() { return m_unpacksize - m_outbytesleft; }
  ...
  virtual lverror_t Read(void *buf, lvsize_t count, lvsize_t *bytesRead) {
    ...
    if (readBytes != (int)count) {
      CRLog::trace("ZIP stream: %d bytes read instead of %d", (int)readBytes,
                   (int)count);
    }
    if (bytesRead)
      *bytesRead = (lvsize_t)readBytes;
    // CRLog::trace("%d bytes requested, %d bytes read, %d bytes left", count,
    // readBytes, m_outbytesleft);
    return LVERR_OK;
  }
  virtual lverror_t SetSize(lvsize_t) { return LVERR_NOTIMPL; }
  static LVStream *Create(LVStreamRef stream, lvpos_t pos, lString16 name,
                          lUInt32 srcPackSize, lUInt32 srcUnpSize) {
    ZipLocalFileHdr hdr;
    ...
    if (hdr.getMethod() == 0) {
      ...
      if (hdr.getPackSize() != hdr.getUnpSize())
        return NULL;
      LVStreamFragment *fragment =
          new LVStreamFragment(stream, pos, hdr.getPackSize());
      fragment->SetName(name.c_str());
      return fragment;
    } else if (hdr.getMethod() == 8) {
      // deflate
      LVStreamRef srcStream(
          new LVStreamFragment(stream, pos, hdr.getPackSize()));
      LVZipDecodeStream *res = new LVZipDecodeStream(srcStream, pos, packSize,
                                                     unpSize, hdr.getCRC());
...
public:
  virtual LVStreamRef OpenStream(const wchar_t *fname, lvopen_mode_t /*mode*/) {
    ...
    LVStreamRef stream(LVZipDecodeStream::Create(
        strm, m_list[found_index]->GetSrcPos(), fn,
        m_list[found_index]->GetSrcSize(), m_list[found_index]->GetSize()));
    if (!stream.isNull()) {
  ...
  virtual int ReadContents() {
    ...
    for (;;) {
      ...
      if (ZipHeader.NameLen > max_NM) {
        CRLog::error("ZIP entry name length is too big: %d",
                     (int)ZipHeader.NameLen);
        return 0;
      }
      lUInt32 SizeToRead =
          (ZipHeader.NameLen < max_NM) ? ZipHeader.NameLen : max_NM;
      char fnbuf[max_NM + 1];
      ...
      item->SetItemInfo(fName.c_str(), ZipHeader.UnpSize,
                        (ZipHeader.getAttr() & 0x3f));
      item->SetSrc(ZipHeader.getOffset(), ZipHeader.PackSize, ZipHeader.Method);

// #define DUMP_ZIP_HEADERS
#ifdef DUMP_ZIP_HEADERS
      CRLog::trace(
          "ZIP entry '%s' unpSz=%d, pSz=%d, m=%x, offs=%x, zAttr=%x, flg=%x",
          LCSTR(fName), (int)ZipHeader.UnpSize, (int)ZipHeader.PackSize,
          (int)ZipHeader.Method, (int)ZipHeader.getOffset(),
          (int)ZipHeader.getZIPAttr(), (int)ZipHeader.getAttr());
      //, addL=%d, commL=%d, dn=%d
  ...
  static LVArcContainerBase *OpenArchieve(LVStreamRef stream) {
    ...
    stream->SetPos(0);
    lvsize_t bytesRead = 0;
    if (stream->Read(hdr, hdrSize, &bytesRead) != LVERR_OK ||
        bytesRead != hdrSize)
      return NULL;
    stream->SetPos(0);
    // detect arc type
...
public:
  ...
  */
  virtual bool Eof() { return m_pos >= m_size; }
  virtual lvopen_mode_t GetMode() { return m_mode; }
...
LVStreamRef LVCreateMemoryStream(void *buf, int bufSize, bool createCopy,
                                 lvopen_mode_t mode) {
  LVMemoryStream *stream = new LVMemoryStream();
  if (!buf)
    stream->Create();
...
#define TCR_READ_BUF_SIZE 4096
  ...
  LVTCRStream(LVStreamRef stream)
      : _stream(stream), _index(NULL), _decoded(NULL), _decodedSize(0),
  ...
  bool decodePart(unsigned index) {
    ...
    if ((index + 1) * TCR_READ_BUF_SIZE > _packedSize)
      bytesToRead =
          TCR_READ_BUF_SIZE - ((index + 1) * TCR_READ_BUF_SIZE - _packedSize);
    if (bytesToRead <= 0 || bytesToRead > TCR_READ_BUF_SIZE)
      return false;
    if (_stream->SetPos(_packedStart + index * TCR_READ_BUF_SIZE) ==
        (lvpos_t)(~0))
      return false;
...
public:
  ...
  bool init() {
    ...
    for (int i = 0; i < 256; i++) {
      ...
      if (sz &&
          (_stream->Read(buf, sz, &bytesRead) != LVERR_OK || bytesRead != sz))
        return false;
      _codes[i].set(buf, sz);
    }
  ...
  */
  ...
  /**
      \return lvsize_t file size
  */
  virtual lvsize_t GetSize() { return _unpSize; }
  ...
  /**
      ...
      \return lverror_t status: LVERR_OK if success
  */
  virtual lverror_t SetSize(lvsize_t) { return LVERR_FAIL; }
...
void LVAppendPathDelimiter(lString16 &pathName) {
  if (pathName.empty() ||
      (pathName.length() == 1 && pathName[0] == ASSET_PATH_PREFIX))
    return;
...
lString16 LVCombinePaths(lString16 basePath, lString16 newPath) {
  if (newPath[0] == '/' || newPath[0] == '\\' ||
      (newPath.length() > 0 && newPath[1] == ':' && newPath[2] == '\\'))
  ...
  do {
    ...
    for (int i = 0; i < (int)(s.length() - pattern.length()); i++) {
      ...
      else if (s[i] == separator && s[i + 1] == '.' && s[i + 2] == '.' &&
               s[i + 3] == separator) {
...
lString16 LVExtractLastPathElement(lString16 &pathName) {
  ...
  lString16 res = pathName.substr(last_delim_pos + 1,
                                  pathName.length() - last_delim_pos - 1);
...
/// returns full path to file identified by pathName, with base directory ==
/// basePath
...
bool LVDirectoryIsWritable(const lString16 &pathName) {
  ...
  bool created = false;
  {
    ...
    if (!stream.isNull()) {
      created = true;
      lvsize_t bytesWritten = 0;
      if (stream->Write("TEST", 4, &bytesWritten) == LVERR_OK &&
          bytesWritten == 4) {
        res = true;
      }
    }
...
bool LVCreateDirectory(lString16 path) {
  ...
  if (dir.isNull()) {
    ...
    if (!LVCreateDirectory(basedir)) {
      CRLog::error("Failed to create directory %s",
...
/**
    ...
    \return reference to opened stream if success, NULL if error
*/
LVStreamRef LVMapFileStream(const lChar16 *pathname, lvopen_mode_t mode,
                            lvsize_t minSize) {
...
bool LVRenameFile(lString8 oldname, lString8 newname) {
  ...
  bool res = MoveFileW(Utf8ToUnicode(oldname).c_str(),
                       Utf8ToUnicode(newname).c_str()) != 0;
  if (!res) {
...
class LVBlockWriteStream : public LVNamedStream {
  ...
  struct Block {
    ...
    void save(const lUInt8 *ptr, lvpos_t pos, lvsize_t len) {
      ...
      CRLog::trace("block %x save %x, %x", (int)block_start, (int)pos,
                   (int)len);
#endif
      int offset = (int)(pos - block_start);
      if (offset > size || offset < 0 || (int)len > size ||
          offset + (int)len > size) {
  ...
  /// set write bytes limit to call flush(true) automatically after writing of
  /// each sz bytes
  ...
  lverror_t readBlock(Block *block) {
    ...
    block->block_end = end;
#if TRACE_BLOCK_WRITE_STREAM
    CRLog::trace("block %x filling from stream %x, %x", (int)block->block_start,
                 (int)block->block_start,
                 (int)(block->block_end - block->block_start));
#endif
    res = _baseStream->Read(block->buf, end - start, &bytesRead);
    if (res != LVERR_OK)
      CRLog::error("Error while reading block %x from file of size %x",
                   block->block_start, ssize);
    return res;
  }
  ...
  lverror_t writeBlock(Block *block) {
    if (block->modified_start < block->modified_end) {
#if TRACE_BLOCK_WRITE_STREAM
      CRLog::trace("WRITE BLOCK %x (%x, %x)", (int)block->block_start,
                   (int)block->modified_start,
                   (int)(block->modified_end - block->modified_start));
#endif
      ...
      lverror_t res = _baseStream->Write(
          block->buf + (block->modified_start - block->block_start),
          block->modified_end - block->modified_start, &bytesWritten);
      if (res == LVERR_OK) {
      ...
      if (item->containsPos(pos)) {
        if (item != _firstBlock) {
          ...
          *p = item->next;
          ...
          CRLog::trace("found block %x (%x, %x)", (int)item->block_start,
                       (int)item->modified_start,
                       (int)(item->modified_end - item->modified_start));
#endif
        }
        return item;
      }
  ...
  bool readFromCache(void *buf, lvpos_t pos, lvsize_t count) {
    ...
    if (p) {
#if TRACE_BLOCK_WRITE_STREAM
      CRLog::trace("read from cache block %x (%x, %x)", (int)p->block_start,
                   (int)pos, (int)(count));
#endif
  ...
  lverror_t writeToCache(const void *buf, lvpos_t pos, lvsize_t count) {
    ...
    if (p) {
#if TRACE_BLOCK_WRITE_STREAM
      CRLog::trace("saving data to existing block %x (%x, %x)",
                   (int)p->block_start, (int)pos, (int)count);
#endif
    ...
    if (_count >= _blockCount - 1) {
      ...
      for (Block *p = _firstBlock; p; p = p->next) {
        if (p->next && !p->next->next) {
          ...
          CRLog::trace("dropping block %x (%x, %x)", (int)p->next->block_start,
                       (int)p->next->modified_start,
                       (int)(p->next->modified_end - p->next->modified_start));
#endif
          writeBlock(p->next);
          delete p->next;
          _count--;
...
#if TRACE_BLOCK_WRITE_STREAM
    CRLog::trace("saving data to new block %x (%x, %x)", (int)p->block_start,
                 (int)pos, (int)count);
#endif
    p->save((const lUInt8 *)buf, pos, count);
    p->next = _firstBlock;
...
public:
  ...
  virtual ~LVBlockWriteStream() { Flush(true); }

  virtual const lChar16 *GetName() { return _baseStream->GetName(); }
  virtual lvopen_mode_t GetMode() { return _baseStream->GetMode(); }

  LVBlockWriteStream(LVStreamRef baseStream, int blockSize, int blockCount)
      : _baseStream(baseStream), _blockSize(blockSize), _blockCount(blockCount),
        _firstBlock(NULL), _count(0) {
    _pos = _baseStream->GetPos();
    _size = _baseStream->GetSize();
  }

  virtual lvpos_t GetSize() { return _size; }
  ...
  virtual lverror_t Seek(lvoffset_t offset, lvseek_origin_t origin,
    ...
    if (res == LVERR_OK) {
      ...
      CRLog::error("baseStream->Seek(%d,%x) failed: %d", (int)origin,
                   (int)offset, (int)res);
    }
    return res;
  }
...
#if TRACE_BLOCK_WRITE_STREAM
        CRLog::trace("direct reading from stream (%x, %x)", (int)_pos,
                     (int)blockSpaceLeft);
#endif
  ...
  virtual bool Eof() { return _pos >= _size; }
};

LVStreamRef LVCreateBlockWriteStream(LVStreamRef baseStream, int blockSize,
                                     int blockCount) {
