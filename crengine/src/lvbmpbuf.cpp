...
void lvdrawbufInit(draw_buf_t *buf, int bitsPerPixel, int width, int height,
                   lUInt8 *data) {
...
void lvdrawbufFillRect(draw_buf_t *buf, int x0, int y0, int x1, int y1,
                       unsigned char pixel) {
...
void lvdrawbufDraw2(draw_buf_t *buf, int x, int y, const lUInt8 *bitmap,
...
void lvdrawbufDrawUnpacked(draw_buf_t *buf, int x, int y, const lUInt8 *bitmap,
                           int width, int height) {
  int buf_width = buf->bytesPerRow << 2; /* 2bpp */
  int bx = 0;
  int by = 0;
...
void lvdrawbufDrawPacked(draw_buf_t *buf, int x, int y, const lUInt8 *bitmap,
                         int width, int height,
...
void lvdrawbufDraw(draw_buf_t *buf, int x, int y, const lUInt8 *bitmap,
                   int numRows, int bytesPerRow) {
...
*/
/*
void lvdrawbufDrawText( draw_buf_t * buf, int x, int y, const lvfont_handle
pfont, const lChar16 * text, int len, lChar16 def_char )
{
   ...
   while (len)
   {
      if (len==1 || *text != UNICODE_SOFT_HYPHEN_CODE)
      {
          ...
          if (glyph)
          {
              //lvdrawbufDraw( buf, x + glyph->originX, y + baseline -
glyph->originY, glyph->glyph, glyph->blackBoxY, glyph->rowBytes );
              lvdrawbufDrawPacked( buf, x + glyph->originX,
                  y + baseline - glyph->originY, glyph->glyph,
                  glyph->blackBoxX, glyph->blackBoxY, pDecodeTable );
              //lvdrawbufDraw2( buf, x + glyph->originX, y + baseline -
glyph->originY, glyph->glyph, glyph->blackBoxX, glyph->blackBoxY ); x +=
glyph->width;
          }
      }
  ...
  while (len) {
    if (len == 1 || *text != UNICODE_SOFT_HYPHEN_CODE) {
      ...
      if (!glyph)
        glyph = lvfontGetGlyph(pfont, def_char); /* substitute char */
      if (glyph) {
        lvfontUnpackGlyph(glyph->glyph, pDecodeTable, glyph_buf,
