...
LVRendPageContext::LVRendPageContext(LVRendPageList *pageList, int pageHeight)
    : callback(NULL), totalFinalBlocks(0), renderedFinalBlocks(0),
      lastPercent(-1), page_list(pageList), page_h(pageHeight), footNotes(64),
      curr_note(NULL) {
  if (callback) {
...
bool LVRendPageContext::updateRenderProgress(int numFinalBlocksRendered) {
  ...
  int percent =
      totalFinalBlocks > 0 ? renderedFinalBlocks * 100 / totalFinalBlocks : 0;
  if (percent < 0)
    percent = 0;
  if (percent > 100)
    percent = 100;
...
public:
  ...
  PageSplitState(LVRendPageList *pl, int pageHeight)
      : page_h(pageHeight), page_list(pl), pagestart(NULL), pageend(NULL),
        next(NULL), last(NULL), footheight(0), footnote(NULL), footstart(NULL),
        footend(NULL), footlast(NULL), lastpageend(0) {}
  ...
  void AddToList() {
    ...
    int h =
        (pagestart && pageend) ? pageend->getEnd() - pagestart->getStart() : 0;
#ifdef DEBUG_FOOTNOTES
    if (CRLog::isTraceEnabled()) {
      if (pagestart && pageend)
        CRLog::trace("AddToList(%d, %d) footnotes: %d  pageHeight=%d",
                     pagestart->start, pageend->start + pageend->height,
                     footnotes.length(), h);
      else
        CRLog::trace(
            "AddToList(Only footnote: %d) footnotes: %d  pageHeight=%d",
  ...
  void AddLine(LVRendLineInfo *line) {
    if (pagestart == NULL) {
      ...
      unsigned flgSplit =
          CalcSplitFlag(last->getSplitAfter(), line->getSplitBefore());
  ...
  void AddFootnoteFragmentToList() {
    ...
    if (footend == NULL)
      footend = footstart;
    // CRLog::trace("AddFootnoteFragmentToList(%d, %d)", footstart->start,
    // footend->end );
    int h =
        footend->getEnd() - footstart->getStart(); // currentFootnoteHeight();
    if (h > 0 && h < page_h) {
      footheight += h;
#ifdef DEBUG_FOOTNOTES
      CRLog::trace("AddFootnoteFragmentToList(%d, %d)", footstart->getStart(),
                   h);
#endif
...
void LVRendPageContext::split() {
  ...
  for (int lindex = 0; lindex < lineCount; lindex++) {
    ...
    if (line->getLinks()) {
      ...
      //     CRLog::trace("LVRendPageContext::split() line %d: found %d links",
      //     lindex, line->getLinks()->length() );
      // }
