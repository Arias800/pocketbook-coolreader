...
LVFileParserBase::LVFileParserBase(LVStreamRef stream)
    : m_stream(stream), m_buf(NULL), m_buf_size(0), m_stream_size(0),
      m_buf_len(0), m_buf_pos(0), m_buf_fpos(0), m_stopped(false),
      m_progressCallback(NULL), m_lastProgressTime((time_t)0),
      m_progressLastPercent(0), m_progressUpdateCounter(0),
...
void LVFileParserBase::updateProgress() {
  ...
  m_progressUpdateCounter =
      (m_progressUpdateCounter + 1) & PROGRESS_UPDATE_RATE_MASK;
...
LVTextFileBase::LVTextFileBase(LVStreamRef stream)
    : LVFileParserBase(stream), m_enc_type(ce_8bit_cp), m_conv_table(NULL),
      m_eof(false) {
  clearCharBuffer();
}
        ...
        /* Code set 1 (BIG5 extended) */
        {
          ...
          if ((ch2 >= 0x40 && ch2 < 0x7f) || (ch2 >= 0xa1 && ch2 < 0xff)) {
            if (ch >= 0xa1) {
              if (ch < 0xa3) {
                unsigned int i =
                    157 * (ch - 0xa1) + (ch2 - (ch2 >= 0xa1 ? 0x62 : 0x40));
              ...
              if (!((ch == 0xc6 && ch2 >= 0xa1) || ch == 0xc7)) {
                ...
                unsigned int i =
                    157 * (ch - 0xc6) + (ch2 - (ch2 >= 0xa1 ? 0x62 : 0x40));
                if (i < 133) {
                  /* 63 <= i < 133. */
...
bool LVTextFileBase::AutodetectEncoding(bool utfOnly) {
  ...
  m_stream->SetPos(oldpos);
  if (res) {
    // CRLog::debug("Code page decoding results: encoding=%s, lang=%s",
    // enc_name, lang_name);
...
bool LVFileParserBase::Seek(lvpos_t pos, int bytesToPrefetch) {
  ...
  unsigned bytesToRead =
      (bytesToPrefetch > m_buf_size) ? bytesToPrefetch : m_buf_size;
...
int LVTextFileBase::ReadTextBytes(lvpos_t pos, int bytesToRead, lChar16 *buf,
                                  int buf_size, int flags) {
  if (!Seek(pos, bytesToRead)) {
    CRLog::error("LVTextFileBase::ReadTextBytes seek error! cannot set pos to "
                 "%d to read %d bytes",
...
bool LVFileParserBase::FillBuffer(int bytesToRead) {
  ...
  //        CRLog::trace("fpos=%06x+%06x, sz=%04x, data: %02x %02x %02x %02x
...
#endif
#if JIS_ENCODING_SUPPORT == 1
  } else if (m_encoding_name == "shift-jis" || m_encoding_name == "shift_jis" ||
             m_encoding_name == "sjis" || m_encoding_name == "ms_kanji" ||
...
static const lChar16 *heading_volume[] = {L"volume", L"vol",
...
public:
  LVTextLineQueue(LVTextFileBase *f, int maxLineLen)
      : file(f), first_line_index(0), maxLineSize(maxLineLen),
        lastParaWasTitle(false), inSubSection(false) {
    min_left = -1;
    max_right = -1;
    avg_left = 0;
    avg_right = 0;
  ...
  // get line count read from file. Use length() instead to get count of lines
  // queued.
  int GetLineCount() { return first_line_index + length(); }
  // get line by line file index
  LVTextFileLine *GetLine(int index) { return get(index - first_line_index); }
...
#define MAX_PRE_STATS 1000
    ...
    // int best_left_second_align_percent = max_left_second_stats * 100 /
    // length();
    ...
    for (i = 0; i < length(); i++) {
      ...
      if (line->lpos > min_left + 1) {
        ...
        if (absCompare(center_dist, left_dist) < 0) {
          if (line->lpos > min_left + fw / 10 &&
              line->lpos < max_right - fw / 10 && lw < 9 * fw / 10) {
            center_lines++;
          }
        } else
          ident_lines++;
    ...
    if (avg_right >= 80) {
      if (empty_lines > non_empty_lines &&
          empty_lines < non_empty_lines * 110 / 100) {
        formatFlags =
            tftParaPerLine | tftDoubleEmptyLineBeforeHeaders; // default format
        return;
      }
    ...
    CRLog::debug("detectFormatFlags() min_left=%d, max_right=%d, ident=%d, "
                 "empty=%d, flags=%d",
                 min_left, max_right, ident_lines_percent, empty_lines_percent,
                 formatFlags);
  ...
  bool testProjectGutenbergHeader() {
    ...
    if (firstLine.substr(0, pgPrefix.length()) != pgPrefix)
      return false;
    firstLine = firstLine.substr(pgPrefix.length(),
                                 firstLine.length() - pgPrefix.length());
  ...
  bool testAuthorDotTitleFormat() {
    ...
    if (bookTitle.empty() ||
        (lGetCharProps(bookTitle[bookTitle.length()]) & CH_PROP_PUNCT))
      return false;
    return true;
  }
  ...
  bool DetectBookDescription(LVXMLParserCallback *callback) {
    ...
    for (i = 0; i < author_list.length(); i++) {
      ...
      if (name_list.length() > 0) {
        ...
        if (!firstName.empty())
          callback->OnText(firstName.c_str(), firstName.length(),
        ...
        if (!middleName.empty())
          callback->OnText(middleName.c_str(), middleName.length(),
                           TXTFLG_TRIM | TXTFLG_TRIM_REMOVE_EOL_HYPHENS);
        callback->OnTagClose(NULL, L"middle-name");
        callback->OnTagOpenNoAttr(NULL, L"last-name");
        if (!lastName.empty())
          callback->OnText(lastName.c_str(), lastName.length(),
                           TXTFLG_TRIM | TXTFLG_TRIM_REMOVE_EOL_HYPHENS);
  ...
  void AddPara(int startline, int endline, LVXMLParserCallback *callback) {
    ...
    if (startline == endline && endline < length() - 1) {
      if (!(formatFlags & tftParaIdents) || get(startline)->lpos > 0)
        if (get(endline + 1)->rpos == 0 &&
          ...
          singleLineFollowedByEmpty =
              get(startline)->text.length() < MAX_HEADING_CHARS;
          if ((startline <= 1 || get(startline - 2)->rpos == 0))
            singleLineFollowedByTwoEmpty =
                get(startline)->text.length() < MAX_HEADING_CHARS;
        }
    }
    str.trimDoubleSpaces(false, false, true);
    ...
    if (formatFlags & tftDoubleEmptyLineBeforeHeaders) {
      ...
      if (singleLineFollowedByEmpty && startline < 3 &&
          str.length() < MAX_HEADING_CHARS)
        isHeader = true;
      ...
      if ((startline == endline && str.length() < 4) ||
          (paraCount < 2 && str.length() < 50 && startline < length() - 2 &&
           (get(startline + 1)->rpos == 0 ||
            get(startline + 2)->rpos == 0))) // endline<3 &&
        isHeader = true;
      if (startline == endline && get(startline)->isHeading())
        isHeader = true;
      if (startline == endline && (formatFlags & tftCenteredHeaders) &&
          isCentered(get(startline)))
        isHeader = true;
    ...
    if (!str.empty()) {
      ...
      callback->OnText(str.c_str(), str.length(),
                       TXTFLG_TRIM | TXTFLG_TRIM_REMOVE_EOL_HYPHENS);
  ...
  public:
    PMLTextImport(LVXMLParserCallback *cb)
        : callback(cb), insideInvisibleText(false), align(0), chapterIndent(0),
          insideChapterTitle(false), sectionId(0), inSection(false),
          inParagraph(false), indented(false), inLink(false) {
    ...
    void startPage() {
      ...
      callback->OnAttribute(
          NULL, L"id", (cs16("_section") + fmt::decimal(sectionId)).c_str());
      callback->OnTagBody();
      inSection = true;
      endOfParagraph();
    }
    ...
    void processLine(lString16 text) {
      ...
      for (int j = 0; j < len; j++) {
        ...
        if (ch == '\\') {
          if (ch2 == 'a') {
            // \aXXX	Insert non-ASCII character whose Windows 1252 code is
            // decimal XXX.
            ...
            // \UXXXX	Insert non-ASCII character whose Unicode code is
            // hexidecimal XXXX.
            ...
            // Indents the specified percentage of the screen width, 50% in this
            // case. If the current drawing position is already past the
            // specified screen location, this tag is ignored.
            j += 2;
            ...
            // \q="#linkanchor"Some text\q	Reference a link anchor which is
            // at another spot in the document. The string after the anchor
            // specification and before the trailing \q is underlined or
            // otherwise shown to be a link when viewing the document.
            if (!inLink) {
              j += 2;
            ...
            // Embed a horizontal rule of a given percentage width of the
            // screen, in this case 50%. This tag causes a line break before and
            // after it. The rule is centered. The percent sign is mandatory.
            j += 2;
            lString16 w = readParam(str, j);
            addSeparator(50);
            continue;
            ...
            // \Cn="Chapter title"
            // Insert "Chapter title" into the chapter listing, with level n
            // (like \Xn). The text is not shown on the page and does not force
            // a page break. This can sometimes be useful to insert a chapter
            // mark at the beginning of an introduction to the chapter, for
            // example.
            ...
            case 'x': // New chapter; also causes a new page break.
                      // Enclose chapter title (and any style codes) with \x and
                      // \x
            case 'X': // New chapter, indented n levels (n between 0 and 4
                      // inclusive) in the Chapter dialog; doesn't cause a page
                      // break. Enclose chapter title (and any style codes) with
                      // \Xn and \Xn
            {
              int level = 0;
              if (ch2 == 'X') {
    ...
    do {
      for (int i = remainingLines; i < length(); i++) {
        ...
        if (item->rpos > item->lpos) {
          ...
          callback->OnText(item->text.c_str(), item->text.length(),
                           item->flags);
          file->updateProgress();
...
/// constructor
LVTextBookmarkParser::LVTextBookmarkParser(LVStreamRef stream,
                                           LVXMLParserCallback *callback)
...
#define TEXT_PARSER_DETECT_SIZE 16384
  ...
  int charsDecoded =
      ReadTextBytes(0, m_buf_len, chbuf, TEXT_PARSER_DETECT_SIZE - 1, 0);
  bool res = false;
...
static bool extractItem(lString16 &dst, const lString16 &src,
                        const char *prefix) {
...
static void postParagraph(LVXMLParserCallback *callback, const char *prefix,
                          lString16 text, bool allowEmptyLine) {
...
LVTextParser::LVTextParser(LVStreamRef stream, LVXMLParserCallback *callback,
                           bool isPreFormatted)
    : LVTextFileBase(stream), m_callback(callback),
...
/// parses input stream
bool LVXMLTextCache::Parse() { return true; }

/// returns true if format is recognized by parser
bool LVXMLTextCache::CheckFormat() { return true; }
...
lString16 LVXMLTextCache::getText(lUInt32 pos, lUInt32 size, lUInt32 flags) {
  ...
  // CRLog::debug("ReadTextBytes(%d,%d) done - %d chars read", (int)pos,
  // (int)size, (int)chcount);
  text.limit(chcount);
  PreProcessXmlString(text, flags);
  if ((flags & TXTFLG_TRIM) &&
      (!(flags & TXTFLG_PRE) || (flags & TXTFLG_PRE_PARA_SPLITTING))) {
...
LVXMLParser::LVXMLParser(LVStreamRef stream, LVXMLParserCallback *callback,
    ...
    : LVTextFileBase(stream), m_callback(callback), m_trimspaces(true),
      m_state(0), m_citags(false), m_allowHtml(allowHtml), m_fb2Only(fb2Only)
...
#define XML_PARSER_DETECT_SIZE 8192
  ...
  int charsDecoded =
      ReadTextBytes(0, m_buf_len, chbuf, XML_PARSER_DETECT_SIZE - 1, 0);
...
bool LVXMLParser::Parse() {
  ...
  for (; !m_eof && !errorFlag;) {
    ...
    case ps_attr: {
      ...
      m_callback->OnAttribute(attrns.c_str(), attrname.c_str(),
                              attrvalue.c_str());
    ...
    case ps_text: {
      //                if ( dumpActive ) {
      //                    lString16 s;
      //                    s << PeekCharFromBuffer(0) << PeekCharFromBuffer(1)
      //                    << PeekCharFromBuffer(2) << PeekCharFromBuffer(3)
      //                      << PeekCharFromBuffer(4) << PeekCharFromBuffer(5)
      //                      << PeekCharFromBuffer(6) << PeekCharFromBuffer(7);
  ...
  // CRLog::trace("LVXMLParser::Parse() is finished, m_stopped=%s",
  // m_stopped?"true":"false");
  m_callback->OnStop();
  return !errorFlag;
}
...
static const ent_def_t def_entity_table[] = {
    {L"nbsp", 160},     {L"iexcl", 161},   {L"cent", 162},
    {L"pound", 163},    {L"curren", 164},  {L"yen", 165},
    {L"brvbar", 166},   {L"sect", 167},    {L"uml", 168},
    {L"copy", 169},     {L"ordf", 170},    {L"laquo", 171},
    {L"not", 172},      {L"shy", 173},     {L"reg", 174},
    {L"macr", 175},     {L"deg", 176},     {L"plusmn", 177},
    {L"sup2", 178},     {L"sup3", 179},    {L"acute", 180},
    {L"micro", 181},    {L"para", 182},    {L"middot", 183},
    {L"cedil", 184},    {L"sup1", 185},    {L"ordm", 186},
    {L"raquo", 187},    {L"frac14", 188},  {L"frac12", 189},
    {L"frac34", 190},   {L"iquest", 191},  {L"Agrave", 192},
    {L"Aacute", 193},   {L"Acirc", 194},   {L"Atilde", 195},
    {L"Auml", 196},     {L"Aring", 197},   {L"AElig", 198},
    {L"Ccedil", 199},   {L"Egrave", 200},  {L"Eacute", 201},
    {L"Ecirc", 202},    {L"Euml", 203},    {L"Igrave", 204},
    {L"Iacute", 205},   {L"Icirc", 206},   {L"Iuml", 207},
    {L"ETH", 208},      {L"Ntilde", 209},  {L"Ograve", 210},
    {L"Oacute", 211},   {L"Ocirc", 212},   {L"Otilde", 213},
    {L"Ouml", 214},     {L"times", 215},   {L"Oslash", 216},
    {L"Ugrave", 217},   {L"Uacute", 218},  {L"Ucirc", 219},
    {L"Uuml", 220},     {L"Yacute", 221},  {L"THORN", 222},
    {L"szlig", 223},    {L"agrave", 224},  {L"aacute", 225},
    {L"acirc", 226},    {L"atilde", 227},  {L"auml", 228},
    {L"aring", 229},    {L"aelig", 230},   {L"ccedil", 231},
    {L"egrave", 232},   {L"eacute", 233},  {L"ecirc", 234},
    {L"euml", 235},     {L"igrave", 236},  {L"iacute", 237},
    {L"icirc", 238},    {L"iuml", 239},    {L"eth", 240},
    {L"ntilde", 241},   {L"ograve", 242},  {L"oacute", 243},
    {L"ocirc", 244},    {L"otilde", 245},  {L"ouml", 246},
    {L"divide", 247},   {L"oslash", 248},  {L"ugrave", 249},
    {L"uacute", 250},   {L"ucirc", 251},   {L"uuml", 252},
    {L"yacute", 253},   {L"thorn", 254},   {L"yuml", 255},
    {L"quot", 34},      {L"amp", 38},      {L"lt", 60},
    {L"gt", 62},        {L"apos", '\''},   {L"OElig", 338},
    {L"oelig", 339},    {L"Scaron", 352},  {L"scaron", 353},
    {L"Yuml", 376},     {L"circ", 710},    {L"tilde", 732},
    {L"ensp", 8194},    {L"emsp", 8195},   {L"thinsp", 8201},
    {L"zwnj", 8204},    {L"zwj", 8205},    {L"lrm", 8206},
    {L"rlm", 8207},     {L"ndash", 8211},  {L"mdash", 8212},
    {L"lsquo", 8216},   {L"rsquo", 8217},  {L"sbquo", 8218},
    {L"ldquo", 8220},   {L"rdquo", 8221},  {L"bdquo", 8222},
    {L"dagger", 8224},  {L"Dagger", 8225}, {L"permil", 8240},
    {L"lsaquo", 8249},  {L"rsaquo", 8250}, {L"euro", 8364},
    {L"fnof", 402},     {L"Alpha", 913},   {L"Beta", 914},
    {L"Gamma", 915},    {L"Delta", 916},   {L"Epsilon", 917},
    {L"Zeta", 918},     {L"Eta", 919},     {L"Theta", 920},
    {L"Iota", 921},     {L"Kappa", 922},   {L"Lambda", 923},
    {L"Mu", 924},       {L"Nu", 925},      {L"Xi", 926},
    {L"Omicron", 927},  {L"Pi", 928},      {L"Rho", 929},
    {L"Sigma", 931},    {L"Tau", 932},     {L"Upsilon", 933},
    {L"Phi", 934},      {L"Chi", 935},     {L"Psi", 936},
    {L"Omega", 937},    {L"alpha", 945},   {L"beta", 946},
    {L"gamma", 947},    {L"delta", 948},   {L"epsilon", 949},
    {L"zeta", 950},     {L"eta", 951},     {L"theta", 952},
    {L"iota", 953},     {L"kappa", 954},   {L"lambda", 955},
    {L"mu", 956},       {L"nu", 957},      {L"xi", 958},
    {L"omicron", 959},  {L"pi", 960},      {L"rho", 961},
    {L"sigmaf", 962},   {L"sigma", 963},   {L"tau", 964},
    {L"upsilon", 965},  {L"phi", 966},     {L"chi", 967},
    {L"psi", 968},      {L"omega", 969},   {L"thetasym", 977},
    {L"upsih", 978},    {L"piv", 982},     {L"bull", 8226},
    {L"hellip", 8230},  {L"prime", 8242},  {L"Prime", 8243},
    {L"oline", 8254},   {L"frasl", 8260},  {L"weierp", 8472},
    {L"image", 8465},   {L"real", 8476},   {L"trade", 8482},
    {L"alefsym", 8501}, {L"larr", 8592},   {L"uarr", 8593},
    {L"rarr", 8594},    {L"darr", 8595},   {L"harr", 8596},
    {L"crarr", 8629},   {L"lArr", 8656},   {L"uArr", 8657},
    {L"rArr", 8658},    {L"dArr", 8659},   {L"hArr", 8660},
    {L"forall", 8704},  {L"part", 8706},   {L"exist", 8707},
    {L"empty", 8709},   {L"nabla", 8711},  {L"isin", 8712},
    {L"notin", 8713},   {L"ni", 8715},     {L"prod", 8719},
    {L"sum", 8721},     {L"minus", 8722},  {L"lowast", 8727},
    {L"radic", 8730},   {L"prop", 8733},   {L"infin", 8734},
    {L"ang", 8736},     {L"and", 8743},    {L"or", 8744},
    {L"cap", 8745},     {L"cup", 8746},    {L"int", 8747},
    {L"there4", 8756},  {L"sim", 8764},    {L"cong", 8773},
    {L"asymp", 8776},   {L"ne", 8800},     {L"equiv", 8801},
    {L"le", 8804},      {L"ge", 8805},     {L"sub", 8834},
    {L"sup", 8835},     {L"nsub", 8836},   {L"sube", 8838},
    {L"supe", 8839},    {L"oplus", 8853},  {L"otimes", 8855},
    {L"perp", 8869},    {L"sdot", 8901},   {L"lceil", 8968},
    {L"rceil", 8969},   {L"lfloor", 8970}, {L"rfloor", 8971},
    {L"lang", 9001},    {L"rang", 9002},   {L"loz", 9674},
    {L"spades", 9824},  {L"clubs", 9827},  {L"hearts", 9829},
    {L"diams", 9830},   {NULL, 0},
};

/// in-place XML string decoding, don't expand tabs, returns new length (may be
/// less than initial len)
int PreProcessXmlString(lChar16 *str, int len, lUInt32 flags,
                        const lChar16 *enc_table) {
  int state = 0;
  lChar16 nch = 0;
  lChar16 lch = 0;
  lChar16 nsp = 0;
...
int LVTextFileBase::fillCharBuffer() {
  ...
  if (m_read_buffer_len >
      (XML_CHAR_BUFFER_SIZE - (XML_CHAR_BUFFER_SIZE >> 3))) {
    memcpy(m_read_buffer, m_read_buffer + m_read_buffer_pos,
           available * sizeof(lChar16));
    m_read_buffer_pos = 0;
    m_read_buffer_len = available;
  }
  int charsRead = ReadChars(m_read_buffer + m_read_buffer_len,
                            XML_CHAR_BUFFER_SIZE - m_read_buffer_len);
  m_read_buffer_len += charsRead;
  // #ifdef _DEBUG
  //     CRLog::trace("buf: %s\n", UnicodeToUtf8(lString16(m_read_buffer,
  //     m_read_buffer_len)).c_str() );
  // #endif
...
bool LVXMLParser::ReadText() {
  ...
  while (!flgBreak) {
    ...
    for (; m_read_buffer_pos + i < m_read_buffer_len; i++) {
      ...
      lChar16 nextch = m_read_buffer_pos + i + 1 < m_read_buffer_len
                           ? m_read_buffer[m_read_buffer_pos + i + 1]
                           : 0;
      flgBreak = ch == '<' || m_eof;
      if (flgBreak && !tlen) {
        m_read_buffer_pos++;
      ...
      splitParas = false;
      if (last_eol && pre_para_splitting &&
          (ch == ' ' || ch == '\t' || ch == 160) && tlen > 0) //!!!
        splitParas = true;
    ...
    if (tlen > TEXT_SPLIT_SIZE || flgBreak || splitParas) {
      ...
      if ((flags & TXTFLG_TRIM) &&
          (!(flags & TXTFLG_PRE) || (flags & TXTFLG_PRE_PARA_SPLITTING))) {
        nlen = TrimDoubleSpaces(
            buf, nlen,
            ((flags & TXTFLG_TRIM_ALLOW_START_SPACE) || pre_para_splitting)
                ? true
                : false,
            (flags & TXTFLG_TRIM_ALLOW_END_SPACE) ? true : false,
            (flags & TXTFLG_TRIM_REMOVE_EOL_HYPHENS) ? true : false);
...
bool LVXMLParser::SkipSpaces() {
  for (lUInt16 ch = PeekCharFromBuffer(); !m_eof;
       ch = PeekNextCharFromBuffer()) {
    if (!IsSpaceChar(ch))
      break; // char found!
  }
...
lString16 htmlCharset(lString16 htmlHeader) {
  ...
  CRLog::trace("http-equiv content-type: %s",
               UnicodeToUtf8(htmlHeader).c_str());
...
bool LVHTMLParser::CheckFormat() {
  ...
  if (charsDecoded > 30) {
    ...
    if (s.pos("<html") >= 0 &&
        (s.pos("<head") >= 0 ||
         s.pos("<body") >= 0)) //&& s.pos("<FictionBook") >= 0
      res = true;
    lString16 name = m_stream->GetName();
    name.lowercase();
    bool html_ext = name.endsWith(".htm") || name.endsWith(".html") ||
                    name.endsWith(".hhc") || name.endsWith(".xhtml");
...
static const char *AC_THEAD[] = {"thead", "tr",    "thead",
                                 "tfoot", "tbody", NULL};
static const char *AC_TFOOT[] = {"tfoot", "tr",    "thead",
                                 "tfoot", "tbody", NULL};
static const char *AC_TBODY[] = {"tbody", "tr",    "thead",
                                 "tfoot", "tbody", NULL};
static const char *AC_OPTION[] = {"option", "option", NULL};
static const char *AC_PRE[] = {"pre", "pre", NULL};
static const char *AC_INPUT[] = {"input", NULL};
const char **HTML_AUTOCLOSE_TABLE[] = {
    AC_INPUT, AC_OPTION, AC_PRE, AC_P,     AC_LI,    AC_UL,    AC_OL,    AC_TD,
    AC_TH,    AC_DD,     AC_DL,  AC_DT,    AC_TR,    AC_COL,   AC_BR,    AC_HR,
    AC_PARAM, AC_IMG,    AC_DIV, AC_THEAD, AC_TFOOT, AC_TBODY, AC_TABLE, NULL};
...
public:
  ...
  LVBase64Stream(lString8 data) : m_curr_text(data), m_size(0), m_pos(0) {
    // calculate size
    rewind();
...
public:
  ...
  virtual bool OnBlob(lString16 /*name*/, const lUInt8 * /*data*/,
                      int /*size*/) {
    return true;
  }
