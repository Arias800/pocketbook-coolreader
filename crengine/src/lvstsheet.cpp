...
standard_color_t standard_color_table[] = {{"black", 0x000000},
                                           {"green", 0x008000},
                                           {"silver", 0xC0C0C0},
                                           {"lime", 0x00FF00},
                                           {"gray", 0x808080},
...
static const char *css_d_names[] = {"inherit",
                                    "inline",
                                    "block",
                                    "list-item",
                                    "run-in",
...
static const char *css_ws_names[] = {"inherit", "normal", "pre", "nowrap",
                                     NULL};

static const char *css_ta_names[] = {"inherit", "left",    "right",
                                     "center",  "justify", NULL};

static const char *css_td_names[] = {
    "inherit", "none", "underline", "overline", "line-through", "blink", NULL};

static const char *css_hyph_names[] = {"inherit", "none", "auto", NULL};

static const char *css_hyph_names2[] = {"inherit", "optimizeSpeed",
                                        "optimizeQuality", NULL};

static const char *css_hyph_names3[] = {"inherit", "none", "explicit", NULL};

static const char *css_pb_names[] = {"inherit", "auto",  "always", "avoid",
                                     "left",    "right", NULL};

static const char *css_fs_names[] = {"inherit", "normal", "italic", "oblique",
                                     NULL};

static const char *css_fw_names[] = {
    "inherit", "normal", "bold", "bolder", "lighter", "100", "200", "300",
    "400",     "500",    "600",  "700",    "800",     "900", NULL};
static const char *css_va_names[] = {
    "inherit",  "baseline", "sub",    "super",       "top",
    "text-top", "middle",   "bottom", "text-bottom", NULL};

static const char *css_ti_attribute_names[] = {"hanging", NULL};

static const char *css_ff_names[] = {
    "inherit", "serif", "sans-serif", "cursive", "fantasy", "monospace", NULL};

static const char *css_lst_names[] = {
    "inherit",     "disc",        "circle",      "square",
    "decimal",     "lower-roman", "upper-roman", "lower-alpha",
    "upper-alpha", "none",        NULL};

static const char *css_lsp_names[] = {"inherit", "inside", "outside", NULL};
...
bool LVCssSelectorRule::check(const ldomNode *&node) {
  ...
  case cssrt_predecessor: // E + F
    //
    {
      ...
      if (index > 0) {
        ldomNode *elem =
            node->getParentNode()->getChildElementNode(index - 1, _id);
        if (elem) {
          node = elem;
  ...
  case cssrt_attrhas: // E[foo~="value"]
    // one of space separated values
    {
      ...
      if (p < 0)
        return false;
      if ((p > 0 && val[p - 1] != ' ') || (p + _value.length() < val.length() &&
                                           val[p + _value.length()] != ' '))
        return false;
      return true;
    }
...
bool parse_attr_value(const char *&str, char *buf) {
  ...
  if (*str == '\"') {
    ...
    for (; str[pos] && str[pos] != ' ' && str[pos] != '\t' && str[pos] != ']';
         pos++) {
      if (pos >= 64)
        return false;
    }
...
LVCssSelectorRule::LVCssSelectorRule(LVCssSelectorRule &v)
    : _type(v._type), _id(v._id), _attrid(v._attrid), _next(NULL),
      _value(v._value) {
  if (v._next)
...
LVCssSelector::LVCssSelector(LVCssSelector &v)
    : _id(v._id), _decl(v._decl), _specificity(v._specificity), _next(NULL),
      _rules(NULL) {
  if (v._next)
...
void LVStyleSheet::apply(const ldomNode *node, css_style_rec_t *style) {
  ...
  LVCssSelector *selector_id =
      id > 0 && id < _selectors.length() ? _selectors[id] : NULL;
...
lUInt32 LVStyleSheet::getHash() {
  ...
  // CRLog::trace("LVStyleSheet::getHash() selector count: %d  hash: %x",
  // _selectors.length(), hash);
  return hash;
}
...
bool LVLoadStylesheetFile(lString16 pathName, lString8 &css) {
  ...
  if (LVProcessStyleSheetImport(s, import_file)) {
    lString16 importFilename =
        LVMakeRelativeFilename(pathName, Utf8ToUnicode(import_file));
