...
static struct {
  ...
  const lChar16 *table;
  int id;
} _enc_table[] = {{"windows-1250", __cp1250, CRENC_ID_CP1250},
...
static struct {
  const char *name;
  unsigned char **table;
} _uni2byte_enc_table[] = {{"windows-1251", cp1251_page_uni2charset},
...
class CDoubleCharStat {
  ...
  struct CDblCharNode {
    ...
    static inline void Add(CDblCharNode *&pnode, unsigned char c1,
                           unsigned char c2) {
      if (pnode)
        pnode->Add(c1, c2);
      else
    ...
    void GetData(dbl_char_stat_long_t *&pData, int &len,
...
public:
  ...
  void Add(unsigned char c1, unsigned char c2) {
    /*   	if ( !(c1>127 || c1>='a' && c1<='z' || c1>='A' && c1<='Z' ||
       c1=='\'')
               && !(c2>127 || c2>='a' && c2<='z' || c2>='A' && c2<='Z' ||
       c2=='\'') )
          {
             return;
          }
...
int sort_dblstats_by_ch(const void *p1, const void *p2) {
  const dbl_char_stat_long_t *n1 =
      static_cast<const dbl_char_stat_long_t *>(p1);
  const dbl_char_stat_long_t *n2 =
      static_cast<const dbl_char_stat_long_t *>(p2);
  if (n1->ch1 > n2->ch1)
    return 1;
...
public:
  ...
  virtual ~CDoubleCharStat2() { Close(); }
};
...
bool isValidUtf8Data(const unsigned char *buf, int buf_size) {
  ...
  while (buf < end_buf) {
    ...
    if ((ch & 0x80) == 0) {
      ...
      CRLog::trace("unexpected char %02x at position %x, str=%s", ch,
                   (buf - 1 - start),
                   lString8((const char *)(buf - 1), 32).c_str());
      return false;
...
void MakeDblCharStat(const unsigned char *buf, int buf_size,
                     dbl_char_stat_t *stat, int stat_len, bool skipHtml) {
...
double CompareCharStats(const short *stat1, const short *stat2, double &k1,
                        double &k2) {
  double sum = 0;
  double psum = 0;
  double psum2 = 0;
...
double CompareDblCharStats(const dbl_char_stat_t *stat1,
                           const dbl_char_stat_t *stat2, int stat_len,
                           double &k1, double &k2) {
  double sum = 0;
  int len1 = stat_len;
  int len2 = stat_len;
  ...
  while (len1 && len2) {
    ...
    if (stat1->ch1 == stat2->ch1 && stat1->ch2 == stat2->ch2) {
      ...
      stat2++;
      len2--;
    } else if (stat1->ch1 < stat2->ch1 ||
               (stat1->ch1 == stat2->ch1 && stat1->ch2 < stat2->ch2)) {
      // add stat
...
bool detectXmlHtmlEncoding(const unsigned char *buf, int buf_len,
                           char *html_enc_name) {
  ...
  if (content_type_p >= 0) {
    ...
    charset_p += 8;
    int charset_end_p =
        strnstr(buf + meta_p + charset_p, meta_end_p - charset_p, "\"");
    if (charset_end_p < 0)
      return false;
...
int AutodetectCodePage(const unsigned char *buf, int buf_size, char *cp_name,
                       char *lang_name, bool skipHtml) {
  ...
  for (int i = 0; cp_stat_table[i].ch_stat; i++) {
    ...
    double q2 = CompareDblCharStats(cp_stat_table[i].dbl_ch_stat, dbl_char_stat,
                                    DBL_CHAR_STAT_SIZE, q21, q22);
    ...
    if (q2 < 0.00001)
      q2 = 0.00001;
    double q = q11 * 0 + q12 * 2 + q21 * 0 +
               q22 * 6; //(q_>0) ? (q1*2+q2*7) / (q_) : 1000000;
    q = q / (q1 + q2);
    // CRLog::debug("%d %10s %4s : %lf %lf %lf - %lf %lf %lf  :  %lf", i,
    // cp_stat_table[i].cp_name, cp_stat_table[i].lang_name, q1, q11, q12, q2,
    // q21, q22, q);
    if (q > bestq) {
      bestn = i;
      bestq = q;
  ...
  CRLog::debug("Detected codepage:%s lang:%s index:%d %s", cp_name, lang_name,
               bestn, skipHtml ? "(skipHtml)" : "");
  if (skipHtml) {
...
void MakeStatsForFile(const char *fname, const char *cp_name,
                      const char *lang_name, int index, FILE *f,
                      lString8 &list) {
  ...
  for (i = 0; i < DBL_CHAR_STAT_SIZE / 16; i++) {
    for (int j = 0; j < 16; j++) {
      fprintf(f, "{0x%02x,0x%02x,0x%04x}, ",
              (unsigned int)dbl_char_stat[i * 16 + j].ch1,
              (unsigned int)dbl_char_stat[i * 16 + j].ch2,
              (unsigned int)((lUInt16)dbl_char_stat[i * 16 + j].count));
    }
    fprintf(f, "// %d..%d\n", i * 16, i * 16 + 15);
  }
  char str[100];
  sprintf(str, "{ch_stat_%s_%s%d,dbl_ch_stat_%s_%s%d,\"%s\",\"%s\"}, \n",
          cp_name, lang_name, index, cp_name, lang_name, index, cp_name,
          lang_name);
  list += str;
  fprintf(f, "};\n\n");
  delete[] buf;
