...
#define RTF_ACT(name, index) {RTF_##name, #name, CWT_ACT, index, 0},
#define RTF_CMD(name, type, index) {RTF_##name, #name, type, index, 0},
#define RTF_CHC(name, index) {RTF_##name, #name, CWT_CHAR, index, 0},
#define RTF_CHR(character, name, index)                                        \
  {RTF_##name, character, CWT_CHAR, index, 0},
#define RTF_DST(name, index) {RTF_##name, #name, CWT_DEST, index, 0},
...
static const rtf_control_word *findControlWord(const char *name) {
  ...
  for (;;) {
    ...
    //     CRLog::error("findControlWord invalid index: %d of %d", c,
    //     rtf_words_count); return NULL;
    // }
...
public:
  LVRtfDefDestination(LVRtfParser &parser)
      : LVRtfDestination(parser), tblState(tbls_none), in_section(false),
        in_title(false), in_para(false), last_space(false), last_notitle(true),
        in_subtitle(false) {}
  // set table state, open/close tags if necessary
  void SetTableState(rtfTblState state) {
    static const lChar16 *tags[5] = {NULL,     // tbls_none=0,
  ...
  virtual void OnTblProp(int id, int) {
    ...
    case tpi_irowN: // N is the row index of this row.
      break;
    case tpi_irowbandN: // N is the row index of the row, adjusted to account
                        // for header rows. A header row has a value of �1.
      break;
    ...
    case tpi_clmgf: // The first cell in a range of table cells to be merged.
      break;
    case tpi_clmrg: // Contents of the table cell are merged with those of the
                    // preceding cell.
      break;
    case tpi_clvmgf: // The first cell in a range of table cells to be
                     // vertically merged.
      break;
    case tpi_clvmrg: // Contents of the table cell are vertically merged with
...
public:
  LVRtfNullDestination(LVRtfParser &parser) : LVRtfDestination(parser) {}
  virtual void OnControlWord(const char *, int) {}
...
LVRtfParser::LVRtfParser(LVStreamRef stream, LVXMLParserCallback *callback)
    : LVFileParserBase(stream), m_callback(callback), txtbuf(NULL),
      imageIndex(0) {
...
LVRtfDestination::LVRtfDestination(LVRtfParser &parser)
    : m_parser(parser), m_stack(parser.getStack()),
...
bool LVRtfParser::Parse() {
  ...
  //
  lString16 bookTitle =
      LVExtractFilenameWithoutExtension(getFileName()); // m_stream->GetName();
...
void LVRtfParser::Reset() { LVFileParserBase::Reset(); }
...
lChar16 *LVRtfParser::GetCharsetTable() { return NULL; }
...
#ifdef LOG_RTF_PARSING
    CRLog::trace("CW: %s\\%s %d", asterisk ? "\\*" : "", control,
                 param == PARAM_VALUE_NONE ? 0 : param);
#endif
    if (asterisk) {
