...
public:
  ...
  LZSSUtil();
  /// encode buffer
  bool Encode(const lUInt8 *in_buf, int in_length, lUInt8 *out_buf,
              int &out_length);
  /// decode buffer
  bool Decode(const lUInt8 *in_buf, int in_length, lUInt8 *out_buf,
              int &out_length);
};
...
public:
  InBuf(const lUInt8 *buf, int size) : _buf(buf), _size(size), _pos(0) {}
  bool get(lUInt32 &ch) {
    if (_pos >= _size)
      return false;
    ch = _buf[_pos++];
...
bool LZSSUtil::Encode(const lUInt8 *in_buf, int in_length, lUInt8 *out_buf,
                      int &out_length) {
...
bool LZSSUtil::Decode(const lUInt8 *in_buf, int in_length, lUInt8 *out_buf,
                      int &out_length) {
  InBuf in(in_buf, in_length);
  OutBuf out(out_buf, out_length);
  lUInt32 i, k, r;
...
WOLBase::WOLBase(LVStream *stream)
    : _stream(stream), _book_title_size(0), _cover_image_size(0),
      _page_data_size(0), _catalog_size(0), _wolf_start_pos(0),
      _subcatalog_level23_items(0), _subcatalog_offset(0),
      _catalog_level1_items(0), _catalog_subcatalog_size(0) {}

WOLReader::WOLReader(LVStream *stream) : WOLBase(stream) {}
...
static void readMem(const lUInt8 *buf, int offset, lUInt32 &dest) {
  dest = ((lUInt32)buf[offset + 3] << 24) | ((lUInt32)buf[offset + 2] << 16) |
         ((lUInt32)buf[offset + 1] << 8) | buf[offset];
}
...
bool WOLReader::readHeader() {
  ...
  for (;;) {
    ...
    if (sscanf(str.c_str(),
               "img bitcount=%d compact=%d width=%d height=%d length=%d",
               &params.bitcount, &params.compact, &params.width, &params.height,
               &params.length) != 5)
      return false;
...
LVGrayDrawBuf *WOLReader::getImage(int index) {
  ...
  if (unpacker.Decode(buf.ptr(), buf.length(), uncomp.ptr(), uncomp_len)) {
    ...
    LVGrayDrawBuf *image =
        new LVGrayDrawBuf(img->width, img->height, img->bitcount);
...
WOLWriter::WOLWriter(LVStream *stream)
    : WOLBase(stream), _catalog_opened(false) {
  lUInt8 header[0x80];
...
typedef struct {
  lUInt32 PageOffs; //[0x00] PageOffs: offset from beginning of section
                    //"WolfPages" to description of page (<img...>).
  lUInt32 NameOffs; //[0x04] NameOffs: offset from beginning of file to beginnig
                    // of name (in "Names" area).
  lUInt16
      NameSize; //[0x08] NameSize: length (in chars) of name (in "Names" area).
  lUInt16 ChildsCount; //[0x0A] ChildsCount: count of subitems for this element
                       // from TOC.
  lUInt32
      PrevPeerOffs; //[0x0C] PrevPeerOffs: offset from beginning of file to the
...
void WOLWriter::writeToc() {
  ...
  if (len == 0) {
    ...
    *_stream << "<catalog><item>" << _book_name << "</item>";
    ...
    *_stream << "<catalog>";
    for (int ti = 0; ti < len; ti++) {
      ...
      if (src->getLevel() == 1) {
        *_stream << "<item>" << src->name << "</item>"
                 << cnv.lsf(getPageOffset(src->page));
    ...
    *_stream << "</catalog>";
    ...
    for (i = 0; i < len; i++) {
      ...
      item->ChildOffs =
          cnv.lsf(src->firstChild
                      ? src->firstChild->catindex * 80 + _subcatalog_offset + 12
                      : 0);
      item->ParentOffs = cnv.lsf(src->parent ? src->parent->catindex * 80 +
                                                   _subcatalog_offset + 12
                                             : 0);
      item->NextPeerOffs =
          cnv.lsf(src->nextSibling ? src->nextSibling->catindex * 80 +
                                         _subcatalog_offset + 12
                                   : 0);
      item->PrevPeerOffs =
          cnv.lsf(src->prevSibling ? src->prevSibling->catindex * 80 +
                                         _subcatalog_offset + 12
                                   : 0);
    ...
    for (i = 0; i < len; i++) {
      ...
      fprintf(log,
              "%2d   %07d : (%d,%d,%d) \t parent=%07d  child=%07d  prev=%07d  "
              "next=%07d\t %s\n",
              i, currOffset, item->Level1Idx, item->Level2Idx, item->Level3Idx,
              item->ParentOffs, item->ChildOffs, item->PrevPeerOffs,
              item->NextPeerOffs, item->ItemName);
    }
#endif
    *_stream << "<subcatalog>";
    _stream->Write(toc, sizeof(wol_toc_subcatalog_item) * len, NULL);
    *_stream << names;
    // SubcatalogSize = 26 + CountOfAllTOCItems * 80 +
    // Length(NamesInAllTOCLevels)
...
void WOLWriter::addTitle(const lString8 &title, const lString8 &subject,
                         const lString8 &author, const lString8 &adapter,
                         const lString8 &translator, const lString8 &publisher,
                         const lString8 &time_publish,
                         const lString8 &introduction, const lString8 &isbn) {
  //
  _book_name = title;
  lString8 buf;
  buf.reserve(128);
...
void WOLWriter::addCoverImage(const lUInt8 *buf, int size) {
  static const lUInt8 cover_hdr[10] = {0xFF, 0xFF, 0x58, 0x02, 0x01,
                                       0x00, 0x4B, 0x00, 0x20, 0x03};
...
void WOLWriter::addImage(int width, int height,
  ...
  */
  ...
  lString8 buf;
  buf.reserve(128);
  buf << "<img bitcount=" << fmt::decimal(num_bits)
      << " compact=1 width=" << fmt::decimal(width)
      << " height=" << fmt::decimal(height)
      << " length=" << fmt::decimal((int)compressed_len) << ">";
  *_stream << buf;
...
void WOLWriter::addTocItem(int level1index, int level2index, int level3index,
                           int pageNumber, lString8 title) {
#ifdef _DEBUG_LOG
  fprintf(log, "addTocItem(lvl=%d,%d,%d, page=%d, text=%s\n", level1index,
          level2index, level3index, pageNumber, title.c_str());
#endif
  TocItemInfo *item =
      new TocItemInfo(_tocItems.length(), level1index, level2index, level3index,
                      pageNumber, title);
  _tocItems.add(item);
...
void WOLWriter::addImage(LVGrayDrawBuf &image) {
  addImage(image.GetWidth(), image.GetHeight(), image.GetScanLine(0),
           image.GetBitsPerPixel());
}

typedef struct {
  lUInt16 compression; // Compression: 0xFFFF for raw data (no compression);
                       // 0x0001 for LZSS compression.
  lUInt16 width;       // ImageWidth: width of image (in pixels)
  lUInt16
      bpp; // BitsPerPixel: 1 for monochrome images; 2 for 4-level gray images.
  lUInt16 bytesPerLine; // BytesPerLine: count of bytes in one row of image. (=
