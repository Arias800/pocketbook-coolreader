...
#define CHECK_GUARD_BYTE                                                       \
  {                                                                            \
    if (_bpp != 1 && _bpp != 2 && _bpp != 3 && _bpp != 4 && _bpp != 8 &&       \
        _bpp != 16 && _bpp != 32)                                              \
...
void LVDrawBuf::RoundRect(int x0, int y0, int x1, int y1, int borderWidth,
                          int radius, lUInt32 color, int cornerFlags) {
  FillRect(x0 + ((cornerFlags & 1) ? radius : 0), y0,
           x1 - 1 - ((cornerFlags & 2) ? radius : 0), y0 + borderWidth, color);
  FillRect(x0, y0 + ((cornerFlags & 1) ? radius : 0), x0 + borderWidth,
           y1 - 1 - ((cornerFlags & 4) ? radius : 0), color);
  FillRect(x1 - borderWidth, y0 + ((cornerFlags & 2) ? radius : 0), x1,
           y1 - ((cornerFlags & 8) ? radius : 0), color);
  FillRect(x0 + ((cornerFlags & 4) ? radius : 0), y1 - borderWidth,
           x1 - ((cornerFlags & 8) ? radius : 0), y1, color);
...
static void ApplyAlphaRGB(lUInt32 &dst, lUInt32 src, lUInt32 alpha) {
  if (alpha == 0)
    ...
    lUInt32 n1 = (((dst & 0xFF00FF) * alpha + (src & 0xFF00FF) * opaque) >> 8) &
                 0xFF00FF;
    lUInt32 n2 = (((dst & 0x00FF00) * alpha + (src & 0x00FF00) * opaque) >> 8) &
                 0x00FF00;
    dst = n1 | n2;
  }
}
...
static void ApplyAlphaRGB565(lUInt16 &dst, lUInt16 src, lUInt32 alpha) {
  if (alpha == 0)
    ...
    lUInt32 opaque = 256 - alpha;
    lUInt32 r =
        (((dst & 0xF800) * alpha + (src & 0xF800) * opaque) >> 8) & 0xF800;
    lUInt32 g =
        (((dst & 0x07E0) * alpha + (src & 0x07E0) * opaque) >> 8) & 0x07E0;
    lUInt32 b =
        (((dst & 0x001F) * alpha + (src & 0x001F) * opaque) >> 8) & 0x001F;
    dst = (lUInt16)(r | g | b);
  }
}
...
static const short dither_2bpp_8x8[] = {
    0,  32, 12, 44, 2,  34, 14, 46, 48, 16, 60, 28, 50, 18, 62, 30,
    8,  40, 4,  36, 10, 42, 6,  38, 56, 24, 52, 20, 58, 26, 54, 22,
    3,  35, 15, 47, 1,  33, 13, 45, 51, 19, 63, 31, 49, 17, 61, 29,
    11, 43, 7,  39, 9,  41, 5,  37, 59, 27, 55, 23, 57, 25, 53, 21,
};
...
lUInt32 DitherNBitColor(lUInt32 color, lUInt32 x, lUInt32 y, int bits) {
  ...
  // gray = (r + 2*g + b)>>2
  // int cl = ((((color>>16) & 255) + ((color>>(8-1)) & (255<<1)) + ((color) &
  // 255)) >> 2) & 255;
  int cl = ((((color >> 16) & 255) + ((color >> (8 - 1)) & (255 << 1)) +
             ((color) & 255)) >>
            2) &
           255;
  int white = (1 << bits) - 1;
...
lUInt32 Dither2BitColor(lUInt32 color, lUInt32 x, lUInt32 y) {
  int cl = ((((color >> 16) & 255) + ((color >> 8) & 255) + ((color) & 255)) *
            (256 / 3)) >>
           8;
  if (cl < 5)
    return 0;
...
static lUInt8 revByteBits1(lUInt8 b) {
  return ((b & 1) << 7) | ((b & 2) << 5) | ((b & 4) << 3) | ((b & 8) << 1) |
         ((b & 16) >> 1) | ((b & 32) >> 3) | ((b & 64) >> 4) | ((b & 128) >> 5);
}

lUInt8 revByteBits2(lUInt8 b) {
  return ((b & 0x03) << 6) | ((b & 0x0C) << 2) | ((b & 0x30) >> 2) |
         ((b & 0xC0) >> 6);
}
...
public:
  ...
  static int *GenNinePatchMap(int src_len, int dst_len, int frame1,
                              int frame2) {
    int *map = new int[dst_len];
    ...
    for (int i = 0; i < dst_len; i++) {
      ...
      //            CRLog::trace("frame[%d, %d] src=%d dst=%d %d -> %d", frame1,
      //            frame2, src_len, dst_len, i, map[i]); if (map[i] >= src_len)
      //            {
  ...
  LVImageScaledDrawCallback(LVBaseDrawBuf *dstbuf, LVImageSourceRef img, int x,
                            int y, int width, int height, bool dith)
      : src(img), dst(dstbuf), dst_x(x), dst_y(y), dst_dx(width),
        dst_dy(height), xmap(0), ymap(0), dither(dith) {
    src_dx = img->GetWidth();
...
int LVBaseDrawBuf::GetWidth() { return _dx; }

int LVBaseDrawBuf::GetHeight() { return _dy; }

int LVGrayDrawBuf::GetBitsPerPixel() { return _bpp; }

void LVGrayDrawBuf::Draw(LVImageSourceRef img, int x, int y, int width,
                         int height, bool dither) {
...
void LVGrayDrawBuf::FillRectPattern(int x0, int y0, int x1, int y1,
                                    lUInt32 color032, lUInt32 color132,
                                    lUInt8 *pattern) {
  if (x0 < _clip.left)
    x0 = _clip.left;
  if (y0 < _clip.top)
  ...
  for (int y = y0; y < y1; y++) {
    ...
    if (_bpp == 1) {
      for (int x = x0; x < x1; x++) {
        ...
        lUInt8 mask = 0x80 >> (x & 7);
        int index = x >> 3;
        line[index] = (lUInt8)((line[index] & ~mask) |
                               ((patternBit ? color1 : color0) & mask));
      }
    } else if (_bpp == 2) {
      for (int x = x0; x < x1; x++) {
        lUInt8 patternBit = (patternMask << (x & 7)) & 0x80;
...
static const lUInt8 inverted_bytes[] = {
    ,
    0xf,  0xd,  0xe,  0xc,  0x7,  0x5,  0x6,  0x4,  0xb,  0x9,  0xa,  0x8,
    0x3,  0x1,  0x2,  0x0};
...
void LVGrayDrawBuf::Draw(int x, int y, const lUInt8 *bitmap, int width,
                         int height, lUInt32 *) {
  ...
  if (y < _clip.top) {
    ...
    if (_hidePartialGlyphs &&
        height <= initial_height / 2) // HIDE PARTIAL VISIBLE GLYPHS
      return;
    if (height <= 0)
      return;
  }
...
lUInt8 *LVGrayDrawBuf::GetScanLine(int y) { return _data + _rowsize * y; }
...
int LVColorDrawBuf::GetBitsPerPixel() { return _bpp; }

void LVColorDrawBuf::Draw(LVImageSourceRef img, int x, int y, int width,
                          int height, bool dither) {
...
inline static lUInt32 AA(lUInt32 color) { return (color >> 24) & 0xFF; }

inline static lUInt32 RR(lUInt32 color) { return (color >> 16) & 0xFF; }

inline static lUInt32 GG(lUInt32 color) { return (color >> 8) & 0xFF; }

inline static lUInt32 BB(lUInt32 color) { return color & 0xFF; }
...
/// get linearly interpolated pixel value (coordinates are fixed floating points
/// *16)
...
/// fills rectangle with specified color
void LVColorDrawBuf::FillRectPattern(int x0, int y0, int x1, int y1,
                                     lUInt32 color0, lUInt32 color1,
                                     lUInt8 *pattern) {
  if (x0 < _clip.left)
    x0 = _clip.left;
  if (y0 < _clip.top)
...
void LVColorDrawBuf::Resize(int dx, int dy) {
  ...
  // CRLog::trace("LVColorDrawBuf::Resize : resizing %d x %d to %d x %d", _dx,
  // _dy, dx, dy);
  //  delete old bitmap
  ...
  if (dx > 0 && dy > 0) {
    ...
    _drawbmp = CreateDIBSection(NULL, &bmi, DIB_RGB_COLORS, (void **)(&_data),
                                NULL, 0);
...
void LVColorDrawBuf::Draw(int x, int y, const lUInt8 *bitmap, int width,
                          int height, lUInt32 *palette) {
  // int buf_width = _dx; /* 2bpp */
  int initial_height = height;
  int bx = 0;
  int by = 0;
      ...
      for (xx = width; xx > 0; --xx) {
        ...
        if (opaque >= 0xF)
          *dst = bmpcl16;
          ...
          lUInt16 cl1 = (lUInt16)(((alpha * ((*dst) & 0xF81F) +
                                    opaque * (bmpcl16 & 0xF81F)) >>
                                   4) &
                                  0xF81F);
          lUInt16 cl2 = (lUInt16)(((alpha * ((*dst) & 0x07E0) +
                                    opaque * (bmpcl16 & 0x07E0)) >>
                                   4) &
                                  0x07E0);
          *dst = cl1 | cl2;
        }
      ...
      for (xx = width; xx > 0; --xx) {
        ...
        if (opaque >= 0x78)
          *dst = bmpcl;
          ...
          lUInt32 cl1 =
              ((alpha * ((*dst) & 0xFF00FF) + opaque * (bmpcl & 0xFF00FF)) >>
               7) &
              0xFF00FF;
          lUInt32 cl2 =
              ((alpha * ((*dst) & 0x00FF00) + opaque * (bmpcl & 0x00FF00)) >>
               7) &
              0x00FF00;
          *dst = cl1 | cl2;
        }
...
void LVGrayDrawBuf::DrawTo(HDC dc, int x, int y, int options,
                           lUInt32 *palette) {
  if (!dc || !_data)
    return;
...
/// draws buffer content to DC doing color conversion if necessary
void LVColorDrawBuf::DrawTo(HDC dc, int x, int y, int options,
                            lUInt32 *palette) {
...
void LVGrayDrawBuf::DrawTo(LVDrawBuf *buf, int x, int y, int options,
                           lUInt32 *palette) {
  CR_UNUSED2(options, palette);
  lvRect clip;
  buf->GetClipRect(&clip);

  if (!(!clip.isEmpty() || buf->GetBitsPerPixel() != GetBitsPerPixel() ||
        GetWidth() != buf->GetWidth() || GetHeight() != buf->GetHeight())) {
    // simple copy
...
/// draws buffer content to another buffer doing color conversion if necessary
void LVColorDrawBuf::DrawTo(LVDrawBuf *buf, int x, int y, int options,
                            lUInt32 *palette) {
  CR_UNUSED(options);
  CR_UNUSED(palette);
  //
...
#if (GRAY_INVERSE == 1)
              lUInt8 cl =
                  (((lUInt8)(*src) & 0xC000) ^ 0xC000) >> ((shift << 1) + 8);
#else
...
/// draws rescaled buffer content to another buffer doing color conversion if
/// necessary
void LVGrayDrawBuf::DrawRescaled(LVDrawBuf *src, int x, int y, int dx, int dy,
                                 int options) {
  CR_UNUSED(options);
  if (dx < 1 || dy < 1)
    return;
  ...
  // CRLog::trace("LVGrayDrawBuf::DrawRescaled bpp=%d %dx%d srcbpp=%d (%d,%d)
  // (%d,%d)", _bpp, GetWidth(), GetHeight(), src->GetBitsPerPixel(), x, y, dx,
  // dy);
  CHECK_GUARD_BYTE;
  for (int yy = 0; yy < dy; yy++) {
    if (y + yy >= clip.top && y + yy < clip.bottom) {
      ...
      if (linearInterpolation) {
          ...
          if (x + xx >= clip.left && x + xx < clip.right) {
            ...
            if (_bpp == 1) {
              ...
              *dst = dithered;
              ...
              lUInt8 *dst = dst0 + x + xx;
              lUInt32 dithered =
                  DitherNBitColor(cl, xx, yy, _bpp); // << (8 - _bpp);
              if (alpha < 16)
                *dst = (lUInt8)dithered;
              else if (alpha < 240) {
        ...
        {
          if (_ownData && _data[_rowsize * _dy] != GUARD_BYTE) {
            CRLog::error("lin interpolation, corrupted buffer, yy=%d of %d", yy,
                         dy);
            crFatalError(-5, "corrupted bitmap buffer");
          }
        }
...
/// draws rescaled buffer content to another buffer doing color conversion if
/// necessary
void LVColorDrawBuf::DrawRescaled(LVDrawBuf *src, int x, int y, int dx, int dy,
                                  int options) {
  CR_UNUSED(options);
  if (dx < 1 || dy < 1)
    return;
...
/// returns white pixel value
lUInt32 LVColorDrawBuf::GetWhiteColor() { return 0xFFFFFF; }
/// returns black pixel value
lUInt32 LVColorDrawBuf::GetBlackColor() { return 0x000000; }
...
#if !defined(__SYMBIAN32__) && defined(_WIN32) && !defined(QT_GL)
      ,
      _drawdc(NULL), _drawbmp(NULL)
#endif
      ,
      _bpp(bpp), _ownData(true) {
  _rowsize = dx * (_bpp >> 3);
  Resize(dx, dy);
}
