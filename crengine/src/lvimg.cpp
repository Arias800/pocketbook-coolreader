...
void CR9PatchInfo::calcRectangles(const lvRect &dst, const lvRect &src,
                                  lvRect dstitems[9],
...
public:
  CRNinePatchDecoder(int dx, int dy, CR9PatchInfo *info)
      : _dx(dx), _dy(dy), _info(info) {}
  virtual ~CRNinePatchDecoder() {}
  virtual void OnStartDecode(LVImageSource *obj) { CR_UNUSED(obj); }
...
cr_resync_to_restart(j_decompress_ptr, int) { return FALSE; }
...
METHODDEF(void)
cr_term_source(j_decompress_ptr) { /* no work necessary here */ }
...
METHODDEF(void)
cr_jpeg_error(j_common_ptr cinfo) {
  // fprintf(stderr, "cr_jpeg_error() : fatal error while decoding JPEG
  // image\n");
  ...
  // CRLog::error("cr_jpeg_error : returning control to setjmp point %08x",
  // &myerr->setjmp_buffer);
...
public:
  LVXPMImageSource(const char **data)
    ...
    bool err = false;
    int charsperpixel;
    if (sscanf(data[0], "%d %d %d %d", &_width, &_height, &_ncolors,
               &charsperpixel) != 4) {
      err = true;
    } else if (_width > 0 && _width < 255 && _height > 0 && _height < 255 &&
               _ncolors >= 2 && _ncolors < 255 && charsperpixel == 1) {
...
public:
  ...
  virtual bool Decode(LVImageDecoderCallback *callback) {
    ...
    // fprintf(stderr, "    jpeg_read_header() finished succesfully: image size
    // = %d x %d\n", _width, _height);
    ...
    if (callback) {
      ...
      while (cinfo.output_scanline < cinfo.output_height) {
        ...
        for (int x = 0; x < (int)cinfo.output_width; x++) {
          row[x] = (((lUInt32)p[0]) << 16) | (((lUInt32)p[1]) << 8) |
                   (((lUInt32)p[2]) << 0);
          p += 3;
        }
...
bool LVPngImageSource::Decode(LVImageDecoderCallback *callback) {
  ...
  lUInt32 *row = NULL;
  _stream->SetPos(0);
  png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, (png_voidp)this,
                                   lvpng_error_func, lvpng_warning_func);
  if (!png_ptr)
    return false;
  ...
  png_set_read_fn(png_ptr, (void *)this, lvpng_read_func);
...
public:
  ...
  int DecodeFromBuffer(unsigned char *buf, int buf_size,
                       LVImageDecoderCallback *callback);
...
public:
  ...
  void Draw(LVImageDecoderCallback *callback) {
    ...
    int interlacePos = 0;
    int interlaceTable[] = {8, 0, 8, 4, 4,
                            2, 2, 1, 1, 1}; // pairs: step, offset
...
LVGifImageSource::~LVGifImageSource() { Clear(); }
...
int LVGifImageSource::DecodeFromBuffer(unsigned char *buf, int buf_size,
                                       LVImageDecoderCallback *callback) {
...
public:
  ...
  int ReadInCode() {
    int code =
        (p_in_stream[0]) + (p_in_stream[1] << 8) + (p_in_stream[2] << 16);
    code >>= in_bit_pos;
    code &= (1 << bits) - 1;
    in_bit_pos += bits;
    if (in_bit_pos >= 8) {
  ...
  int CodeExists(int code) { return (code < lastadd); };
...
int LVGifFrame::DecodeFromBuffer(unsigned char *buf, int buf_size,
                                 int &bytes_read) {
  bytes_read = 0;
  unsigned char *p = buf;
  if (*p != ',' || buf_size <= 10)
  ...
  if (m_cx < 1 || m_cx > 4096 || m_cy < 1 || m_cy > 4096 ||
...
LVImageSourceRef LVCreateDummyImageSource(ldomNode *node, int width,
                                          int height) {
...
public:
  LVStretchImgSource(LVImageSourceRef src, int newWidth, int newHeight,
                     ImageTransform hTransform, ImageTransform vTransform,
                     int splitX, int splitY)
      : _src(src), _src_dx(src->GetWidth()), _src_dy(src->GetHeight()),
        _dst_dx(newWidth), _dst_dy(newHeight), _hTransform(hTransform),
        _vTransform(vTransform), _split_x(splitX), _split_y(splitY) {
...
bool LVStretchImgSource::OnLineDecoded(LVImageSource *obj, int y,
                                       lUInt32 *data) {
  bool res = false;
...
/// creates image which stretches source image by filling center with pixels at
/// splitX, splitY
LVImageSourceRef LVCreateStretchFilledTransform(LVImageSourceRef src,
                                                int newWidth, int newHeight,
                                                ImageTransform hTransform,
                                                ImageTransform vTransform,
                                                int splitX, int splitY) {
  if (src.isNull())
    return LVImageSourceRef();
  return LVImageSourceRef(new LVStretchImgSource(
      src, newWidth, newHeight, hTransform, vTransform, splitX, splitY));
}

/// creates image which fills area with tiled copy
LVImageSourceRef LVCreateTileTransform(LVImageSourceRef src, int newWidth,
                                       int newHeight, int offsetX,
                                       int offsetY) {
  if (src.isNull())
    return LVImageSourceRef();
  return LVImageSourceRef(
      new LVStretchImgSource(src, newWidth, newHeight, IMG_TRANSFORM_TILE,
                             IMG_TRANSFORM_TILE, offsetX, offsetY));
}
...
class LVColorTransformImgSource : public LVImageSource,
...
public:
  LVColorTransformImgSource(LVImageSourceRef src, lUInt32 addRGB,
                            lUInt32 multiplyRGB)
      : _src(src), _add(addRGB), _multiply(multiplyRGB), _drawbuf(NULL) {}
    ...
    for (int y = 0; y < dy; y++) {
      ...
      for (int x = 0; x < dx; x++) {
        ...
        if (a != 0xFF000000) {
          ...
          row[x] =
              a | (limit256(r) << 16) | (limit256(g) << 8) | (limit256(b) << 0);
        }
      }
...
/// creates image source which transforms colors of another image source (add
/// RGB components (c - 0x80) * 2 from addedRGB first, then multiplyed by
/// multiplyRGB fixed point components (0x20 is 1.0f)
LVImageSourceRef LVCreateColorTransformImageSource(LVImageSourceRef srcImage,
                                                   lUInt32 addRGB,
                                                   lUInt32 multiplyRGB) {
  return LVImageSourceRef(
      new LVColorTransformImgSource(srcImage, addRGB, multiplyRGB));
}

class LVAlphaTransformImgSource : public LVImageSource,
                                  public LVImageDecoderCallback {
protected:
  LVImageSourceRef _src;
  LVImageDecoderCallback *_callback;
...
/// creates image source which applies alpha to another image source (0 is no
/// change, 255 is totally transparent)
LVImageSourceRef LVCreateAlphaTransformImageSource(LVImageSourceRef srcImage,
                                                   int alpha) {
  if (alpha <= 0)
    return srcImage;
...
class LVUnpackedImgSource : public LVImageSource,
                            public LVImageDecoderCallback {
protected:
  bool _isGray;
  int _bpp;
...
public:
  LVUnpackedImgSource(LVImageSourceRef src, int bpp)
      : _isGray(bpp <= 8), _bpp(bpp), _grayImage(NULL), _colorImage(NULL),
        _colorImage16(NULL), _dx(src->GetWidth()), _dy(src->GetHeight()) {
    if (bpp <= 8) {
  ...
  inline lUInt8 grayPack(lUInt32 pixel) {
    lUInt8 gray = (lUInt8)(((pixel & 255) + ((pixel >> 16) & 255) +
                            ((pixel >> 7) & 510)) >>
                           2);
...
public:
  LVDrawBufImgSource(LVColorDrawBuf *buf, bool own)
      : _buf(buf), _own(own), _dx(buf->GetWidth()), _dy(buf->GetHeight()) {}
  virtual ldomNode *GetSourceNode() { return NULL; }
  virtual LVStream *GetSourceStream() { return NULL; }
  virtual void Compact() {}
  virtual int GetWidth() { return _dx; }
...
/// creates decoded memory copy of image, if it's unpacked size is less than
/// maxSize
LVImageSourceRef LVCreateUnpackedImageSource(LVImageSourceRef srcImage,
                                             int maxSize, bool gray) {
  if (srcImage.isNull())
    return srcImage;
...
/// draws battery icon in specified rectangle of draw buffer; if font is
/// specified, draws charge %
// first icon is for charging, the rest - indicate progress icon[1] is lowest
// level, icon[n-1] is full power if no icons provided, battery will be drawn
void LVDrawBatteryIcon(LVDrawBuf *drawbuf, const lvRect &batteryRc, int percent,
                       bool charging, LVRefVec<LVImageSource> icons,
                       LVFont *font) {
  lvRect rc(batteryRc);
  ...
  if (icons.length() > 1) {
    ...
    drawbuf->Draw(icon, rc.left, rc.top, sz.x, sz.y, false);
    if (charging)
      drawText = false;
    rc.left += 3;
  } else {
  ...
  if (drawText) {
    ...
    font->DrawTextString(drawbuf, x - 1, y, txt.c_str(), txt.length(), '?',
                         NULL);
    font->DrawTextString(drawbuf, x + 1, y, txt.c_str(), txt.length(), '?',
                         NULL);
    //        font->DrawTextString(drawbuf, x-1, y+1, txt.c_str(), txt.length(),
    //        '?', NULL); font->DrawTextString(drawbuf, x+1, y-1, txt.c_str(),
    //        txt.length(), '?', NULL);
    font->DrawTextString(drawbuf, x, y - 1, txt.c_str(), txt.length(), '?',
                         NULL);
    font->DrawTextString(drawbuf, x, y + 1, txt.c_str(), txt.length(), '?',
                         NULL);
    //        font->DrawTextString(drawbuf, x+1, y+1, txt.c_str(), txt.length(),
