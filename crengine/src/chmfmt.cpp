...
struct crChmExternalFileStream : public chmExternalFileStream {
  ...
  // LONGINT64 (read)( chmExternalFileStream * instance, unsigned char * buf,
  ...
  static LONGINT64 cr_read(chmExternalFileStream *instance, unsigned char *buf,
    ...
    if (((crChmExternalFileStream *)instance)->stream->SetPos((lvpos_t)pos) !=
        pos)
      return 0;
    if (((crChmExternalFileStream *)instance)
            ->stream->Read(buf, (lvsize_t)len, &bytesRead) != LVERR_OK)
      return false;
    return bytesRead;
  }
  /** closes file */
  ...
  crChmExternalFileStream(LVStreamRef s) : stream(s) {
    open = cr_open;
    read = cr_read;
    close = cr_close;
  }
...
public:
  LVCHMStream(chmFile *file) : _file(file), m_pos(0), m_size(0) {}
  ...
  */
  ...
  virtual lverror_t Read(void *buf, lvsize_t count, lvsize_t *nBytesRead) {
    ...
    if (cnt <= 0)
      return LVERR_FAIL;
    LONGINT64 gotBytes =
        chm_retrieve_object(_file, &m_ui, (unsigned char *)buf, m_pos, cnt);
...
public:
  ...
  static int CHM_ENUMERATOR_CALLBACK(struct chmFile * /*h*/,
                                     struct chmUnitInfo *ui, void *context) {
  ...
  bool open() {
    ...
    if (!_file)
      return false;
    chm_enumerate(_file, CHM_ENUMERATE_ALL, CHM_ENUMERATOR_CALLBACK, this);
    return true;
  }
};
...
public:
  ...
  bool setPos(int offset) { return (int)_stream->SetPos(offset) == offset; }
  ...
  int bytesLeft() { return (int)(_stream->GetSize() - _stream->GetPos()); }
  ...
  bool decodeBlock(const lUInt8 *ptr, lUInt32 blockOffset, int size) {
    ...
    while (data + 8 < maxdata) {
      ...
      if (data < maxdata) { // urlOffset > offset ) {
        ...
        // CRLog::trace("urlstr[offs=%x, url=%s]", item->offset,
        // item->url.c_str());
        _table.add(item);
      }
    }
...
public:
  ...
  CHMUrlTableEntry() : offset(0), id(0), topicsIndex(0), urlStrOffset(0) {}
};
...
class CHMUrlTable {
  ...
  CHMUrlTable(LVContainerRef container, LVStreamRef stream)
      : _container(container), _reader(stream), _strings(NULL) {}
  lUInt32 readInt32(const lUInt8 *&data) {
    lUInt32 res = 0;
    res = *(data++);
    res = res | (((lUInt32)(*(data++))) << 8);
  ...
  bool decodeBlock(const lUInt8 *data, lUInt32 offset, int size) {
    for (int i = 0; i < URLTBL_BLOCK_RECORD_COUNT && size > 0; i++) {
      ...
      // CRLog::trace("urltbl[offs=%x, id=%x, ti=%x, urloffs=%x]", item->offset,
      // item->id, item->topicsIndex, item->urlStrOffset);
      _table.add(item);
      offset += 4 * 3;
      size -= 4 * 3;
    }
...
class CHMSystem {
  ...
  CHMSystem(LVContainerRef container, LVStreamRef stream)
      : _container(container), _reader(stream), _fileVersion(0), _lcid(0),
        _dbcs(false), _fullTextSearch(false), _hasKLinks(false),
        _hasALinks(false), _binaryIndexURLTableId(0), _binaryTOCURLTableId(0),
        _enc_table(NULL), _urlTable(NULL) {}

  bool decodeEntry() {
    bool err = false;
    int code = _reader.readInt16(err);
    int length = _reader.readInt16(err);
    // CRLog::trace("CHM binary item code=%d, length=%d, bytesLeft=%d", code,
    // length, _reader.bytesLeft());
    if (err)
      return false;
    LVArray<lUInt8> bytes;
    switch (code) {
    ...
    case 4: {
      ...
      if (enc_name != NULL) {
        ...
        CRLog::info("CHM LCID: %08x -- cannot find charset encoding table",
                    _lcid);
      }
    ...
    case 16:
      ...
      if (_enc_table == NULL) {
        for (int i = _defaultFont.length() - 1; i > 0; i--) {
          if (_defaultFont[i] == ',') {
            int cs = _defaultFont.substr(i + 1, _defaultFont.length() - i - 1)
                         .atoi();
            const lChar16 *cpname = NULL;
            switch (cs) {
            case 0x00:
              cpname = L"windows-1252";
              break;
            case 0xCC:
              cpname = L"windows-1251";
              break;
            case 0xEE:
              cpname = L"windows-1250";
              break;
            case 0xA1:
              cpname = L"windows-1253";
              break;
            case 0xA2:
              cpname = L"windows-1254";
              break;
            case 0xBA:
              cpname = L"windows-1257";
              break;
            case 0xB1:
              cpname = L"windows-1255";
              break;
            case 0xB2:
              cpname = L"windows-1256";
              break;
            default:
              break;
            }
  ...
  bool read() {
    ...
    if (err) {
      CRLog::error(
          "CHM decoding error: %d blocks decoded, stream bytes left=%d", count,
          _reader.bytesLeft());
      return false;
    }
    if (_enc_table == NULL) {
...
public:
  ...
  lString16 getTitle() { return decodeString(_title); }

  lString16 getLanguage() { return decodeString(_language); }

  lString16 getDefaultTopic() { return decodeString(_defaultTopic); }

  lString16 getEncodingName() { return _enc_name; }
...
ldomDocument *LVParseCHMHTMLStream(LVStreamRef stream,
  ...
  writerFilter.setFlags(writerFilter.getFlags() |
                        TXTFLG_CONVERT_8BIT_ENTITY_ENCODING);
...
public:
  CHMTOCReader(LVContainerRef cont, ldomDocument *doc,
  ...
  void addFile(const lString16 &v1) {
    ...
    _appender->addPathSubstitution(v1, cs16("_doc_fragment_") +
                                           fmt::decimal(_fileList.length()));
    _appender->setCodeBase(v1);
  }

  void addTocItem(lString16 name, lString16 url, int level) {
    // CRLog::trace("CHM toc level %d: '%s' : %s", level, LCSTR(name),
    // LCSTR(url) );
    if (url.startsWith(".."))
  ...
  bool init(LVContainerRef cont, lString16 hhcName, lString16 defEncodingName,
    ...
    if (hhcName.empty()) {
      ...
      ldomNode *body =
          doc->getRootNode(); // doc->createXPointer(cs16("/html[1]/body[1]"));
      bool res = false;
      if (body->isElement()) {
        // body element
        recurseToc(body, 0);
...
bool ImportCHMDocument(LVStreamRef stream, ldomDocument *doc,
  ...
  CRLog::info("CHM: toc=%s, enc=%s, title=%s", LCSTR(tocFileName),
              LCSTR(defEncodingName), LCSTR(title));
  //
  ...
  ldomDocumentFragmentWriter appender(
      &writer, cs16("body"), cs16("DocFragment"), lString16::empty_str);
