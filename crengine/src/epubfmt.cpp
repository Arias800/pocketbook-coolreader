...
//             LVStreamRef out = LVOpenFileStream(outFileName.c_str(),
//             LVOM_WRITE); if ( !in.isNull() && !out.isNull() ) {
...
void ReadEpubToc(ldomDocument *doc, ldomNode *mapRoot, LVTocItem *baseToc,
  ...
  if (!mapRoot || !baseToc)
    return;
  lUInt16 navPoint_id =
      mapRoot->getDocument()->getElementNameIndex(L"navPoint");
  lUInt16 navLabel_id =
      mapRoot->getDocument()->getElementNameIndex(L"navLabel");
  ...
  for (int i = 0; i < 5000; i++) {
    ...
    if (!navPoint)
      break;
    ldomNode *navLabel =
        navPoint->findChildElement(LXML_NS_ANY, navLabel_id, -1);
    if (!navLabel)
      continue;
    ...
    if (href.empty() || href[0] != '#')
      continue;
    ldomNode *target =
        doc->getNodeById(doc->getAttrValueIndex(href.substr(1).c_str()));
    if (!target)
      continue;
...
lString16 EpubGetRootFilePath(LVContainerRef m_arc) {
  ...
  // read container.xml
  {
    LVStreamRef container_stream =
        m_arc->OpenStream(L"META-INF/container.xml", LVOM_READ);
    if (!container_stream.isNull()) {
      ldomDocument *doc = LVParseXMLStream(container_stream);
      if (doc) {
        ldomNode *rootfile =
            doc->nodeFromXPath(cs16("container/rootfiles/rootfile"));
...
/// encrypted font demangling proxy: XORs first 1024 bytes of source stream with
/// key
...
public:
  ...
  virtual const lChar16 *GetName() { return _container->GetName(); }
  /// sets stream/container name, may be not implemented for some objects
  virtual void SetName(const lChar16 *name) { _container->SetName(name); }

  virtual void addEncryptedItem(EncryptedItem *item) { _list.add(item); }
  ...
  bool open() {
    LVStreamRef stream =
        _container->OpenStream(L"META-INF/encryption.xml", LVOM_READ);
    if (stream.isNull())
      return false;
...
LVStreamRef GetEpubCoverpage(LVContainerRef arc) {
  ...
  LVStreamRef content_stream =
      m_arc->OpenStream(rootfilePath.c_str(), LVOM_READ);
  ...
  // reading content stream
  {
    ...
    for (int i = 1; i < 20; i++) {
      ldomNode *item = doc->nodeFromXPath(lString16("package/metadata/meta[")
                                          << fmt::decimal(i) << "]");
      if (!item)
        break;
    ...
    for (int i = 1; i < 50000; i++) {
      ldomNode *item = doc->nodeFromXPath(lString16("package/manifest/item[")
                                          << fmt::decimal(i) << "]");
      if (!item)
        break;
      ...
      if (!href.empty() && !id.empty()) {
        if (id == coverId) {
          ...
          coverPageImageStream =
              m_arc->OpenStream(coverFileName.c_str(), LVOM_READ);
        }
      }
    }
    delete doc;
...
public:
  ...
  void onToken(char token) {
    ...
    case '}':
      if (_state == 2) {
        if (!_url.empty()) {
          //                    CRLog::trace("@font { face: %s; bold: %s;
          //                    italic: %s; url: %s", _face.c_str(), _bold ?
          //                    "yes" : "no",
          //                                 _italic ? "yes" : "no",
          //                                 LCSTR(_url));
          if (islocal.length() == 5)
...
bool ImportEpubDocument(LVStreamRef stream, ldomDocument *m_doc,
  ...
  // reading content stream
  {
    ...
    //        // for debug
    //        {
    //            LVStreamRef out = LVOpenFileStream("/tmp/content.xml",
    //            LVOM_WRITE); doc->saveToStream(out, NULL, true);
    //        }
      ...
      if (!href.empty() && !id.empty()) {
        ...
        if (id == coverId) {
          ...
          if (!stream.isNull()) {
            ...
            if (!img.isNull()) {
              CRLog::info("EPUB coverpage image is correct: %d x %d",
    ...
    if (epubItems.length() > 0) {
      ...
      if (spine) {
        ...
        EpubItem *ncx =
            epubItems.findById(spine->getAttributeValue("toc")); // TODO
        ...
        for (int i = 1; i < 50000; i++) {
          ...
          if (!item)
            break;
          EpubItem *epubItem =
              epubItems.findById(item->getAttributeValue("idref"));
          if (epubItem) {
...
//abbr, acronym, address, blockquote, br, cite, code, dfn, div, em, h1, h2, h3, h4, h5, h6, kbd, p, pre, q, samp, span, strong, var
  ...
  // m_doc->saveToStream(LVOpenFileStream("/tmp/epub_dump.xml", LVOM_WRITE),
  // NULL, true);
