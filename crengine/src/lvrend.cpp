...
public:
  ...
  // simpleLogFile & operator << ( int d ) { fprintf( f, "%d(0x%X) ", d, d );
  // fflush( f ); return *this; }
  ...
  simpleLogFile &operator<<(const lString16 &str) {
...
public:
  ...
  ldomNode *elem;
  CCRTableCell()
      : col(NULL), row(NULL), width(0), height(0), percent(0), txtlen(0),
        colspan(1), rowspan(1), padding_left(0), padding_right(0),
        padding_top(0), padding_bottom(0), halign(0), valign(0), elem(NULL) {}
};
...
public:
  ...
  CCRTableRow()
      : index(0), height(0), y(0), numcols(0) // sum of colspan
        ,
        linkindex(-1), elem(NULL), rowgroup(NULL) {}
};
...
public:
  ...
  CCRTableRowGroup() : index(0), height(0), y(0), elem(NULL) {}
};
...
public:
  ...
  ldomNode *elem;
  CCRTableCol()
      : index(0), width(0), percent(0), txtlen(0), nrows(0),
...
public:
  ...
  int LookupElem(ldomNode *el, int state) {
    ...
    for (int i = 0; i < el->getChildCount(); i++) {
      ...
      if (item) {
        ...
        // CRLog::trace("LookupElem[%d] (%s, %d) %d", i,
        // LCSTR(item->getNodeName()), state, (int)item->getRendMethod() );
        switch (rendMethod) {
        ...
        case erm_list_item:
        case erm_block: // render as block element (render as containing other
                        // elements)
        case erm_final: // final element: render the whole it's content as
                        // single render block
        case erm_mixed: // block and inline elements are mixed: autobox inline
                        // portions of nodes; TODO
        case erm_table_cell: // table cell
        {
          ...
          int cs =
              StrToIntPercent(item->getAttributeValue(attr_colspan).c_str());
          if (cs > 0 && cs < 100) {
            cell->colspan = cs;
          } else {
            cs = 1;
          ...
          int rs =
              StrToIntPercent(item->getAttributeValue(attr_rowspan).c_str());
          if (rs > 0 && rs < 100) {
            cell->rowspan = rs;
          } else {
            rs = 1;
  ...
  void PlaceCells() {
    ...
    for (i = 0; i < rows.length(); i++) {
      ...
      for (j = 0; j < rows[i]->cells.length(); j++) {
        ...
        // int rs = cell->rowspan;
        while (x < cols.length() &&
               cols[x]->nrows > i) { // find free cell position
          x++;
        ...
        // update col width
        if (cell->colspan == 1) {
          if (cell->width > 0 && cell->col->width < cell->width &&
              cell->col->percent == 0) {
            cell->col->width = cell->width;
          } else if (cell->percent > 0 && cell->col->width == 0 &&
    ...
    for (i = 0; i < rows.length(); i++) {
      for (j = 0; j < rows[i]->cells.length(); j++) {
        ...
        cell->padding_left =
            (short)lengthToPx(cell->elem->getStyle()->padding[0], width, em);
        cell->padding_right =
            (short)lengthToPx(cell->elem->getStyle()->padding[1], width, em);
        cell->padding_top =
            (short)lengthToPx(cell->elem->getStyle()->padding[2], width, em);
        cell->padding_bottom =
            (short)lengthToPx(cell->elem->getStyle()->padding[3], width, em);
      }
    }
    // update col x
  ...
  int renderCells(LVRendPageContext &context) {
    ...
    if (caption) {
      ...
      int padding_right =
          lengthToPx(caption->getStyle()->padding[1], width, em);
      int padding_top = lengthToPx(caption->getStyle()->padding[2], width, em);
      int padding_bottom =
          lengthToPx(caption->getStyle()->padding[3], width, em);
      LFormattedTextRef txform;
      caption_h = caption->renderFinalBlock(txform, &fmt,
                                            w - padding_left - padding_right) +
                  padding_top + padding_bottom;
      fmt.setY(TABLE_BORDER_WIDTH); // cell->padding_top ); //cell->row->y -
                                    // cell->row->y );
      ...
      for (j = 0; j < rows[i]->cells.length(); j++) {
        ...
        if (i == y) {
          ...
          if (cell->elem->getRendMethod() == erm_final) {
            LFormattedTextRef txform;
            int h = cell->elem->renderFinalBlock(
                txform, &fmt,
                cell->width - cell->padding_left - cell->padding_right);
            ...
            fmt.setWidth(
                cell->width); //  - cell->padding_left - cell->padding_right
            fmt.setHeight(
                cell->height); // - cell->padding_top - cell->padding_bottom
    ...
    for (i = 0; i < rows.length(); i++) {
      ...
      for (j = 0; j < rows[i]->cells.length(); j++) {
        ...
        if (i == y) {
          ...
          // cell->padding_left - cell->padding_right );
          CCRTableRow *lastrow = rows[cell->row->index + cell->rowspan - 1];
          fmt.setHeight(
              lastrow->height + lastrow->y -
              cell->row->y); // - cell->padding_top - cell->padding_bottom
        }
      }
    }
    ...
    if (context.getPageList() != NULL) {
      // int break_before = CssPageBreak2Flags(
      // node->getStyle()->page_break_before ); int break_after =
      // CssPageBreak2Flags( node->getStyle()->page_break_after ); int
      // break_inside = CssPageBreak2Flags( node->getStyle()->page_break_inside
      // );
      ...
      for (int i = 0; i < count; i++) {
        ...
        int y1 =
            rect.top + row->y + row->height + TABLE_BORDER_WIDTH; // end of row
        if (i == count - 1) {
...
void freeFormatData(ldomNode *node) { node->clearRenderData(); }
...
int LVRendGetFontEmbolden() { return rend_font_embolden; }

LVFontRef getFont(css_style_rec_t *style, int documentId) {
  int sz = style->font_size.value;
  if (style->font_size.type != css_val_px &&
  ...
  LVFontRef fnt = fontMan->GetFont(
      sz, fw, style->font_style == css_fs_italic, style->font_family,
      lString8(style->font_name.c_str()), documentId);
...
// Render final block
//=======================================================================
void renderFinalBlock(ldomNode *enode, LFormattedText *txform,
                      RenderRectAccessor *fmt, int &baseflags, int ident,
                      int line_h) {
  if (enode->isElement()) {
    lvdom_element_render_method rm = enode->getRendMethod();
    if (rm == erm_invisible ||
        (enode->getNodeId() == el_a && 0 == enode->getChildCount()))
    ...
    if (rm == erm_list_item) {
      ...
      ListNumberingPropsRef listProps =
          enode->getDocument()->getNodeNumberingProps(
              enode->getParentNode()->getDataIndex());
      if (listProps.isNull()) {
        int counterValue = 0;
        ...
        for (int i = 0; i < parent->getChildCount(); i++) {
          ...
          if (child &&
              child->getNodeListMarker(counterValue, marker, markerWidth)) {
            if (markerWidth > maxWidth)
              maxWidth = markerWidth;
          }
        }
        listProps = ListNumberingPropsRef(
            new ListNumberingProps(counterValue, maxWidth));
        enode->getDocument()->setNodeNumberingProps(
            enode->getParentNode()->getDataIndex(), listProps);
      }
      int counterValue = 0;
      if (enode->getNodeListMarker(counterValue, marker, marker_width)) {
        ...
        lUInt32 cl = style->color.type != css_val_color ? 0xFFFFFFFF
                                                        : style->color.value;
        ...
        if (sp == css_lsp_outside)
          margin = -marker_width;
        marker += "\t";
        txform->AddSourceLine(marker.c_str(), marker.length(), cl, bgcl, font,
                              flags | LTEXT_FLAG_OWNTEXT, line_h, margin, NULL);
    ...
    //        if ( ntype ) {
    //            CRLog::trace("Node %s is Object ?  %d",
    //            LCSTR(enode->getNodeName()), ntype->is_object );
    //        } else {
    //            CRLog::trace("Node %s (%d) has no css_elem_def_props_t",
    //            LCSTR(enode->getNodeName()), enode->getNodeId() );
    //        }
    if (ntype && ntype->is_object) {
      ...
      if (isBlock) {
        int flags = styleToTextFmtFlags(enode->getStyle(), baseflags);
        // txform->AddSourceLine(L"title", 5, 0x000000, 0xffffff, font,
        // baseflags, interval, margin, NULL, 0, 0);
        LVFont *font = enode->getFont().get();
        lUInt32 cl = style->color.type != css_val_color ? 0xFFFFFFFF
                                                        : style->color.value;
        lUInt32 bgcl = style->background_color.type != css_val_color
                           ...
                           : style->background_color.value;
        lString16 title;
        // txform->AddSourceLine( title.c_str(), title.length(), cl, bgcl, font,
        // LTEXT_FLAG_OWNTEXT|LTEXT_FLAG_NEWLINE, line_h, 0, NULL ); baseflags
        title = enode->getAttributeValue(attr_suptitle);
        if (!title.empty()) {
          lString16Collection lines;
          lines.parse(title, cs16("\\n"), true);
          for (int i = 0; i < lines.length(); i++)
            txform->AddSourceLine(lines[i].c_str(), lines[i].length(), cl, bgcl,
                                  font, flags | LTEXT_FLAG_OWNTEXT, line_h, 0,
                                  NULL);
        }
      ...
      if (thisIsRunIn) {
        ...
        lUInt32 bgcl = style->background_color.type != css_val_color
                           ? 0xFFFFFFFF
                           : style->background_color.value;
        lChar16 delimiter[] = {UNICODE_NO_BREAK_SPACE,
                               UNICODE_NO_BREAK_SPACE}; // 160
        txform->AddSourceLine(
            delimiter, sizeof(delimiter) / sizeof(lChar16), cl, bgcl, font,
            LTEXT_FLAG_OWNTEXT | LTEXT_RUNIN_FLAG, line_h, 0, NULL);
        flags &= ~LTEXT_RUNIN_FLAG;
      }
    }
    ...
    logfile << "<" << enode->getNodeName() << ">     flags( " << baseflags
            << "-> " << flags << ")  rect( " << rect.left << rect.top
    ...
    if (!txt.empty()) {
      ...
      lUInt32 cl =
          style->color.type != css_val_color ? 0xFFFFFFFF : style->color.value;
      lUInt32 bgcl = style->background_color.type != css_val_color
                         ? 0xFFFFFFFF
                         : style->background_color.value;
      ...
      */
      ...
      if (txt.length() > 0)
        txform->AddSourceLine(txt.c_str(), txt.length(), cl, bgcl, font,
                              baseflags | tflags, line_h, ident, enode, 0,
                              letter_spacing);
      baseflags &= ~LTEXT_FLAG_NEWLINE; // clear newline flag
    }
  } else {
...
void getPageBreakStyle(ldomNode *el, css_page_break_t &before,
                       css_page_break_t &inside, css_page_break_t &after) {
  ...
  while (el) {
    ...
    if ((!firstChild || before != css_pb_auto) &&
        (!lastChild || after != css_pb_auto) && inside != css_pb_auto)
      return;
    ldomNode *parent = el->getParentNode();
    if (!parent)
      return;
...
int renderBlockElement(LVRendPageContext &context, ldomNode *enode, int x,
                       int y, int width) {
  if (enode->isElement()) {
    bool isFootNoteBody = false;
    if (enode->getNodeId() == el_section &&
      ...
      if (body) {
        if (body->getAttributeValue(attr_name) == "notes" ||
            body->getAttributeValue(attr_name) == "comments")
          if (!enode->getAttributeValue(attr_id).empty())
            isFootNoteBody = true;
      }
    }
    //        if ( isFootNoteBody )
    //            CRLog::trace("renderBlockElement() : Footnote body detected!
    //            %s", LCSTR(ldomXPointer(enode,0).toString()) );
    // if (!fmt)
    //    crFatalError();
    ...
    int margin_left =
        lengthToPx(enode->getStyle()->margin[0], width, em) + DEBUG_TREE_DRAW;
    int margin_right =
        lengthToPx(enode->getStyle()->margin[1], width, em) + DEBUG_TREE_DRAW;
    int margin_top =
        lengthToPx(enode->getStyle()->margin[2], width, em) + DEBUG_TREE_DRAW;
    int margin_bottom =
        lengthToPx(enode->getStyle()->margin[3], width, em) + DEBUG_TREE_DRAW;
    int padding_left =
        lengthToPx(enode->getStyle()->padding[0], width, em) + DEBUG_TREE_DRAW;
    int padding_right =
        lengthToPx(enode->getStyle()->padding[1], width, em) + DEBUG_TREE_DRAW;
    int padding_top =
        lengthToPx(enode->getStyle()->padding[2], width, em) + DEBUG_TREE_DRAW;
    int padding_bottom =
        lengthToPx(enode->getStyle()->padding[3], width, em) + DEBUG_TREE_DRAW;
    ...
    LFormattedTextRef txform;
    {
      ...
      case erm_block: {
        ...
        if (y < st_y)
          y = st_y;
        fmt.setHeight(y +
                      padding_bottom); //+ margin_top + margin_bottom ); //???
        if (isFootNoteBody)
          context.leaveFootNote();
        return y + margin_top + margin_bottom +
               padding_bottom; // return block height
      } break;
      case erm_list_item:
      case erm_final:
      case erm_table_cell: {
        ...
        //     CRLog::trace("rendering final node: %s %d %s",
        //     LCSTR(enode->getNodeName()), enode->getDataIndex(),
        //     LCSTR(ldomXPointer(enode,0).toString()) );
        h = enode->renderFinalBlock(txform, &fmt,
                                    width - padding_left - padding_right);
        context.updateRenderProgress(1);
        // if ( context.updateRenderProgress(1) )
        //    CRLog::trace("last rendered node: %s %d",
        //    LCSTR(enode->getNodeName()), enode->getDataIndex());
#ifdef DEBUG_DUMP_ENABLED
        logfile << "\n";
#endif
    ...
    if (flgSplit) {
      ...
      if (context.getPageList() != NULL) {
        ...
        //                if (before!=css_pb_auto) {
        //                    CRLog::trace("page break before node %s class=%s
        //                    text=%s", LCSTR(enode->getNodeName()),
        //                    LCSTR(enode->getAttributeValue(L"class")),
        //                    LCSTR(enode->getText(' ', 120) ));
        //                }
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (!isFootNoteBody &&
              enode->getDocument()->getDocFlag(
                  DOC_FLAG_ENABLE_FOOTNOTES)) { // disable footnotes for
                                                // footnotes
            for (int w = 0; w < line->word_count; w++) {
              ...
              if (line->words[w].flags & LTEXT_WORD_IS_LINK_START) {
                const src_text_fragment_t *src =
                    txform->GetSrcInfo(line->words[w].src_text_index);
                if (src && src->object) {
                  ldomNode *node = (ldomNode *)src->object;
                  ldomNode *parent = node->getParentNode();
                  if (parent->getNodeId() == el_a &&
                      parent->hasAttribute(LXML_NS_ANY, attr_href) &&
                      parent->getAttributeValue(LXML_NS_ANY, attr_type) ==
                          "note") {
                    lString16 href =
                        parent->getAttributeValue(LXML_NS_ANY, attr_href);
...
void DrawDocument(LVDrawBuf &drawbuf, ldomNode *enode, int x0, int y0, int dx,
                  int dy, int doc_x, int doc_y, int page_height,
                  ldomMarkedRangeList *marks, ldomMarkedRangeList *bookmarks) {
  if (enode->isElement()) {
    RenderRectAccessor fmt(enode);
    doc_x += fmt.getX();
    doc_y += fmt.getY();
    ...
    int padding_left =
        !draw_padding_bg
            ? 0
            : lengthToPx(enode->getStyle()->padding[0], width, em) +
                  DEBUG_TREE_DRAW;
    int padding_right =
        !draw_padding_bg
            ? 0
            : lengthToPx(enode->getStyle()->padding[1], width, em) +
                  DEBUG_TREE_DRAW;
    int padding_top =
        !draw_padding_bg
            ? 0
            : lengthToPx(enode->getStyle()->padding[2], width, em) +
                  DEBUG_TREE_DRAW;
    // int padding_bottom = !draw_padding_bg ? 0 : lengthToPx(
    // enode->getStyle()->padding[3], width, em ) + DEBUG_TREE_DRAW;
    if ((doc_y + height <= 0 || doc_y > 0 + dy) &&
        (enode->getRendMethod() != erm_table_row &&
         enode->getRendMethod() != erm_table_row_group)) // 0~=y0
    {
      return; // out of range
    }
    ...
    if (bg.type == css_val_color) {
      ...
      drawbuf.FillRect(x0 + doc_x, y0 + doc_y, x0 + doc_x + fmt.getWidth(),
                       y0 + doc_y + fmt.getHeight(), bg.value);
    }
#if (DEBUG_TREE_DRAW != 0)
    lUInt32 color;
    static lUInt32 const colors2[] = {0x555555, 0xAAAAAA, 0x555555, 0xAAAAAA,
                                      0x555555, 0xAAAAAA, 0x555555, 0xAAAAAA};
    static lUInt32 const colors4[] = {0x555555, 0xFF4040, 0x40FF40, 0x4040FF,
                                      0xAAAAAA, 0xFF8000, 0xC0C0C0, 0x808080};
    ...
    case erm_block: {
      ...
      for (int i = 0; i < cnt; i++) {
        ldomNode *child = enode->getChildNode(i);
        DrawDocument(drawbuf, child, x0, y0, dx, dy, doc_x, doc_y, page_height,
                     marks, bookmarks); //+fmt->getX() +fmt->getY()
      }
#if (DEBUG_TREE_DRAW != 0)
      drawbuf.FillRect(doc_x + x0, doc_y + y0, doc_x + x0 + fmt.getWidth(),
                       doc_y + y0 + 1, color);
      drawbuf.FillRect(doc_x + x0, doc_y + y0, doc_x + x0 + 1,
                       doc_y + y0 + fmt.getHeight(), color);
      drawbuf.FillRect(doc_x + x0 + fmt.getWidth() - 1, doc_y + y0,
                       doc_x + x0 + fmt.getWidth(),
                       doc_y + y0 + fmt.getHeight(), color);
      ...
      bool needBorder = enode->getRendMethod() == erm_table ||
                        enode->getStyle()->display == css_d_table_cell;
      if (needBorder) {
        drawbuf.FillRect(doc_x + x0, doc_y + y0, doc_x + x0 + fmt.getWidth(),
                         doc_y + y0 + 1, tableBorderColor);
        drawbuf.FillRect(doc_x + x0, doc_y + y0, doc_x + x0 + 1,
                         doc_y + y0 + fmt.getHeight(), tableBorderColor);
        drawbuf.FillRect(doc_x + x0 + fmt.getWidth() - 1, doc_y + y0,
                         doc_x + x0 + fmt.getWidth(),
                         doc_y + y0 + fmt.getHeight(), tableBorderColorDark);
        drawbuf.FillRect(doc_x + x0, doc_y + y0 + fmt.getHeight() - 1,
                         doc_x + x0 + fmt.getWidth(),
                         doc_y + y0 + fmt.getHeight(), tableBorderColorDark);
      }
    ...
    case erm_table_caption: {
      ...
      LFormattedTextRef txform;
      enode->renderFinalBlock(txform, &fmt,
                              fmt.getWidth() - padding_left - padding_right);
      fmt.push();
      {
        lvRect rc;
        enode->getAbsRect(rc);
        ...
        if (marks && marks->length()) {
          ...
          txform->Draw(&drawbuf, doc_x + x0 + padding_left,
                       doc_y + y0 + padding_top, &nmarks, nbookmarks);
        } else {
          txform->Draw(&drawbuf, doc_x + x0 + padding_left,
                       doc_y + y0 + padding_top, marks, nbookmarks);
        }
        if (nbookmarks)
          delete nbookmarks;
      }
      ...
      if (needBorder) {
        ...
        drawbuf.FillRect(doc_x + x0, doc_y + y0 + fmt.getHeight() - 1,
                         ...
                         doc_y + y0 + fmt.getHeight(), tableBorderColorDark);
        // drawbuf.FillRect( doc_x+x0, doc_y+y0, doc_x+x0+fmt->getWidth(),
        // doc_y+y0+1, tableBorderColorDark ); drawbuf.FillRect( doc_x+x0,
        // doc_y+y0, doc_x+x0+1, doc_y+y0+fmt->getHeight(), tableBorderColorDark
        // ); drawbuf.FillRect( doc_x+x0+fmt->getWidth()-1, doc_y+y0,
        // doc_x+x0+fmt->getWidth(), doc_y+y0+fmt->getHeight(), tableBorderColor
        // ); drawbuf.FillRect( doc_x+x0, doc_y+y0+fmt->getHeight()-1,
        // doc_x+x0+fmt->getWidth(), doc_y+y0+fmt->getHeight(), tableBorderColor
        // );
      }
...
inline void spreadParent(css_length_t &val, css_length_t &parent_val,
  ...
  if (val.type == css_val_inherited ||
      (val.type == css_val_unspecified && inherited))
    val = parent_val;
}

void setNodeStyle(ldomNode *enode, css_style_ref_t parent_style,
                  LVFontRef parent_font) {
  ...
  if (enode->getDocument()->getDocFlag(DOC_FLAG_ENABLE_INTERNAL_STYLES) &&
...
#define UPDATE_LEN_FIELD(fld)                                                  \
  ...
  case css_val_em:                                                             \
    pstyle->fld.type = css_val_px;                                             \
    pstyle->fld.value =                                                        \
        parent_style->font_size.value * pstyle->fld.value / 256;               \
    break;                                                                     \
  default:                                                                     \
    pstyle->fld.type = css_val_px;                                             \
    pstyle->fld.value = 0;                                                     \
  ...
  // (pstyle->text_align==css_ta_inherit))
  //{
  ...
  case css_val_em: // value = em*256
    ...
    pstyle->font_size.value =
        parent_style->font_size.value * pstyle->font_size.value / 256;
    break;
  case css_val_percent:
    pstyle->font_size.type = css_val_px;
    pstyle->font_size.value =
        parent_style->font_size.value * pstyle->font_size.value / 100;
    break;
  case css_val_unspecified:
  case css_val_in: // 2.54 cm
  case css_val_cm:
...
int renderTable(LVRendPageContext &context, ldomNode *node, int x, int y,
                int width) {
  CR_UNUSED2(x, y);
