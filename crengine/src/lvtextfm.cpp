...
// disable CJK support since it breaks usual text formatting with floating
// punctuation and space trunctaion turned on
#define CJK_PATCH 0
...
formatted_line_t *lvtextAllocFormattedLine() {
  formatted_line_t *pline =
      (formatted_line_t *)malloc(sizeof(formatted_line_t));
  memset(pline, 0, sizeof(formatted_line_t));
  return pline;
}

formatted_line_t *lvtextAllocFormattedLineCopy(formatted_word_t *words,
                                               int word_count) {
  formatted_line_t *pline =
      (formatted_line_t *)malloc(sizeof(formatted_line_t));
  memset(pline, 0, sizeof(formatted_line_t));
  lUInt32 size =
      (word_count + FRM_ALLOC_SIZE - 1) / FRM_ALLOC_SIZE * FRM_ALLOC_SIZE;
...
formatted_word_t *lvtextAddFormattedWord(formatted_line_t *pline) {
  int size = (pline->word_count + FRM_ALLOC_SIZE - 1) / FRM_ALLOC_SIZE *
             FRM_ALLOC_SIZE;
  if (pline->word_count >= size) {
    size += FRM_ALLOC_SIZE;
    pline->words = (formatted_word_t *)realloc(
        pline->words, sizeof(formatted_word_t) * (size));
  }
  return &pline->words[pline->word_count++];
}

formatted_line_t *lvtextAddFormattedLine(formatted_text_fragment_t *pbuffer) {
  int size = (pbuffer->frmlinecount + FRM_ALLOC_SIZE - 1) / FRM_ALLOC_SIZE *
             FRM_ALLOC_SIZE;
  if (pbuffer->frmlinecount >= size) {
    size += FRM_ALLOC_SIZE;
    pbuffer->frmlines = (formatted_line_t **)realloc(
        pbuffer->frmlines, sizeof(formatted_line_t *) * (size));
  }
  return (pbuffer->frmlines[pbuffer->frmlinecount++] =
              lvtextAllocFormattedLine());
}

formatted_line_t *lvtextAddFormattedLineCopy(formatted_text_fragment_t *pbuffer,
                                             formatted_word_t *words,
                                             int words_count) {
  int size = (pbuffer->frmlinecount + FRM_ALLOC_SIZE - 1) / FRM_ALLOC_SIZE *
             FRM_ALLOC_SIZE;
  if (pbuffer->frmlinecount >= size) {
  ...
  return (pbuffer->frmlines[pbuffer->frmlinecount++] =
              lvtextAllocFormattedLineCopy(words, words_count));
}

formatted_text_fragment_t *lvtextAllocFormatter(lUInt16 width) {
  formatted_text_fragment_t *pbuffer =
      (formatted_text_fragment_t *)malloc(sizeof(formatted_text_fragment_t));
  memset(pbuffer, 0, sizeof(formatted_text_fragment_t));
  pbuffer->width = width;
  int defMode = MAX_IMAGE_SCALE_MUL > 1
                    ? (ARBITRARY_IMAGE_SCALE_ENABLED == 1 ? 2 : 1)
                    : 0;
  int defMult = MAX_IMAGE_SCALE_MUL;
  pbuffer->img_zoom_in_mode_block =
      defMode; /**< can zoom in block images: 0=disabled, 1=integer scale,
                  2=free scale */
  pbuffer->img_zoom_in_scale_block =
      defMult; /**< max scale for block images zoom in: 1, 2, 3 */
  pbuffer->img_zoom_in_mode_inline =
      defMode; /**< can zoom in inline images: 0=disabled, 1=integer scale,
                  2=free scale */
  pbuffer->img_zoom_in_scale_inline =
      defMult; /**< max scale for inline images zoom in: 1, 2, 3 */
  pbuffer->img_zoom_out_mode_block =
      defMode; /**< can zoom out block images: 0=disabled, 1=integer scale,
                  2=free scale */
  pbuffer->img_zoom_out_scale_block =
      defMult; /**< max scale for block images zoom out: 1, 2, 3 */
  pbuffer->img_zoom_out_mode_inline =
      defMode; /**< can zoom out inline images: 0=disabled, 1=integer scale,
                  2=free scale */
  pbuffer->img_zoom_out_scale_inline =
      defMult; /**< max scale for inline images zoom out: 1, 2, 3 */
...
void lvtextAddSourceLine(
    ...
    lUInt16 offset, lInt8 letter_spacing) {
  int srctextsize = (pbuffer->srctextlen + FRM_ALLOC_SIZE - 1) /
                    FRM_ALLOC_SIZE * FRM_ALLOC_SIZE;
  if (pbuffer->srctextlen >= srctextsize) {
    srctextsize += FRM_ALLOC_SIZE;
    pbuffer->srctext = (src_text_fragment_t *)realloc(
        pbuffer->srctext, sizeof(src_text_fragment_t) * (srctextsize));
  }
  src_text_fragment_t *pline = &pbuffer->srctext[pbuffer->srctextlen++];
  pline->t.font = font;
  //    if (font) {
  //        // DEBUG: check for crash
  //        CRLog::trace("c font = %08x  txt = %08x", (lUInt32)font,
  //        (lUInt32)text);
...
void lvtextAddSourceObject(
    formatted_text_fragment_t *pbuffer, lUInt16 width, lUInt16 height,
    lUInt32 flags,   /* flags */
    lUInt8 interval, /* interline space, *16 (16=single, 32=double) */
    lUInt16 margin,  /* first line margin */
    void *object,    /* pointer to custom object */
...
void LFormattedText::AddSourceObject(
  ...
  lvtextAddSourceObject(m_pbuffer, width, height, flags, interval, margin,
                        object, letter_spacing);
}
...
#define ITEMS_RESERVED 16
    if (!m_staticBufs || m_length > STATIC_BUFS_SIZE - 1) {
      if (m_length + ITEMS_RESERVED > m_size) {
        ...
        m_text = (lChar16 *)realloc(m_staticBufs ? NULL : m_text,
                                    sizeof(lChar16) * m_size);
        m_flags = (lUInt8 *)realloc(m_staticBufs ? NULL : m_flags,
                                    sizeof(lUInt8) * m_size);
        m_charindex = (lUInt16 *)realloc(m_staticBufs ? NULL : m_charindex,
                                         sizeof(lUInt16) * m_size);
        m_srcs = (src_text_fragment_t **)realloc(m_staticBufs ? NULL : m_srcs,
                                                 sizeof(src_text_fragment_t *) *
                                                     m_size);
        m_widths = (int *)realloc(m_staticBufs ? NULL : m_widths,
                                  sizeof(int) * m_size);
      }
      m_staticBufs = false;
    } else {
  ...
  void resizeImage(int &width, int &height, int maxw, int maxh, bool isInline) {
    // CRLog::trace("Resize image (%dx%d) max %dx%d %s", width, height, maxw,
    // maxh, isInline ? "inline" : "block");
  ...
  void resizeImage(int &width, int &height, int maxw, int maxh,
                   bool arbitraryImageScaling, int maxScaleMult) {
    // CRLog::trace("Resize image (%dx%d) max %dx%d %s  *%d", width, height,
    // maxw, maxh, arbitraryImageScaling ? "arbitrary" : "integer",
    // maxScaleMult);
    if (maxScaleMult < 1)
      maxScaleMult = 1;
...
#define MAX_TEXT_CHUNK_SIZE 4096
    ...
    for (i = 0; i <= m_length; i++) {
      ...
      if (!lastFont)
        lastFont = newFont;
      if (i > start && (newFont != lastFont || isObject || prevCharIsObject ||
        ...
        if (m_charindex[i - 1] != OBJECT_CHAR_INDEX) {
          ...
          int len = i - start;
          int chars_measured =
              lastFont->measureText(m_text + start, len, widths, flags,
                                    0x7FFF, // pbuffer->width,
                                    // 300, //TODO
                                    '?', m_srcs[start]->letter_spacing, false);
          ...
          //                    lString16 buf;
          //                    for ( int k=0; k<len; k++ ) {
          //                        buf << L"_" << lChar16(m_text[start+k]) <<
          //                        L"_" << lString16::itoa(widths[k]);
          //                    }
          //                    TR("       %s", LCSTR(buf));
    ...
    //            buf << L" " << lChar16(m_text[i]) << L" " <<
    //            lString16::itoa(m_widths[i]);
    //        }
    //        TR("%s", LCSTR(buf));
  }
...
#define MAX_WORD_SIZE 64
  ...
  void addLine(int start, int end, int x, src_text_fragment_t *para,
               int interval, bool first, bool last, bool preFormattedOnly,
               bool needReduceSpace) {
    ...
    bool visualAlignmentEnabled =
        gFlgFloatingPunctuationEnabled != 0 &&
        (align == LTEXT_ALIGN_WIDTH || align == LTEXT_ALIGN_RIGHT);
    ...
    for (int i = start; i <= end; i++) {
      ...
      if (i > wstart && (newSrc != lastSrc || space || lastWord
        ...
        if (lastSrc->flags & LTEXT_SRC_IS_OBJECT) {
          ...
          resizeImage(width, height, m_pbuffer->width - x,
                      m_pbuffer->page_height, m_length > 1);
          word->width = width;
          ...
          int fhWithInterval =
              (fh * interval) >> 4; // font height + interline space
          int fhInterval =
              fhWithInterval -
              fh;     // interline space only (negative for intervals < 100%)
          int wy = 0; // fhInterval / 2;
          //                    if ( interval>16 )
          //                        wy = -((font->getSize() * (interval-16)) >>
          //                        4) >> 1;
          //                    else if ( interval<16 )
          //                        wy = ((font->getSize() * (16-interval)) >>
          //                        4) >> 1;
          if (vertical_align) {
          ...
          word->t.len = i - wstart;
          word->width = m_widths[i > 0 ? i - 1 : 0] -
                        (wstart > 0 ? m_widths[wstart - 1] : 0);
          word->min_width = word->width;
          word->max_width = word->width;
          TR("addLine - word(%d, %d) x=%d (%d..%d)[%d] |%s|", wstart, i,
             frmline->width, wstart > 0 ? m_widths[wstart - 1] : 0,
             m_widths[i - 1], word->width,
             LCSTR(lString16(m_text + wstart, i - wstart)));
          //                    lChar16 lastch = m_text[i-1];
          //                    if ( lastch==UNICODE_NO_BREAK_SPACE )
          //                        CRLog::trace("last char is
          ...
          if (m_flags[i - 1] & LCHAR_IS_SPACE) {
            ...
            if (!visualAlignmentEnabled && lastWord) {
              word->width = m_widths[i > 1 ? i - 2 : 0] -
                            (wstart > 0 ? m_widths[wstart - 1] : 0);
              word->min_width = word->width;
              word->max_width = word->width;
            }
          } else if (frmline->word_count > 1 &&
                     m_flags[wstart] & LCHAR_IS_SPACE) {
            // if ( word->t.len<2 || m_text[i-1]!=UNICODE_NO_BREAK_SPACE ||
            // m_text[i-2]!=UNICODE_NO_BREAK_SPACE)
            //                        if (
            //                        m_text[wstart]==UNICODE_NO_BREAK_SPACE &&
            //                        m_text[wstart+1]==UNICODE_NO_BREAK_SPACE)
            //                            CRLog::trace("Double nbsp
            //                            text[-1]=%04x", m_text[wstart-1]);
            //                        else
            frmline->words[frmline->word_count - 2].flags |=
          ...
          if (visualAlignmentEnabled && lastWord) {
            ...
            word->width += wAlign;
            while ((m_flags[endp] & LCHAR_IS_SPACE) &&
                   endp > 0) { // || lastc=='\r' || lastc=='\n'
              word->width -= m_widths[endp] - m_widths[endp - 1];
              endp--;
              lastc = m_text[endp];
            }
            if (word->flags & LTEXT_WORD_CAN_HYPH_BREAK_LINE_AFTER) {
              word->width -= font->getHyphenWidth(); // TODO: strange fix - need
                                                     // some other solution
            } else if (lastc == '.' || lastc == ',' || lastc == '!' ||
                       lastc == ':' || lastc == ';'
#if CJK_PATCH
                       || lastc == L'。' || lastc == L'，' || lastc == L'！' ||
                       lastc == L'：' || lastc == L'；' || lastc == L'”' ||
                       lastc == L'’' || lastc == L'」' || lastc == L'』'
#endif
            ) {
          ...
          h = fhWithInterval - b;
          //                    }  else  {
          //                        b = (( font->getBaseline() * interval) >>
          //                        4); h = ( ( font->getHeight() * interval) >>
          //                        4) - b;
          //                    }
        }
...
#define UNICODE_CJK_PUNCTUATION_HALF_AND_FULL_WIDTH_END 0xFFEE
  ...
  bool isCJKPunctuation(lChar16 c) {
    return (c >= UNICODE_CJK_PUNCTUATION_BEGIN &&
            c <= UNICODE_CJK_PUNCTUATION_END) ||
           (c >= UNICODE_GENERAL_PUNCTUATION_BEGIN &&
            c <= UNICODE_GENERAL_PUNCTUATION_END) ||
  ...
  void processParagraph(int start, int end) {
    ...
    // override align=left for this case
    ...
    for (; pos < m_length;) {
      ...
      int spaceReduceWidth =
          0; // max total line width which can be reduced by narrowing of spaces
      int firstCharMargin = getAdditionalCharWidthOnLeft(
          pos); // for first italic char with elements below baseline
      for (i = pos; i < m_length; i++) {
        if (x + m_widths[i] - w0 >
            maxWidth + spaceReduceWidth - firstCharMargin)
          break;
        ...
        if (flags & LCHAR_ALLOW_WRAP_AFTER || i == m_length - 1)
          ...
          lastHyphWrap = i;
        if (m_pbuffer->min_space_condensing_percent != 100 &&
            i < m_length - 1 && (m_flags[i] & LCHAR_IS_SPACE) &&
      ...
      int wordpos = i - 1;
      int normalWrapWidth =
          lastNormalWrap > 0 ? x + m_widths[lastNormalWrap] - w0 : 0;
      int deprecatedWrapWidth =
          lastDeprecatedWrap > 0 ? x + m_widths[lastDeprecatedWrap] - w0 : 0;
      ...
      if (lastMandatoryWrap < 0 && lastNormalWrap < m_length - 1 &&
          unusedPercent > 5 &&
          !(m_srcs[wordpos]->flags & LTEXT_SRC_IS_OBJECT) &&
          (m_srcs[wordpos]->flags & LTEXT_HYPHENATE)) {
        // hyphenate word
        int start, end;
...
#if TRACE_LINE_SPLITTING == 1
        if (len > 0) {
          CRLog::trace("wordBounds(%s) unusedSpace=%d wordWidth=%d",
                       LCSTR(lString16(m_text + start, len)), unusedSpace,
                       m_widths[end] - m_widths[start]);
          TR("wordBounds(%s) unusedSpace=%d wordWidth=%d",
             LCSTR(lString16(m_text + start, len)), unusedSpace,
             m_widths[end] - m_widths[start]);
        }
#endif
        if (start < end && start < wordpos && end >= lastNormalWrap &&
            len >= MIN_WORD_LEN_TO_HYPHENATE) {
          ...
          int max_width = maxWidth + spaceReduceWidth - x - (wordStart_w - w0) -
                          firstCharMargin;
          int _hyphen_width =
              ((LVFont *)m_srcs[wordpos]->t.font)->getHyphenWidth();
          if (HyphMan::hyphenate(m_text + start, len, widths, flags,
                                 _hyphen_width, max_width)) {
            for (int i = 0; i < len; i++)
      ...
      bool needReduceSpace =
          true; // todo: calculate whether space reducing required
      int endp = wrapPos + (lastMandatoryWrap < 0 ? 1 : 0);
#if CJK_PATCH
      int downSkipCount = 0;
      int upSkipCount = 0;
      if (endp > 1 && isCJKLeftPunctuation(*(m_text + endp))) {
        CRLog::trace("skip skip punctuation %s, at index %d",
                     LCSTR(lString16(m_text + endp, 1)), endp);
      } else if (endp > 1 && endp < m_length &&
                 isCJKLeftPunctuation(*(m_text + endp - 1))) {
        endp--;
        wrapPos--;
        CRLog::trace("up skip left punctuation %s, at index %d",
                     LCSTR(lString16(m_text + endp, 1)), endp);
        ...
        for (int epos = endp; epos >= start; epos++, downSkipCount++) {
          if (!isCJKPunctuation(*(m_text + epos)))
            break;
          // CRLog::trace("down skip punctuation %s, at index %d",
          // LCSTR(lString16(m_text + epos, 1)), epos);
        }
        for (int epos = endp; epos >= start; epos--, upSkipCount++) {
          if (!isCJKPunctuation(*(m_text + epos)))
            break;
          // CRLog::trace("up skip punctuation %s, at index %d",
          // LCSTR(lString16(m_text + epos, 1)), epos);
        }
      ...
      for (int k = endp - 1; k >= start; k--) {
        if (!((m_flags[k] & LCHAR_IS_SPACE) &&
              !(m_flags[k] & LCHAR_IS_OBJECT))) {
          lastnonspace = k;
          break;
        }
      }
      int dw = lastnonspace >= start
                   ? getAdditionalCharWidth(lastnonspace, lastnonspace + 1)
                   : 0;
      if (dw) {
        TR("additional width = %d, after char %s", dw,
           LCSTR(lString16(m_text + endp - 1, 1)));
        m_widths[lastnonspace] += dw;
      }
      addLine(pos, endp, x + firstCharMargin, para, interval, pos == 0,
              wrapPos >= m_length - 1, preFormattedOnly, needReduceSpace);
      pos = wrapPos + 1;
    }
  }
  ...
  void splitParagraphs() {
    ...
    //                TR("run-in found");
    //            TR("  %d: flg=%04x al=%d ri=%d '%s'", i, flg, (flg &
    //            LTEXT_FLAG_NEWLINE), (flg & LTEXT_RUNIN_FLAG)?1:0,
    //            (flg&LTEXT_SRC_IS_OBJECT ? "<image>" :
    //            LCSTR(lString16(m_pbuffer->srctext[i].t.text,
    //            m_pbuffer->srctext[i].t.len)) ) );
    //        }
    //        TR("============================");
    bool prevRunIn = m_pbuffer->srctextlen > 0 &&
                     (m_pbuffer->srctext[0].flags & LTEXT_RUNIN_FLAG);
    for (i = 1; i <= m_pbuffer->srctextlen; i++) {
      if ((i == m_pbuffer->srctextlen) ||
          ((m_pbuffer->srctext[i].flags & LTEXT_FLAG_NEWLINE) && !prevRunIn)) {
        processParagraph(start, i);
        start = i;
      }
      prevRunIn = (i < m_pbuffer->srctextlen) &&
                  (m_pbuffer->srctext[i].flags & LTEXT_RUNIN_FLAG);
    }
  }
...
void DrawBookmarkTextUnderline(LVDrawBuf &drawbuf, int x0, int y0, int x1,
                               int y1, int y, int flags,
                               text_highlight_options_t *options) {
  if (!(flags & (4 | 8)))
    return;
...
void LFormattedText::Draw(LVDrawBuf *buf, int x, int y,
  ...
  for (i = 0; i < m_pbuffer->frmlinecount; i++) {
    ...
    if (line_y + frmline->height >= clip.top) {
      ...
      // buf->FillRect( x+frmline->x, y + frmline->y, x+frmline->x +
      // frmline->width, y + frmline->y + frmline->height, bgcl );
          ...
          if (lastWordColor != bgcl || lastWordStart == -1) {
            if (lastWordStart != -1)
              if (((lastWordColor >> 24) & 0xFF) < 128)
                buf->FillRect(lastWordStart, y + frmline->y, lastWordEnd,
                              y + frmline->y + frmline->height, lastWordColor);
            lastWordColor = bgcl;
      ...
      if (marks != NULL && marks->length() > 0) {
        lvRect lineRect(frmline->x, frmline->y, frmline->x + frmline->width,
        ...
        for (int i = 0; i < marks->length(); i++) {
          ...
          if (range->intersects(lineRect, mark)) {
            //
            buf->FillRect(mark.left + x, mark.top + y, mark.right + x,
                          mark.bottom + y,
      ...
      if (bookmarks != NULL && bookmarks->length() > 0) {
        ...
        for (int i = 0; i < bookmarks->length(); i++) {
          ...
          if (range->intersects(lineRect, mark)) {
            //
            DrawBookmarkTextUnderline(*buf, mark.left + x, mark.top + y,
                                      mark.right + x, mark.bottom + y,
                                      mark.bottom + y - 2, range->flags,
      ...
      for (j = 0; j < frmline->word_count; j++) {
        ...
        if (word->flags & LTEXT_WORD_IS_OBJECT) {
          ...
          if ( srcFlags & LTEXT_BACKGROUND_MARK_FLAGS ) {
              ...
              rc.top = line_y + (frmline->baseline - font->getBaseline()) +
          word->y; rc.right = rc.left + word->width; rc.bottom = rc.top +
          font->getHeight(); buf->FillRect( rc.left, rc.top, rc.right,
          rc.bottom, 0xAAAAAA );
          }
          */
          ...
          font->DrawTextString(
              buf, x + frmline->x + word->x,
              line_y + (frmline->baseline - font->getBaseline()) + word->y, str,
              word->t.len, '?', NULL, flgHyphen, srcline->flags & 0x0F00,
      ...
      if (marks != NULL && marks->length() > 0) {
        ...
        for (int i = 0; i < marks->length(); i++) {
          ...
          if (range->intersects(lineRect, mark)) {
            ...
            buf->InvertRect(mark.left + x, mark.bottom + y - 2, mark.right + x,
                            mark.bottom + y + 1);
#else
#ifdef CR_INVERT_PRSERVE_GRAYS
            if (buf->GetBitsPerPixel() > 2) {
              buf->InvertRect(mark.left + x, mark.top + y, mark.right + x,
                              mark.bottom + y);
              buf->InvertRect(mark.left + x + 2, mark.top + y + 2,
                              mark.right + x - 2, mark.bottom + y - 2);
              /*                            lvRect markRc(mark.left + x,
                 mark.top + y, mark.right + x, mark.bottom + y); if (markRc.left
                 - 2 > clip.left) markRc.left -=1; markRc.left -=1; if
                 (markRc.right + 2 < clip.right) markRc.right += 2; // This is
                 some magic :) buf->Rect( markRc, 2, buf->GetTextColor()); */
            } else
