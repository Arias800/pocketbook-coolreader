...
void DrawBuf2DC(CWindowGc &dc, int x, int y, LVDrawBuf *buf,
                unsigned long *palette, int scale) {
  ...
  User::LeaveIfError(offScreenBitmap->Create(
      TSize(buf->GetWidth() * scale, buf->GetHeight() * scale), EColor64K));
    ...
    for (int yyi = 0; yyi < scale; yyi++) {
      if (buf->GetBitsPerPixel() == 32) {
        for (int xx = 0; xx < width; xx++) {
          ...
          TUint16 p16 = ((p32 & 0xF80000) >> 8) | ((p32 & 0x00FC00) >> 5) |
                        ((p32 & 0x0000F8) >> 3);
  ...
  dc.BitBlt(TPoint(0, 0),
            offScreenBitmap); //, TRect(x, y, buf->GetWidth()*scale,
                              // buf->GetHeight()*scale));
