...
ldomTextSelectionTool::ldomTextSelectionTool(
    ldomXRange &initialRange, ldomTextSelectionTool::interval_t initialInterval,
    ldomTextSelectionTool::origin_t initialOrigin)
    : _initialRange(initialRange), _currRange(initialRange),
      _currInterval(initialInterval), _currOrigin(initialOrigin) {}

/// moves selection, returns true if selection is moved, and it's necessary to
/// ensure positionToShow is visible on screen
bool ldomTextSelectionTool::moveBy(ldomTextSelectionTool::interval_t interval,
                                   ldomTextSelectionTool::direction_t dir,
                                   ldomTextSelectionTool::origin_t origin,
                                   int count, ldomXPointer &positionToShow) {
  CR_UNUSED5(interval, dir, origin, count, positionToShow);
  return false;
}

/// selects middle interval of specified type
bool ldomTextSelectionTool::selectMiddleInterval(interval_t interval,
                                                 ldomXPointer &positionToShow) {
...
/// moves selection, returns true if selection is moved, and it's necessary to
/// ensure positionToShow is visible on screen
bool ldomTextSelectionTool::doCommand(int cmd, int param,
                                      ldomXPointer &positionToShow) {
  CR_UNUSED(param);
  int count = 1;
  if (cmd > 1 && cmd < 20)
  ...
  case CMD_SEL_MIDDLE_INTERVAL: // select middle interval
                                // (word/sentence/paragraph depending on current
                                // setting) for initial interval
  ...
  case CMD_SEL_CUR_MODE_ALL: // set current mode to start+end
    _currOrigin = ALL;
    return false;
  case CMD_SEL_CUR_INTERVAL_TOGGLE: // toggle current interval:
  ...
  case CMD_SEL_START_FWD_BY_SENTENCE: // move selection start - forward by
                                      // sentence
  ...
  case CMD_SEL_START_FWD_BY_INTERVAL: // move selection start - forward by
                                      // current interval (word/sentence/para)
  ...
  case CMD_SEL_START_BACK_BY_SENTENCE: // move selection start - back by
                                       // sentence
    return moveBy(WORD, BACK, START, count, positionToShow);
  case CMD_SEL_START_BACK_BY_PARA: // move selection start - back by paragraph
    return moveBy(WORD, BACK, START, count, positionToShow);
  case CMD_SEL_START_BACK_BY_INTERVAL: // move selection start - back by
                                       // interval (word/sentence/para)
  ...
  case CMD_SEL_END_FWD_BY_INTERVAL: // move selection end - forward by interval
                                    // (word/sentence/para)
  ...
  case CMD_SEL_END_BACK_BY_INTERVAL: // move selection end - back by interval
                                     // (word/sentence/para)
  ...
  case CMD_SEL_ALL_FWD_BY_SENTENCE: // move selection start+end - forward by
                                    // sentence
    return moveBy(SENTENCE, FORWARD, ALL, count, positionToShow);
  case CMD_SEL_ALL_FWD_BY_PARA: // move selection start+end - forward by
                                // paragraph
    return moveBy(PARA, FORWARD, ALL, count, positionToShow);
  case CMD_SEL_ALL_FWD_BY_INTERVAL: // move selection start+end - forward by
                                    // interval (word/sentence/para)
  ...
  case CMD_SEL_ALL_BACK_BY_SENTENCE: // move selection start+end - back by
                                     // sentence
  ...
  case CMD_SEL_ALL_BACK_BY_INTERVAL: // move selection start+end - back by
                                     // interval (word/sentence/para)
    return moveBy(_currInterval, BACK, ALL, count, positionToShow);
  case CMD_SEL_CUR_FWD_BY_WORD: // move selection depending on current mode
                                // (start/end/both) - forward by word
    return moveBy(WORD, FORWARD, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_FWD_BY_SENTENCE: // move selection depending on current mode
                                    // (start/end/both) - forward by sentence
    return moveBy(SENTENCE, FORWARD, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_FWD_BY_PARA: // move selection depending on current mode
                                // (start/end/both) - forward by paragraph
    return moveBy(PARA, FORWARD, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_FWD_BY_INTERVAL: // move selection depending on current mode
                                    // (start/end/both) - forward by interval
                                    // (word/sentence/para)
    return moveBy(_currInterval, FORWARD, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_BACK_BY_WORD: // move selection depending on current mode
                                 // (start/end/both) - back by word
    return moveBy(WORD, BACK, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_BACK_BY_SENTENCE: // move selection depending on current mode
                                     // (start/end/both) - back by sentence
    return moveBy(SENTENCE, BACK, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_BACK_BY_PARA: // move selection depending on current mode
                                 // (start/end/both) - back by paragraph
    return moveBy(PARA, BACK, _currOrigin, count, positionToShow);
  case CMD_SEL_CUR_BACK_BY_INTERVAL: // move selection depending on current mode
                                     // (start/end/both) - back by interval
                                     // (word/sentence/para)
...
#ifdef _DEBUG
void testTxtSelector() { CRLog::info("testTxtSelector()"); }
#endif
