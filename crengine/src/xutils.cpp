...
static XImage img_template = {
    1,
    1, /* int width, height;         size of image (filled in by program)   */
...
void DrawBuf2Drawable(Display *display, Drawable d, GC gc, int x, int y,
                      LVDrawBuf *buf, unsigned *palette, int scale) {
    ...
    for (int yyi = 0; yyi < scale; yyi++) {
      if (buf->GetBitsPerPixel() == 2) {
        for (int xx = 0; xx < bytesPerRow; xx++) {
          ...
          int x0 = 0;
          for (int shift = 8 - buf->GetBitsPerPixel(); x0 < pixelsPerByte;
               shift -= buf->GetBitsPerPixel(), x0++) {
      ...
      XPutImage(display, d, gc, img.getXImage(), 0, 0, x, y + yy * scale + yyi,
                buf->GetWidth() * scale, 1);
      XFlush(display);
    }
  }
  ...
  XPutImage(display, d, gc, img.getXImage(), 0, 0, x, y,
            buf->GetWidth() * scale, buf->GetHeight() * scale);
