...
public:
  ...
  virtual lverror_t Seek(lvoffset_t offset, lvseek_origin_t origin,
    ...
    if (bw1 != bw2) {
      CRLog::error("Result position after Seek(%d, %x) doesn't match: %x / %x  "
                   " getpos1=%x, getpos2=%x",
                   (int)origin, (int)offset, (int)bw1, (int)bw2,
                   (int)m_base_stream1->GetPos(),
                   (int)m_base_stream2->GetPos());
  ...
  virtual lvpos_t SetPos(lvpos_t p) {
    ...
    MYASSERT(m_base_stream1->GetPos() == m_base_stream2->GetPos(),
             "setpos, pos");
    bool eof1 = m_base_stream1->Eof();
    bool eof2 = m_base_stream2->Eof();
    if (eof1 != eof2) {
      CRLog::error("EOF after SetPos(%x) don't match: %x / %x   getpos1=%x, "
                   "getpos2=%x, size1=%x, size2=%x",
                   (int)p, (int)eof1, (int)eof2, (int)m_base_stream1->GetPos(),
                   (int)m_base_stream2->GetPos(),
                   (int)m_base_stream1->GetSize(),
                   (int)m_base_stream2->GetSize());
    }
  ...
  virtual lverror_t Read(void *buf, lvsize_t count, lvsize_t *nBytesRead) {
    ...
    if (bw1 != bw2) {
      CRLog::error("BytesRead after Read(%d) don't match: %x / %x   "
                   "getpos1=%x, getpos2=%x   getsize1=%x, getsize2=%x",
                   (int)count, (int)bw1, (int)bw2,
    ...
    bool eof2 = m_base_stream2->Eof();
    if (eof1 != eof2) {
      CRLog::error("EOF after Read(%d) don't match: %x / %x   getpos1=%x, "
                   "getpos2=%x, size1=%x, size2=%x",
                   (int)count, (int)eof1, (int)eof2,
                   (int)m_base_stream1->GetPos(), (int)m_base_stream2->GetPos(),
                   (int)m_base_stream1->GetSize(),
                   (int)m_base_stream2->GetSize());
    }
  ...
  LVCompareTestStream(LVStreamRef stream1, LVStreamRef stream2)
      : m_base_stream1(stream1), m_base_stream2(stream2) {}
  ~LVCompareTestStream() {}
};

LVStreamRef LVCreateCompareTestStream(LVStreamRef stream1,
                                      LVStreamRef stream2) {
