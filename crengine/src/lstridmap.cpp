...
LDOMNameIdMapItem::LDOMNameIdMapItem(lUInt16 _id, const lString16 &_value,
...
bool LDOMNameIdMap::deserialize(SerialBuf &buf) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!item ||
        (item->id < m_size && m_by_id[item->id] != NULL)) { // invalid entry
      if (item)
        delete item;
      buf.seterror();
      return false;
...
static int compare_items(const void *item1, const void *item2) {
  return (*((LDOMNameIdMapItem **)item1))
      ->value.compare((*((LDOMNameIdMapItem **)item2))->value);
}
...
void LDOMNameIdMap::AddItem(LDOMNameIdMapItem *item) {
  ...
  if (item->id >= m_size) {
    ...
    m_by_id = (LDOMNameIdMapItem **)realloc(
        m_by_id, sizeof(LDOMNameIdMapItem *) * newsize);
    m_by_name = (LDOMNameIdMapItem **)realloc(
        m_by_name, sizeof(LDOMNameIdMapItem *) * newsize);
...
void LDOMNameIdMap::AddItem(lUInt16 id, const lString16 &value,
                            const css_elem_def_props_t *data) {
  if (id == 0)
    return;
