...
public:
  ...
  virtual bool hyphenate(const lChar16 *str, int len, lUInt16 *widths,
                         lUInt8 *flags, lUInt16 hyphCharWidth,
                         lUInt16 maxWidth);
...
bool HyphMan::activateDictionaryFromStream(LVStreamRef stream) {
  ...
  if (!method->load(stream)) {
    CRLog::error("HyphMan::activateDictionaryFromStream: Cannot open "
                 "hyphenation dictionary from stream");
    delete method;
    return false;
  }
  ...
  if (HyphMan::_dictList->find(lString16(HYPH_DICT_ID_DICTIONARY)) == NULL) {
    HyphDictionary *dict = new HyphDictionary(
        HDT_DICT_ALAN, cs16("Dictionary"), lString16(HYPH_DICT_ID_DICTIONARY),
        lString16::empty_str);
...
bool HyphDictionary::activate() {
  ...
  if (getType() == HDT_ALGORITHM) {
    ...
    CRLog::info("Selecting hyphenation dictionary %s",
                UnicodeToUtf8(_filename).c_str());
    LVStreamRef stream = LVOpenFileStream(getFilename().c_str(), LVOM_READ);
    if (stream.isNull()) {
      CRLog::error("Cannot open hyphenation dictionary %s",
                   UnicodeToUtf8(_filename).c_str());
      return false;
    }
    TexHyph *method = new TexHyph();
...
void HyphDictionaryList::addDefault() {
  if (!find(lString16(HYPH_DICT_ID_NONE))) {
    _list.add(new HyphDictionary(HDT_NONE, _16("[No Hyphenation]"),
                                 lString16(HYPH_DICT_ID_NONE),
                                 lString16(HYPH_DICT_ID_NONE)));
  }
  if (!find(lString16(HYPH_DICT_ID_ALGORITHM))) {
    _list.add(new HyphDictionary(
        HDT_ALGORITHM, _16("[Algorythmic Hyphenation]"),
...
bool HyphDictionaryList::open(lString16 hyphDirectory, bool clear) {
  ...
  LVStreamRef stream;
  if ((hyphDirectory.endsWith("/") || hyphDirectory.endsWith("\\")) &&
      LVDirectoryExists(hyphDirectory)) {
  ...
  if (!container.isNull()) {
    ...
    CRLog::info("no hyphenation dictionary items found in hyph directory %s",
                LCSTR(hyphDirectory));
  }
  return false;
}
...
public:
  ...
  int cmp(TexPattern *v) { return lStr_cmp(word, v->word); }

  static int hash(const lChar16 *s) {
    return ((lUInt32)(((s[0] * 31 + s[1]) * 31 + s[2]) * 31 + s[3])) %
           PATTERN_HASH_SIZE;
  }

  static int hash3(const lChar16 *s) {
    return ((lUInt32)(((s[0] * 31 + s[1]) * 31 + s[2]) * 31 + 0)) %
           PATTERN_HASH_SIZE;
  }

  static int hash2(const lChar16 *s) {
    return ((lUInt32)(((s[0] * 31 + s[1]) * 31 + 0) * 31 + 0)) %
           PATTERN_HASH_SIZE;
  }
  ...
  int hash() {
    return ((lUInt32)(((word[0] * 31 + word[1]) * 31 + word[2]) * 31 +
                      word[3])) %
           PATTERN_HASH_SIZE;
  }
...
#if DUMP_PATTERNS == 1
          CRLog::debug("Pattern matched: %s %s on %s %s",
                       LCSTR(lString16(p->word)), p->attr, LCSTR(lString16(s)),
                       mask);
#endif
          p->apply(mask);
          found = true;
...
public:
  HyphPatternReader(lString16Collection &result)
      : insidePatternTag(false), data(result) {
    result.clear();
  }
...
bool TexHyph::load(LVStreamRef stream) {
  ...
  if (w) {
    ...
    for (i = 0; i < hyph_count; i++) {
      ...
      //            lChar16 ch = hyph.wl;
      //            CRLog::debug("wl=%s mask=%c%c", LCSTR(lString16(&ch, 1)),
      //            hyph.mask0[0], hyph.mask0[1]);
...
#if DUMP_PATTERNS == 1
        CRLog::debug("Pattern: '%s' - %s", LCSTR(lString16(pattern->word)),
                     pattern->attr);
#endif
        addPattern(pattern);
        patternCount++;
...
#if DUMP_PATTERNS == 1
      CRLog::debug("Pattern: (%s) '%s' - %s", LCSTR(data[i]),
                   LCSTR(lString16(pattern->word)), pattern->attr);
#endif
      addPattern(pattern);
      patternCount++;
    }
...
bool TexHyph::hyphenate(const lChar16 *str, int len, lUInt16 *widths,
                        lUInt8 *flags, lUInt16 hyphCharWidth,
                        lUInt16 maxWidth) {
  if (len <= 3)
    return false;
...
bool AlgoHyph::hyphenate(const lChar16 *str, int len, lUInt16 *widths,
                         lUInt8 *flags, lUInt16 hyphCharWidth,
                         lUInt16 maxWidth) {
          ...
          for (j = i + 1; j < end; ++j) {
            if (chprops[j] & CH_PROP_VOWEL) {
              ...
              if (i - start >= 1 && end - i > 2) {
                ...
                if (nw < maxWidth) {
                  bool disabled = false;
                  const char *dblSequences[] = {"sh", "th", "ph", "ch", NULL};
