...
public:
  ...
  CRPropItem(const CRPropItem &v) : _name(v._name), _value(v._value) {}
...
protected:
  lInt64 _revision;
  bool findItem(const char *name, int nameoffset, int start, int end,
                int &pos) const;
  bool findItem(const char *name, int &pos) const;
  void clear(int start, int end);
  CRPropContainer(const CRPropContainer &v) : _list(v._list) {}
...
void CRPropAccessor::limitValueList(const char *propName,
                                    const char *values[]) {
...
void CRPropAccessor::limitValueList(const char *propName, int values[],
                                    int value_count) {
...
void CRPropAccessor::limitValueRange(const char *propName, int defValue,
...
lString16 CRPropAccessor::getStringDef(const char *propName,
...
lUInt32 CRPropAccessor::getColorDef(const char *propName,
                                    lUInt32 defValue) const {
  lUInt32 v = 0;
...
lvRect CRPropAccessor::getRectDef(const char *propName,
                                  const lvRect &defValue) const {
  lvRect v;
...
lvPoint CRPropAccessor::getPointDef(const char *propName,
                                    const lvPoint &defValue) const {
  lvPoint v;
...
lInt64 CRPropAccessor::getInt64Def(const char *propName,
                                   lInt64 defValue) const {
  lInt64 v = 0;
...
CRPropContainer::CRPropContainer() : _revision(0) {}

/// returns property path in root container
const lString8 &CRPropContainer::getPath() const { return lString8::empty_str; }

/// returns property item count in container
int CRPropContainer::getCount() const { return _list.length(); }
...
/// binary search
bool CRPropContainer::findItem(const char *name, int nameoffset, int start,
...
public:
  ...
  /// returns property path in root container
  virtual const lString8 &getPath() const { return _path; }
  /// returns property item count in container
  virtual int getCount() const {
    sync();
    return _end - _start;
  ...
  virtual void setString(const char *propName, const lString16 &value) {
    ...
    int pos = 0;
    if (!_root->findItem(propName, _path.length(), _start, _end, pos)) {
      _root->_list.insert(pos,
                          new CRPropItem((_path + propName).c_str(), value));
      _root->_revision++;
      sync();
    } else {
...
/// factory function
CRPropRef LVCreatePropsContainer() { return CRPropRef(new CRPropContainer()); }
