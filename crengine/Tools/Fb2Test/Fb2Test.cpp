  ...
  public:
    ...
    void addLine(const lChar16 *str, int flags, LVFontRef font) {
      ...
      txt.AddSourceLine(
          ...
          flags, 16,  /* interline space, *16 (16=single, 32=double) */
          30,         /* first line margin */
          NULL, 0);
    }
  };
  LVFontRef font1 =
      fontMan->GetFont(20, 300, false, css_ff_sans_serif, lString8("Arial"));
  LVFontRef font2 = fontMan->GetFont(20, 300, false, css_ff_serif,
                                     lString8("Times New Roman"));
  Tester t;
  t.addLine(L"Testing simple paragraph formatting. Just a test. ",
            LTEXT_ALIGN_WIDTH | LTEXT_FLAG_OWNTEXT, font1);
  t.addLine(L"Another fragment of text. ", LTEXT_FLAG_OWNTEXT, font1);
  t.addLine(L"And the last one written with another font", LTEXT_FLAG_OWNTEXT,
            font2);
  t.addLine(L"Next paragraph: left-aligned. ",
            LTEXT_ALIGN_LEFT | LTEXT_FLAG_OWNTEXT, font1);
...
lString8 OpenFileDialog(HWND hWndOwner) {
  ...
  ofn.lpstrFilter =
      "All supported files "
      "(*.fb2;*.txt;*.rtf;*.zip;*.tcr;*.html;*.epub)\0*.fb2;*.txt;*.rtf;*.zip;*"
      ".htm;*.html;*.tcr;*.epub\0FictionBook2 files (*.fb2)\0*.fb2\0ZIP files "
      "(*.zip)\0*.zip\0All Files (*.*)\0*.*\0\0";
  ofn.nFilterIndex = 1;
  ofn.lpstrFile = str;
...
void TestWol() {
  {
    ...
    wol.addTitle(lString8("book title"), lString8("subj."),
                 lString8("John Smith"), lString8("adapter"),
                 lString8("translator"), lString8("publisher"),
                 lString8("2006-11-01"), lString8("This is introduction."),
                 lString8("ISBN"));
              ...
              for (int i=0; i<zip->GetObjectCount(); i++)
              {
                  ...
                  if (item)
                  {
                      ...
                      *log << lString8::itoa( (int)item->GetSize() );
                      ...
                      LVStreamRef unpstream = zip->OpenStream( item->GetName(),
     LVOM_READ ); if (!unpstream.isNull())
                      {
                          *log << "\n arc stream opened ok \n";
                          LVStreamRef outstream = LVOpenFileStream(
     item->GetName(), LVOM_WRITE ); if ( !outstream.isNull() )
                          {
                              int copiedBytes = (int)LVPumpStream( outstream,
     unpstream ); *log << " copied " << lString8::itoa(copiedBytes) << "
     bytes\n";
                          }
                          else
                          {
  ...
  */
  ...
  if (!dir.isNull())
    for (i = 0; i < dir->GetObjectCount(); i++) {
      ...
      if (!item->IsContainer() && fileName.length() > 4 &&
          lString16(fileName, fileName.length() - 4, 4) == L".ttf") {
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  flags = WS_DLGFRAME | WS_MINIMIZEBOX | WS_SYSMENU |
          WS_VSCROLL; // WS_OVERLAPPEDWINDOW
  ...
  hWnd = CreateWindowW(L"CoolReader", L"CREngine - Simple FB2 viewer",
                       flags, // WS_OVERLAPPEDWINDOW
                       x, y, dx, dy, NULL, NULL, hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  case WM_VSCROLL: {
    ...
    case SB_THUMBTRACK:
      ...
      break;
    case SB_ENDSCROLL:
      DoCommand(hWnd, DCMD_GO_POS, text_view->GetPos());
      break;
    }
  } break;
  case WM_SIZE: {
