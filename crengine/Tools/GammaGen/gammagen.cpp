...
static const double gamma_levels[GAMMA_LEVELS] = {
    0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95,
    1,   1.05, 1.1, 1.15, 1.2, 1.25, 1.3, 1.35, 1.4, 1.45, 1.5, 1.55, 1.6, 1.65,
    1.7, 1.75, 1.8, 1.85, 1.9, 1.95, 2.0, 3.0,  5.0, 7.0,  11.0};

void genTable(FILE *out, double gamma, int index) {
  fprintf(
      out,
      "static const unsigned char gamma_table_%d[256] = { // gamma=%f\n    ",
      index, gamma);
  for (int i = 0; i <= 255; i++) {
...
int main(int argc, const char **argv) {
  ...
  fprintf(out, "// don't edit: this file is generated by "
               "crengine/Tools/GammaGen/gammagen.cpp\n");
  ...
  fprintf(out,
          "// corrects gamma for value 0..255, gamma_index must be 0..%d (%d "
          "means no correction)\n",
          GAMMA_LEVELS - 1, GAMMA_LEVELS / 2);
  fprintf(out,
          "inline unsigned char cr_correct_gamma( unsigned char value, int "
          "gamma_index ) { return cr_gamma_tables[gamma_index][value]; } \n");
  fprintf(out,
          "// corrects gamma for byte buffer; gamma_index must be 0..%d (%d "
          "means no correction)\n",
          GAMMA_LEVELS - 1, GAMMA_LEVELS / 2);
  fprintf(out, "void cr_correct_gamma_buf( unsigned char * buf, int size, int "
               "gamma_index );\n");
  fprintf(out, "\n\n");
  ...
  fprintf(out,
          ...
          GAMMA_LEVELS - 1, GAMMA_LEVELS / 2);
  fprintf(out,
          "void cr_correct_gamma_buf( unsigned char * buf, int size, int "
          "gamma_index ) {\n"
