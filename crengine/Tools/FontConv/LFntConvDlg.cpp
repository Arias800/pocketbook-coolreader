...
void CLFntConvDlg::OnPaint() {
  if (IsIconic()) {
    ...
    if (m_fontref.get()) {
      ...
      wchar_t text1[] = {'T',   'e',    's',   't',   ':',   ' ',    0x420,
                         0x443, 0x441,  0x441, 0x43A, 0x438, 0x439,  ' ',
                         0x44F, 0x00AD, 0x437, 0x44B, 0x43A, 0x00AD, 0};
      wchar_t text2[] = {0x41F, 0x440, 0x43E, 0x432, 0x435,
                         0x440, 0x43A, 0x430, 0};
      ...
      // lvdrawbufDrawText( &buf, 10, 3+lvfontGetHeader(m_bmpfont)->fontHeight,
      // m_bmpfont, text2, wcslen(text2), '?' );
...
HCURSOR CLFntConvDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }

void CLFntConvDlg::InitFontList() {}

DWORD GetGlyphIndicesW(HDC hdc,       // handle to DC
...
void CLFntConvDlg::FormatSampleText() {
  ...
  if (!font)
    return;
  m_frmtext->AddSourceLine(L"Centered paragraph!", 0, font,
                           LTEXT_FLAG_OWNTEXT | LTEXT_ALIGN_CENTER, 16, 0,
                           NULL);
  m_frmtext->AddSourceLine(
      L"Russian: "
      L"\x042D\x0442\x043E \x0442\x0435\x043A\x0441\x0442 \x043D\x0430 "
      L"\x0440\x0443"
      L"\x0441" SH L"\x0441\x043A\x043E\x043C "
      L"\x044F\x0437\x044B" SH L"\x043A\x0435. ",
      0, font, LTEXT_FLAG_OWNTEXT | LTEXT_ALIGN_WIDTH, 16, 40, NULL);
  m_frmtext->AddSourceLine(L"New para.", 0, font,
                           LTEXT_FLAG_OWNTEXT | LTEXT_ALIGN_WIDTH, 16, 40,
                           NULL);
  m_frmtext->AddSourceLine(
      L"This sentence should be appended to the same paragraph.", 0, font,
      LTEXT_FLAG_OWNTEXT, 16, 40, NULL);
  m_frmtext->AddSourceLine(
      L"This is right aligned paragraph. Is it wrapped properly? Check it!", 0,
      font, LTEXT_FLAG_OWNTEXT | LTEXT_ALIGN_RIGHT, 16, 0, NULL);
  m_frmtext->AddSourceLine(L"This is paragraph tests width alignment. "
                           L"First line has margin.",
                           0, font, LTEXT_FLAG_OWNTEXT | LTEXT_ALIGN_WIDTH, 16,
                           40, NULL);
...
void CLFntConvDlg::OnBtnChooseFont() {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    CFileDialog savedlg(false, "lbf", "bmpfont.lbf",
    ...
    #define GGI_MARK_NONEXISTING_GLYPHS 1
             ...
             //GetGlyphIndicesW(m_drawdc.m_hDC, s, 1, w,
...
int CLFntConvDlg::batchConvert(const char *fn) {
  ...
  if (f) {
    while (fgets(str, 2047, f)) {
      ...
      int bold = 0;
      int italic = 0;
      if (sscanf(str, "%s %d %d %d %s", fname, &size, &bold, &italic,
                 fontname) != 5)
        continue;
      if (size < 8 || size > 52 || bold < 0 || bold > 1 || italic < 0 ||
          italic > 1 || !fname[0] || !fontname[0])
        continue;
