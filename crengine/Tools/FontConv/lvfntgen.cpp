...
glyph_range_buf::glyph_range_buf() : pos(0) {
...
lvfont_glyph_t *font_gen_buf::addGlyph(unsigned short code) {
  ...
  if (!ranges[nrange])
    ranges[nrange] = new glyph_range_buf;
...
bool font_gen_buf::saveToFile(const char *fname) {
  ...
  int hdrsz =
      sizeof(lvfont_header_t) - (1024 - rangecount) * sizeof(unsigned long);
...
void font_gen_buf::init(int fntSize, int fntBaseline, int bitsPerPixel,
                        const char *fontName, const char *fontCopyright) {
...
void font_gen_buf::setDecodeTable(hrle_decode_info_t *table) {
  if (decodeTable)
    free(decodeTable);
  decodeTableSize = sizeof(hrle_decode_info_t) +
                    sizeof(hrle_decode_table_t) * (table->itemcount - 1);
