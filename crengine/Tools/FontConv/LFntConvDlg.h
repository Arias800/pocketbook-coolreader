...
public:
  ...
  int getDecodeTableSize() {
    return sizeof(hrle_decode_info_t) +
           sizeof(hrle_decode_table_t) * (m_decode_table_size - 1);
  }
  void getDecodeTable(hrle_decode_info_t *table) {
    ...
    table->leftmask = ((1 << m_decode_table_bits) - 1)
                      << (8 - m_decode_table_bits);
    memcpy(table->table, m_decode_table,
           sizeof(hrle_decode_table_t) * m_decode_table_size);
  }
  ...
  void Clear() {
    ...
    memset(m_encode_table, 0,
           sizeof(hrle_encode_table_t) * m_num_symbols * m_max_run_len);
  }
  ...
  static int RecurseBits(hrle_stats_t *table, int index, int level,
                         lUInt32 code) {
    int nb = 0;
    ...
    if (table[index].count == 0) {
      ...
      {
        int n =
            RecurseBits(table, table[index].right, level + 1, (code << 1) | 1);
        if (n > nb)
          nb = n;
      }
    }
  ...
  int MakeConfig(hrle_stats_t *srctable, hrle_stats_t *dsttable,
                 int num_items) {
    ...
    for (i = 0; i < num_items; i++) {
      if (dsttable[i].count && dsttable[i].stats > 0)
        run_index[dsttable[i].value * m_max_run_len + dsttable[i].count - 1] =
            (lUInt16)i;
    }
    for (i = m_num_symbols * m_max_run_len - 1; i >= num_items; i--) {
      if (srctable[i].count > 1 && srctable[i].stats > 0) {
        ...
        while (len > 0) {
          for (int j = (len >= m_max_run_len) ? m_max_run_len - 1 : len; j >= 0;
               --j) {
    ...
    for (;;) {
      min_index = -1;
      for (i = 0; i <= head; i++) {
        if (!dsttable[i].parent &&
            (min_index == -1 || min_stat > dsttable[i].stats)) {
          min_index = i;
      ...
      min_index2 = -1;
      for (i = 0; i <= head; i++) {
        if (!dsttable[i].parent && i != min_index &&
            (min_index2 == -1 || min_stat2 > dsttable[i].stats)) {
          min_index2 = i;
  ...
  void MakeTable() {
    ...
    hrle_stats_t *dsttable =
        new hrle_stats_t[m_num_symbols * m_max_run_len * 2 + 1];
    ...
    qsort(table, m_num_symbols * m_max_run_len, sizeof(hrle_stats_t),
          compare_items);
    int min_size = -1;
    ...
    for (; num_items >= m_num_symbols; num_items--) {
      memset(dsttable, 0,
             sizeof(hrle_stats_t) * (m_num_symbols * m_max_run_len * 2 + 1));
    ...
    for (i = 0; i < best_num_items; i++) {
      hrle_encode_table_t *p =
          &m_encode_table[dsttable[i].value * m_max_run_len +
                          dsttable[i].count - 1];
...
public:
  ...
  bool Init(HDC hdc, wchar_t code) {
    ...
    res = GetGlyphOutlineW(hdc, code, GGO_METRICS, &m_metrics, 0, NULL,
                           &identity);
    if (res == GDI_ERROR)
      return false;
    m_glyph_size = GetGlyphOutlineW(hdc, code,
                                    GGO_GRAY8_BITMAP, // GGO_METRICS
                                    &m_metrics, 0, NULL, &identity);
    ...
    res = GetGlyphOutlineW(hdc, code,
                           GGO_GRAY8_BITMAP, // GGO_METRICS
                           &m_metrics, m_glyph_size, m_glyph, &identity);
...
public:
  ...
  void SetCopyright(lString8 copyright) { m_copyright = copyright; }
  void SetCharsetFilter(DWORD flags) { m_charset_filter = flags; }
  bool FilterCode(wchar_t code) {
    return (code <= 0x00BF) || ((code >= 0x2000 && code <= 0x20AC)) ||
           ((m_charset_filter & LCHARSET_LATIN) && (code >= 0x00C0) &&
            (code <= 0x02A8)) ||
           ((m_charset_filter & LCHARSET_CYRILLIC) && (code >= 0x0401) &&
            (code <= 0x04E9)) ||
           ((m_charset_filter & LCHARSET_GREEK) && (code >= 0x0300) &&
            (code <= 0x03CE)) ||
           ((m_charset_filter & LCHARSET_CHINEESE) && (code >= 0x4E00) &&
            (code <= 0xFFFF));
  }
  CLVFntConvertor(LOGFONT *lf)
    ...
    m_buf.init(metric.tmHeight, 0, 2, m_logfont.lfFaceName,
               m_copyright.c_str());
    ...
    m_buf.hdr.fontBaseline =
        (unsigned char)(metric.tmHeight - metric.tmDescent);
  ...
  lUInt16 ConvertGlyphUnpacked(unsigned char *dst, unsigned char *src,
  ...
  lUInt16 ConvertGlyphPacked(lUInt8 *dst, unsigned char *src, int src_dx,
                             int src_dy) {
    int src_rowsize = (src_dx + 3) / 4 * 4;
    int shift = 0;
    int last_digit = -1;
  ...
  bool Convert(const char *fname) {
    ...
    for (int i = 32; i < 0xffff; i++) {
      // fprintf( f, "%04x   %d\n ", i,
      // CLFntGlyph::GetGlyphIndex(m_drawdc.m_hDC, i) );
      if (FilterCode(i) && glyph.Init(m_drawdc.m_hDC, i)) {
        if (glyph.m_glyph_size > 0)
          AddGlyphStats(glyph.m_glyph, glyph.m_metrics.gmBlackBoxX,
                        glyph.m_metrics.gmBlackBoxY);
      }
    }
    ...
    for (i = 32; i < 0xffff; i++) {
      ...
      if (FilterCode(i) && glyph.Init(m_drawdc.m_hDC, i)) {
        ...
        // dest->rowBytes = glyph.m_glyph_size?(dest->blackBoxX +
        // (pxPerByte-1))/pxPerByte:0; int bytes = dest->rowBytes *
        // dest->blackBoxY;
        if (dest->blackBoxY && dest->blackBoxX) {
          lUInt16 sz = ConvertGlyphPacked(dest->glyph, glyph.m_glyph,
                                          dest->blackBoxX, dest->blackBoxY);
          dest->glyphSize = sz;
        } else {
          dest->glyphSize = 0;
        }
