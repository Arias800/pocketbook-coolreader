...
void DrawBuf2DC(CDC &dc, int x, int y, draw_buf_t *buf, COLORREF *palette,
  ...
  for (int yy = 0; yy < buf->height; yy++) {
    ...
    for (int yyi = 0; yyi < scale; yyi++) {
      COLORREF *dst =
          m_drawpixels +
          ((buf->height * scale - (yy * scale + yyi) - 1)) * buf_width * scale;
      for (int xx = 0; xx < buf->bytesPerRow; xx++) {
        unsigned int b = src[xx];
        int x0 = 0;
        for (int shift = 8 - buf->bitsPerPixel; x0 < pixelsPerByte;
             shift -= buf->bitsPerPixel, x0++) {
  ...
  dc.BitBlt(x, y, buf_width * scale, buf->height * scale, &m_drawdc, 0, 0,
            SRCCOPY);
}
