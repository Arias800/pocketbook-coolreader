...
TinyDictWordList::TinyDictWordList()
    : dict(NULL), list(NULL), size(0), count(0) {}
...
public:
  void reset() { crc = crc32(0L, Z_NULL, 0); }
  unsigned get() { return crc; }
  unsigned update(const void *data, unsigned size) {
    crc = crc32(crc, (const unsigned char *)data, size);
    return crc;
  }
  unsigned update(unsigned char b) { return update(&b, sizeof(b)); }
  unsigned update(unsigned short b) { return update(&b, sizeof(b)); }
  unsigned update(unsigned int b) { return update(&b, sizeof(b)); }
  TinyDictCRC() { reset(); }
};
...
class TinyDictZStream {
  ...
  unsigned int readU32() {
    ...
    if (!error && f && fread(buf, 1, 4, f) == 4) {
      crc.update(buf, 4);
      return (((((((unsigned int)buf[3]) << 8) + buf[2]) << 8) + buf[1]) << 8) +
             buf[0];
    }
    error = true;
    return 0;
  ...
  bool zinit(unsigned char *next_in, unsigned avail_in, unsigned char *next_out,
             unsigned avail_out);
...
int TinyDictWord::compare(const char *str) const { return strcmp(word, str); }
...
bool TinyDictIndexFile::find(const char *prefix, bool exactMatch,
...
enum { DICT_TEXT, DICT_GZIP, DICT_DZIP };

bool TinyDictZStream::zinit(unsigned char *next_in, unsigned avail_in,
...
bool TinyDictZStream::readChunk(unsigned n) {
  ...
  if (err != Z_OK) {
    printf("Inflate error %s (%d). avail_in=%d, avail_out=%d \n", zStream.msg,
           err, (int)zStream.avail_in, (int)zStream.avail_out);
    free(tmp);
    return false;
  }
  if (zStream.avail_in) {
...
TinyDictZStream::TinyDictZStream()
    : f(NULL), size(0), txtpos(0), headerLength(0), error(false), chunks(NULL),
      offsets(NULL), chunkLength(0), chunkCount(0), zInitialized(false),
      packed_size(0), unp_buffer(NULL), unp_buffer_start(0), unp_buffer_len(0),
      unp_buffer_size(0) {
...
const char *TinyDictionary::getDictionaryName() { return name; }
...
/// searches dictionary for specified word, caller is responsible for deleting
/// of returned object
TinyDictWordList *TinyDictionary::find(const char *prefix, int options) {
  TinyDictWordList *list = new TinyDictWordList();
  list->setDict(this);
  if (index->find(prefix, (TINY_DICT_OPTION_STARTS_WITH & options) == 0,
                  *list) &&
      list->length() > 0)
    return list;
  delete list;
...
TinyDictionaryList::~TinyDictionaryList() { clear(); }

/// search all dictionaries in list for specified pattern
bool TinyDictionaryList::find(TinyDictResultList &result, const char *prefix,
                              int options) {
  result.clear();
  for (int i = 0; i < count; i++) {
...
/// destructor
TinyDictResultList::~TinyDictResultList() { clear(); }
...
void TinyDictResultList::add(TinyDictWordList *p) {
  if (count >= size) {
    size = size ? size * 2 : 32;
    list =
        (TinyDictWordList **)realloc(list, sizeof(TinyDictWordList *) * size);
  }
  list[count++] = p;
}
