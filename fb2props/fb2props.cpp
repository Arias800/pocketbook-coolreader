...
static lString16 getDateTimeString(time_t t, int langId) {
  ...
  sprintf(str,
#endif
           "%04d/%02d/%02d %02d:%02d", bt->tm_year + 1900, bt->tm_mon + 1,
           bt->tm_mday, bt->tm_hour, bt->tm_min);
  str[31] = 0;
...
int GetBookProperties(char *name, struct BookProperties *pBookProps,
                      int localLanguage) {
  ...
  // for ( int kk=0; kk<1000; kk++)
  {
    ...
    if (!arc.isNull()) {
        ...
        if (item) {
          if (!item->IsContainer()) {
            ...
            if (name.length() > 5) {
              ...
              if (pext[0] == '.' && pext[1] == 'f' && pext[2] == 'b' &&
                  pext[3] == '2') {
                fb2item = item;
              } else if (pext[0] == '.' && pext[1] == 'f' && pext[2] == 'b' &&
                         pext[3] == 'd') {
                fbditem = item;
              }
            }
      ...
      if (!bestitem)
        return 0;
      CRLog::trace("opening item %s from archive",
                   UnicodeToUtf8(bestitem->GetName()).c_str());
